...
void vdev_gfx_render_tui(void) {
    ...
    if (now - g_last_tui_render_time < 0.066) return;
    ...
    if (sdl_window) {
        ...
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT) {
                ...
                me.x = mx;
                me.y = my;
                me.left_button = (buttons & SDL_BUTTON(SDL_BUTTON_LEFT)) ? 1 : 0;
                me.right_button = (buttons & SDL_BUTTON(SDL_BUTTON_RIGHT)) ? 1 : 0;
                me.middle_button = (buttons & SDL_BUTTON(SDL_BUTTON_MIDDLE)) ? 1 : 0;
...
static void play_mml(VMContext *vm, const char *mml) {
    (void)vm;
    int tempo = 120;
    int octave = 4;
    int length = 4;
...
int vdev_sound_open(const char *filename) {
    ...
    float *samples = (float *)malloc(cvt.len_cvt);
    if (!samples) {
        free(cvt.buf);
        return -1;
    }
