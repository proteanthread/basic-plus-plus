/**
 * @file stmt_def.c
 * @brief DEFINT, DEFSNG, DEFDBL, and DEFSTR statement handlers.
 *
 * SECTION 1: WHAT IT DOES
 * - Parses and processes DEFINT, DEFSNG, DEFDBL, and DEFSTR statements.
 * - These statements configure implicit variable type overrides (A-Z) in both
 *   global scope and local procedure contexts.
 *
 * SECTION 2: WHY IT EXISTS
 * - Fulfills Phase 21 (GW-BASIC & QBASIC compatibility) by supporting implicit
 *   variable suffixes based on starting letters.
 *
 * SECTION 3: WHY IT WORKS THIS WAY
 * - It parses ranges (e.g. A-D, G) and single letters separated by commas,
 *   updating the VariableContext overrides table for the active scope prefix.
 *
 * SECTION 4: WHAT CAN BE CHANGED
 * - Error messages and parsing constraints.
 *
 * SECTION 5: WHAT CANNOT BE CHANGED
 * - Type mapping updates during runtime loops.
 *
 ...
 * - Core logic is platform-independent. Standard C17.
 *
 * SECTION 10: FUTURE EXPANSIONS
 * - Dynamic type override scopes and metadata definitions.
 *
...
static BppError parse_def_range(VMContext *vm, LexerContext *lex, ValueType type) {
    ...
    while (true) {
        ...
        if (next.type == TOK_MINUS) {
            if (clen != 2 || !isalpha((unsigned char)name[0])) {
