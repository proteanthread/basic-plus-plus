...
## 2. Description & Usage
Displays the specified program line for editing. If no line number is provided, it displays the first line of the program. The user can then retype the line (by typing the line number followed by new code) or press Enter to leave it unchanged.
...
`source/progmgmt/parser_progmgmt.c` - Function: `pi_parse_edit`

## 5. Implementation Details
Searches the program store (`rt->program->lines`) for the target line number. If found, it prints the line text to standard output. Because the BASIC++ REPL naturally replaces lines when a user types a line number, this command simply aids the user by showing the existing line so they can copy/modify it. Prohibited in bytecode-only mode.

## 6. Cross-References / See Also
AUTO, DELETE, LIST

## 7. Historical Context
Similar to the GW-BASIC EDIT command, which would place the line in the keyboard buffer. In modern terminal environments, it outputs the line so the user can copy-paste and modify.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Enter `10 PRINT "Hello World"`.
3. Type `EDIT 10` and press Enter.
4. The system will print `10 PRINT "Hello World"`.
5. You can now retype or ignore it.
