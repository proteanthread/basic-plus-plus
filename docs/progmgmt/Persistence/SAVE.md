...
- **"filename"**: A string expression or literal specifying the path and name of the file to save.
- **A**: (Optional) Saves the file in ASCII text format.
- **P**: (Optional) Saves the file in a protected (tokenized/binary) format.

### 2. Description & Usage
The `SAVE` keyword writes the current program residing in the interpreter's memory workspace to a file on the local disk. By default (if neither `, A` nor `, P` is specified), classic BASIC interpreters saved in a proprietary binary tokenized format. For BASIC++, the default behavior is to save as plain ASCII text unless compatibility settings force binary tokenization.

The `, A` flag forces the file to be saved as human-readable ASCII text.
The `, P` flag forces the file to be saved in a protected format, which prevents it from being `LIST`ed or edited once loaded back into memory.

**Edge Cases & Boundary Conditions:**
- **Overwrite Behavior:** If a file with the identical name already exists in the target directory, `SAVE` will silently overwrite it without prompting for confirmation.
- **Empty Workspace:** If memory is empty (i.e., just a `NEW` command was issued), `SAVE` will create an empty 0-byte file (or a file with only an EOF marker/header).
- **Invalid Paths:** Attempting to save to a read-only directory, an invalid drive letter, or using illegal characters in the filename will result in a `Bad file name` or `Path/File access error`.
- **String Evaluation:** The filename can be a dynamic string variable, meaning `SAVE F$` is valid.
...
SAVE "MYPROG.BAS"
```
*Expected Behavior:* Saves the workspace to `MYPROG.BAS`.
...
20 SAVE F$, A
```
*Expected Behavior:* Evaluates the variable `F$` and saves the program as ASCII to `APP_V2.BAS`.

### 4. Internal C-Source Mapping
- **Lexer**: `source/parser/lexer.c` (`KW_SAVE`)
- **AST Node**: `source/parser/ast.c` (`STMT_SAVE`)
- **Runtime Execution**: `source/progmgmt/parser_progmgmt.c -> pi_parse_save()`
- **VFS Abstraction**: `source/platform/vfs.c`

### 5. Implementation Details
The `pi_parse_save()` function evaluates the filename argument, ensuring it resolves to a valid `VAL_STRING`. It then calls the platform-agnostic file writing routine.

**Execution Paths and Struct Mutations:**
1. The interpreter locks the AST to ensure no modifications happen during the write traversal.
2. The `Program_Lines` linked list is iterated sequentially from the lowest line number to the highest.
3. For an ASCII save (`, A`), the detokenizer/unparser (`ast_to_string()`) is invoked on each line's AST node, converting the internal structs back into human-readable text strings, which are then written to the file descriptor.
4. For a protected save (`, P`), the program is serialized into a binary format, and an internal encryption or obfuscation flag is set in the header.

**Troubleshooting and Constraints:**
- **What CAN be changed:** The internal routine for ASCII detokenization can be updated to enforce specific casing (e.g., forcing all keywords to uppercase on save) or spacing.
- **What CANNOT be changed:** The `, A` and `, P` flags must remain functionally identical to legacy implementations. The file I/O must strictly use `platform.c` interfaces (never direct OS headers) to ensure BSD/Linux/Windows portability.
- **If it breaks:** If a program saves but cannot be loaded, inspect the detokenizer. Missing spaces around operators during the unparsing phase will create syntax errors upon reloading.
...
In GW-BASIC, `.BAS` files were tokenized binary by default to save disk space on 360K floppies, requiring `SAVE "X", A` to export to text. Because modern environments prioritize Git version control and readability, BASIC++ defaults to ASCII unless strictly configured otherwise.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type: `10 A=1 : 20 PRINT A` and press ENTER.
3. Type: `SAVE "DUMP.BAS", A` and press ENTER.
4. Exit the interpreter (`SYSTEM`).
5. Open `DUMP.BAS` in a standard text editor (like Notepad or cat on Linux).
6. Verify the file contains human-readable text: `10 A=1 : 20 PRINT A`.
