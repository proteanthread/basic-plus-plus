...
## 1. Syntax & Parameters
`RENAME old_filename AS new_filename
RENAME [SUB|FUNCTION|LABEL|VAR|ARRAY|MATRIX] old_name [AS] new_name`

## 2. Description & Usage
Dual-purpose command. It can rename a file on the physical filesystem. It can also rename internal program structures like variables, arrays, subroutines, or functions within the loaded program space.

## 3. Code Examples
```basic
RENAME "old.bas" AS "new.bas"
RENAME VAR X AS Y
```

## 4. Internal C-Source Mapping
`source/filemgmt/parser_filemgmt.c` - Function: `pi_parse_rename`

## 5. Implementation Details
For files, it relies on the C standard library `rename()` wrapped in platform-safe path handling. For internal structural renaming, it traverses the AST or symbol tables and updates the identifier strings in place. Any parsing errors yield an ERR_WHAT.

## 6. Cross-References / See Also
NAME, ERASE, KILL

## 7. Historical Context
GW-BASIC only supported RENAME / NAME for files. BASIC++ extends this to allow refactoring of internal symbols directly from the REPL.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Type `RENAME "test1.tmp" AS "test2.tmp"` (assuming test1.tmp exists).
3. Verify the file was renamed on disk.
