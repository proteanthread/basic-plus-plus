...
## 1. Syntax & Parameters
`REFORMAT [spaces_per_indent]`

## 2. Description & Usage
Automatically re-indents the entire program in memory based on structural control flow keywords (e.g., FOR, WHILE, DO, IF, SUB, FUNCTION). It strips existing leading spaces and applies consistent indentation, making code more readable.
...
`source/progmgmt/parser_cmds.c` - Function: `pi_parse_reformat_cmd`

## 5. Implementation Details
Iterates over `rt->memory->program`. For each line, it identifies structural keywords to increase or decrease the `indent_level`. It allocates new strings for the adjusted text and updates the line structures. Default indentation is 4 spaces. Prohibited in bytecode-only mode.

## 6. Cross-References / See Also
LIST, RENUM

## 7. Historical Context
A modern addition not found in classic GW-BASIC or QBASIC. It acts as an integrated code formatter, aligning with modern IDE capabilities.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Enter `10 FOR I=1 TO 5`
3. Enter `20 PRINT I`
4. Enter `30 NEXT I`
5. Type `REFORMAT 4` and press Enter.
6. Type `LIST` to see the indented code.
