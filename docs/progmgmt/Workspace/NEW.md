...
The `NEW` command clears the current program from memory, resetting the workspace. It is used to start a fresh project or clear out old state before loading a new program. 

**Edge Cases & Behavior Constraints:**
* **Unsaved Data:** `NEW` does not prompt for saving. Any unsaved program lines or variables in memory are immediately and irreversibly destroyed.
* **Variable Clearing:** `NEW` destroys all numeric, string, and array variables. String pool allocations are freed, and the symbol table is cleared.
* **File Handles:** All open files or streams are automatically closed, and their respective file buffers flushed to disk before the workspace is wiped.
* **Execution State:** If executed from within a running program, `NEW` immediately halts execution, wipes memory, and returns the user to the interactive prompt (`Ok`).
* **Memory Limits:** If memory was fragmented, `NEW` acts as a complete reset, restoring the full heap capacity for the active workspace.
...
**Executing NEW within a program (not recommended, but supported):**
```basic
...
* **Lexer:** `source/parser/lexer.c` (`KW_NEW`)
* **AST Node:** `source/parser/ast.c` (`STMT_NEW`)
* **Runtime Execution:** `source/progmgmt/parser_progmgmt.c` (`pi_parse_new()`), `source/core/vm_exec.c` (`vm_reset_workspace()`)

## 5. Implementation Details

When `NEW` is executed, the interpreter engine immediately halts any active execution loop. The core function `vm_reset_workspace(RuntimeState *rt)` is invoked.

**Execution Paths and Struct Mutations:**
1. **File Handles:** Iterates over `rt->open_files` array, invoking `fclose()` and `platform_file_close()` on any active descriptors.
2. **AST Deallocation:** Walks the `rt->ast_root` linked list of `LineNode` structs. Each node's bytecode payload is freed via `free()`, and the node itself is returned to the memory pool. `rt->ast_root` is set to `NULL`.
3. **Symbol Table:** Iterates the `rt->sym_table`. All dynamically allocated strings within `Value` unions (`val.string`) are freed via `strpool_free()`. Arrays are deallocated. The symbol table count is zeroed.
4. **State Flags:** `rt->state` is mutated to `STATE_IDLE`. `rt->pc` (program counter) is nulled.

**What can be changed:** The order of deallocation can be optimized, but file handles must always be closed first to ensure data integrity.
...
**Troubleshooting:** If the interpreter crashes after `NEW`, it usually indicates a double-free bug in the string pool or AST node deallocation logic. A memory leak after `NEW` implies missing cleanup in `vm_reset_workspace()`.
...
In GW-BASIC, `NEW` was the standard method to reset the environment. BASIC++ maintains this legacy behavior but integrates safe 64-bit memory management, ensuring no OS-level file handles are leaked during the wipe.
...
3. Type `10 A$ = "TEST"` and hit Enter.
4. Type `20 OPEN "test.txt" FOR OUTPUT AS #1` and hit Enter.
5. Type `RUN` and hit Enter to initialize the variables and open the file.
6. Type `NEW` and hit Enter.
7. Type `PRINT A$` and hit Enter. Verify that the output is empty or 0, proving variables were cleared.
8. Verify that the file `test.txt` is closed and accessible by the OS, proving `NEW` cleaned up file handles.
