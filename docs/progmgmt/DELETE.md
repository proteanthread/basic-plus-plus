...
## 2. Description & Usage
Deletes a specific program line or a contiguous range of program lines from the currently loaded program memory. It permanently removes the targeted lines from the AST.
...
`source/progmgmt/parser_progmgmt.c` - Function: `pi_parse_delete`

## 5. Implementation Details
Iterates backward through the program store (`rt->program->lines`) and invokes `program_delete(rt->program, ln)` for any line falling within the [start_line, end_line] inclusive range. Iterating in reverse prevents index shifting issues during deletion. Prohibited in bytecode-only mode.

## 6. Cross-References / See Also
AUTO, EDIT, RENUM, NEW

## 7. Historical Context
Matches GW-BASIC behavior. QBASIC (which lacks line numbers) uses a visual editor instead, but BASIC++ bridges the gap by providing REPL line manipulation.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Enter lines `10 PRINT "A"`, `20 PRINT "B"`, `30 PRINT "C"`.
3. Type `DELETE 20-30` and press Enter.
4. Type `LIST`. Only line 10 should remain.
