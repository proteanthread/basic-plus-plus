...
## 2. Description & Usage
Enables automatic line numbering mode during interactive entry. When activated, the interpreter automatically generates and prints the next sequential line number, prompting the user for code. Pressing Enter on an empty line or typing a single period (.) cancels AUTO mode.
...
`source/progmgmt/parser_progmgmt.c` - Function: `pi_parse_auto`

## 5. Implementation Details
Modifies the RuntimeState structure by setting `rt->auto_line` and `rt->auto_step`. The REPL loop checks these variables during each input cycle. The default start line is 10 and default step is 10. Maximum line number is clamped to LINE_NUMBER_MAX. Prohibited in bytecode-only mode.

## 6. Cross-References / See Also
EDIT, DELETE, RENUM

## 7. Historical Context
Fully compatible with GW-BASIC and QBASIC AUTO command. Standard BASIC environments typically implement this for convenience.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Type `AUTO 100, 20` and press Enter.
3. The prompt should show ` 100 `. Type `PRINT "HELLO"`.
4. The next prompt should be ` 120 `. Press Enter on the empty line to exit AUTO mode.
5. Type `LIST` to verify the lines.
