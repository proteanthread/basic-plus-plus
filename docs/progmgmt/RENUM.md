...
## 1. Syntax & Parameters
`RENUM [new_start_line][, step]`

## 2. Description & Usage
Renumbers all program lines starting from `new_start_line` with increments of `step`. It also automatically updates all line number references in GOTO, GOSUB, RESTORE, and THEN statements to match the new numbering scheme.
...
`source/progmgmt/parser_progmgmt.c` - Function: `pi_parse_renum`

## 5. Implementation Details
Allocates `old_nums` and `new_nums` mapping arrays. First pass updates all sequential line numbers. Second pass parses the string text of each line to identify jump keywords (GOTO, GOSUB, THEN, RESTORE), extracts the target number, and replaces it with the mapped new number. Allocates a new string buffer for modified lines. Prohibited in bytecode-only mode.

## 6. Cross-References / See Also
AUTO, DELETE, LIST

## 7. Historical Context
Fully compatible with GW-BASIC RENUM. Essential for line-number based development when space is needed between contiguous lines.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Enter `1 PRINT "A"`, `2 GOTO 1`.
3. Type `RENUM 10, 10` and press Enter.
4. Type `LIST`. The code should now be `10 PRINT "A"` and `20 GOTO 10`.
