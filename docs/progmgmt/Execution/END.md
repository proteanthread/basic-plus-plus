...
## 1. Syntax & Parameters
`END [exit_code]`
- `exit_code` (Optional): A numeric expression representing the return code back to the operating system (in non-interactive modes).

## 2. Description & Usage
The `END` command unconditionally terminates the execution of a running BASIC++ program, closes all open file channels, flushes output buffers, and returns the interpreter to immediate mode (or exits to the OS if running as a script).
While `STOP` merely suspends execution (allowing `CONT`), `END` completely finalizes the program state.

**Edge Cases & Boundary Conditions:**
- **Type Coercion and Overflows:** If an `exit_code` is provided as a floating-point number, it will be implicitly truncated to an integer. Extreme boundary values (e.g., numbers exceeding 32-bit signed integer limits) will trigger runtime deterministic trapping (`ERR_OVERFLOW`).
- **Open File Channels:** `END` invokes an automatic garbage collection and cleanup pass. If there are pending writes in standard I/O file buffers, `END` guarantees they are flushed to disk before the file handles are closed via platform abstraction layers.
- **Nested Execution:** If `END` is encountered inside a nested subroutine (`GOSUB`), an error handler, or a `FOR...NEXT` loop, the call stack is immediately unwound and destroyed. Return pointers are discarded.
- **End of File:** If the instruction pointer reaches the physical end of the file/AST without encountering an `END` statement, an implicit `END` is automatically executed.
...
10 PRINT "Executing..."
20 END
30 PRINT "This will never print."
```

**Example 2: Returning an Exit Code to the OS**
```basic
10 ERROR_FLAG = 1
20 IF ERROR_FLAG = 1 THEN END 1 ELSE END 0
```
...
20 ERROR 50
100 PRINT "Fatal Error Encountered."
110 END 255
```

## 4. Internal C-Source Mapping
- **Lexer**: `source/parser/lexer.c` (`KW_END`)
- **AST Node**: `source/parser/ast.c` (`STMT_END`)
- **Runtime Execution**: `source/progmgmt/parser_progmgmt.c -> pi_parse_end()`

## 5. Implementation Details
The runtime parser evaluates the instruction and delegates to the interpreter state engine. 
When `pi_parse_end()` is invoked, it sets the global execution flag `interp->state = STATE_IDLE`. It then iterates through the `interp->file_channels` array, calling standard C library `fclose()` (wrapped in `platform_file_close()`) on any open handles. Finally, it frees the subroutine call stack and clears the transient memory arena used for loop contexts.

**Struct Mutations:**
The `InterpreterState` struct undergoes significant mutation. `call_stack_ptr` is reset to 0. All `FileChannel` structs have their `is_open` boolean set to false. The `AST_Node` representing the `END` statement is evaluated. If an exit code is present, `eval_expr()` assigns the numeric value into a `Value` union (`val.type = VAL_INT`), which is then passed to the OS exit routine if applicable.

**Troubleshooting Scenarios & Error States:**
- **ERR_TYPE_MISMATCH:** Thrown if the `exit_code` parameter is a string expression instead of a numeric one.
- **ERR_OUT_OF_BOUNDS:** Thrown if the exit code exceeds platform-specific exit code limits (e.g., typically 0-255 on POSIX).
- **Modification Constraints:** 
  - *What CAN be changed:* The internal file flushing mechanism can be optimized for block writes.
  - *What CANNOT be changed:* `END` must never deallocate variables from the symbol table. Variables must remain queryable from the REPL after an `END`.

## 6. Cross-References / See Also
STOP, SYSTEM, RUN, CLEAR

## 7. Historical Context
Fully compatible with GW-BASIC and ECMA-116 standard structural management commands. In GW-BASIC, `END` did not accept an exit code; BASIC++ extends this functionality to better support modern scripting environments and bash/batch pipelines.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe` from your terminal.
2. Type: `10 PRINT "Hello"`
3. Type: `20 END`
4. Type: `30 PRINT "World"`
5. Type `RUN` and press Enter.
6. Verify that only "Hello" is printed and the interpreter returns to the `Ok` prompt without printing "World".
