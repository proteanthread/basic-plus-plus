...
42. `source/dialect/dialect.c` (active dialect tables)
43. `source/dialect/dialect_gwbs.c` (GW-BASIC dialect)
44. `source/dialect/dialect_ecma116.c` (ECMA-116 dialect)
...
52. `source/config/config_file.c` (dialect configuration file handling)
53. `source/modules/stdlib_core.c` (core standard library helpers)
54. `source/modules/stdlib_dialect.c` (dialect library helpers)
...
*   **Dialects**: `dialect_patb.c`, `dialect_trs1.c`, `dialect_trs2.c`, `dialect_ecma55.c`, `dialect_qbasic.c`, `dialect_aint.c`, `dialect_asft.c`, `dialect_atari.c`, `dialect_c64.c`, `dialect_coco.c`, `dialect_mbasic.c`, `dialect_sinclair.c`, `dialect_superbasic.c`, `dialect_sbasic.c`
...
4.  **Single-Dialect Compilation**: Modify `source/dialect/dialect.c` to compile and register only a single dialect (such as Palo Alto Tiny BASIC) and comment out the registration of all other dialects. This allows the compiler to completely strip out unused dialect files and dialect metadata tables, saving flash space.
