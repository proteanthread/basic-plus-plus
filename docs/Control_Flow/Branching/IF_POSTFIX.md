...
- `IF`: The statement executes *only if* the condition evaluates to true.

## 2. Description & Usage
BASIC++ fully supports JOSS-style postfix conditionals. Instead of wrapping a single statement inside a traditional `IF <condition> THEN <statement>` block, the conditional predicate can be appended to the end of the statement itself using `IF`. 

This leads to highly readable, natural-language-like source code, reducing visual clutter when implementing simple branch logic, bounds checking, or early returns. The parser evaluates the conditional expression first; if the condition is met, the preceding statement is executed. If the condition is not met, execution gracefully proceeds to the next line or instruction.
...
REM Example 3: Conditional assignment
X = 0
Y = 100
X = Y + 50 IF Y > 0
PRINT "X is now: "; X
...
During the parsing phase, when the parser identifies a standard executable statement, it looks ahead for the `IF` token trailing the statement before encountering an end-of-statement marker (`:`, `\n`, or `EOF`). If a trailing conditional token is detected, the parser inverts the standard AST node generation. 

Instead of generating the `<statement>` first, it generates a conditional jump block where the `<condition>` is evaluated initially. The target statement is only emitted into the bytecode stream if the jump condition evaluates to true. This ensures that any side-effects of the `<statement>` (like function calls or I/O) are completely deferred and safely avoided if the condition fails. Structural bounds (like `DIM`) bypass this check entirely.
...
Postfix conditionals originated in the **JOSS** (JOHNNIAC Open Shop System) programming language in the 1960s and were later popularized by languages like Perl and Ruby. While traditional dialects like GW-BASIC and QBASIC strictly enforced prefix conditionals (`IF condition THEN statement`), BASIC++ embraces postfix predicates to modernize syntax and promote cleaner, more expressive control flow reminiscent of standard English grammar.

## 8. Manual Testing Guide
1. Launch the BASIC++ console (`basicpp-console.exe` or `./baspp-console`).
2. Type `X = 10` and hit Enter.
3. Type `PRINT "HELLO" IF X = 10` and hit Enter. (It should print HELLO).
4. Type `PRINT "WORLD" IF X = 5` and hit Enter. (It should print nothing).
5. Attempt a bad syntax: `DIM A(10) IF X = 10` and ensure it properly throws a syntax error.
