...
### 2. Description & Usage
The `GOSUB` command instructs the BASIC++ interpreter to branch execution to a specified subroutine, temporarily suspending the current execution path. Before branching, the interpreter records the address (the line number or AST node of the statement immediately following the `GOSUB`) by pushing it onto the internal call stack. This allows a subsequent `RETURN` statement to resume execution precisely where it left off.

**Usage:**
Subroutines in BASIC++ are a primary mechanism for code reuse and modularity, particularly because classic BASIC lacks scoped functions (though modern dialects support them, `GOSUB` remains for strict compatibility). You use `GOSUB` to isolate repetitive tasks like screen rendering, math computations, or string processing.

**Edge Cases & Boundary Conditions:**
*   **Missing Target:** If the target `line_number` or `label` does not exist, a compile-time (or pre-execution AST resolution) `ERR_UNDEFINED_LINE` or `ERR_UNDEFINED_LABEL` is thrown.
*   **Stack Overflow:** Each `GOSUB` without a matching `RETURN` consumes a slot on the call stack. Deeply recursive `GOSUB` calls or infinite loops lacking a `RETURN` will eventually trigger an `ERR_STACK_OVERFLOW`. The maximum stack depth is platform-dependent but guaranteed to be at least 256 frames on 64-bit systems.
*   **Branching into loops:** Branching directly into a `FOR...NEXT` or `WHILE...WEND` block is highly discouraged and can lead to stack misalignment if the loop constructs were not initialized properly.
*   **Complex Syntax Combinations:** `GOSUB` can be the target of a conditional, e.g., `IF X > 10 THEN GOSUB 1000`.

### 3. Code Examples

**Standard GOSUB with Line Numbers:**
```basic
10 PRINT "Main execution started."
20 GOSUB 100
30 PRINT "Returned to main."
40 END
...
The `GOSUB` instruction is identified in the Lexer by the `KW_GOSUB` token.

**Parser Expectations:**
The parser expects a single argument resolving to an integer literal or a string literal representing a label. This target is resolved during the AST link phase.

**Struct Mutations:**
*   **`vm->call_stack`**: A new frame `vm_frame_t` is pushed onto the stack. This frame contains the return pointer (an index to the AST node or instruction pointer following the `GOSUB`).
*   **`vm->stack_pointer`**: Incremented by 1.
*   **`vm->pc` (Program Counter):** Mutated to point to the resolved target line or label.

**Execution Paths & Error Handling:**
1. Evaluation of the target (if it's computed, though strictly `GOSUB` takes literals/labels).
2. `vm_push_frame()` is called. If `vm->stack_pointer >= MAX_CALL_STACK`, an `ERR_OUT_OF_MEMORY` or `ERR_STACK_OVERFLOW` exception is triggered and execution halts.
3. The interpreter branches by modifying `vm->pc`. If the target is unresolved, `ERR_UNDEFINED_LINE` is thrown.

**What to do if it breaks:**
Check if `vm->stack_pointer` is incrementing correctly. Ensure that the resolution map (mapping line numbers to AST nodes) is fully populated before `GOSUB` executes.
...
`GOSUB` is one of the original Dartmouth BASIC commands. In GW-BASIC and QBASIC, `GOSUB` was strictly tied to line numbers (GW-BASIC) or labels (QBASIC). BASIC++ supports both seamlessly. Unlike modern languages with call frames containing local variables, a classic `GOSUB` shares the global variable scope unless explicitly sandboxed via extensions.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe` (Windows) or `baspp-console` (Linux).
2. Type the following program line by line:
   ```
   10 PRINT "A"
   20 GOSUB 100
   30 PRINT "C"
   ...
   ```
3. Type `RUN`.
4. The output must be exactly:
   ```
   A
   B
   C
   ```
5. To test stack overflow, type:
   ```
   10 GOSUB 10
   ```
   Type `RUN`. Ensure the interpreter gracefully catches the overflow with an error message rather than crashing the executable.
