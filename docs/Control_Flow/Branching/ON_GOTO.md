...
### 1. Syntax & Parameters
`ON <expression> GOTO <target_1>, [target_2], ..., [target_n]`

*   **`expression`**: A numeric expression that evaluates to an integer index (1-based).
*   **`target_n`**: A comma-separated list of integer line numbers or alphanumeric labels representing the possible branch targets.

### 2. Description & Usage
The `ON...GOTO` command evaluates a numeric expression and unconditionally branches to the n-th target in the provided list. It acts as a computed `GOTO` or a primitive switch-case statement. 

**Usage:**
This statement is extremely useful for menu systems, state machines, or parsing routines where multiple distinct execution paths exist based on a single variable or calculation. It prevents deep nesting of `IF...THEN...ELSE` statements, improving readability in classic dialects.

**Edge Cases & Boundary Conditions:**
*   **Index Out of Bounds (Zero or Negative):** If `expression` evaluates to 0 or a negative number, `ON...GOTO` throws an `ERR_ILLEGAL_FUNCTION_CALL`.
*   **Index Out of Bounds (Too Large):** If `expression` evaluates to a number greater than the number of targets provided in the list, execution silently falls through to the next physical line in the program. This is the standard behavior in GW-BASIC and QBASIC, and BASIC++ strictly emulates it.
*   **Floating-Point Evaluation:** If `expression` evaluates to a floating-point number (e.g., `1.6`), it is rounded to the nearest integer (becoming `2` in this case) before indexing the list.
*   **Missing Targets:** The list cannot end with a dangling comma. Empty slots (e.g., `ON X GOTO 10,,30`) are typically considered syntax errors in BASIC++.
...
70 GOTO 10
100 PRINT "Starting..." : END
200 PRINT "Loading..." : END
300 END
```
...
30 PRINT "X was not 1, 2, or 3. Fall-through successful."
40 END
```

**Expression Evaluation (Rounding):**
```basic
10 X = 1.8
20 ON X GOTO 100, 200
100 PRINT "Rounded to 1" : END
200 PRINT "Rounded to 2" : END
```
...
The statement is parsed by matching `KW_ON`, parsing an expression, and then matching `KW_GOTO` followed by a comma-separated list of targets.

**Parser Expectations:**
The parser constructs an AST node `AST_ON_GOTO`. It stores the evaluated `expression` as a child node, and dynamically allocates an array or linked list for the target line numbers/labels.

**Struct Mutations:**
*   **`vm->pc` (Program Counter):** Conditionally mutated to point to the resolved target line or label depending on the evaluation of `expression`.

**Execution Paths & Error Handling:**
1. Evaluate `expression` and store it in an integer `idx`.
2. Apply standard rounding rules if the result is floating-point.
3. If `idx <= 0` or `idx > 255` (system limit), throw `ERR_ILLEGAL_FUNCTION_CALL`.
4. If `idx > target_count`, yield execution and advance `vm->pc` to the next instruction sequentially (fall-through).
5. If `1 <= idx <= target_count`, extract the `idx`-th target from the AST node.
6. Resolve the target via `line_map` or `label_map`. If missing, throw `ERR_UNDEFINED_LINE`.
7. Mutate `vm->pc` to branch.

**What to do if it breaks:**
If it falls through unexpectedly, verify that rounding logic matches `QBASIC` specs. If rounding `1.5` yields `1` instead of `2` due to compiler-specific float-to-int casts, adjust the VM's internal rounding macro in `vm_branch.c`.
...
`ON...GOTO` originates from early BASIC environments lacking structured `SELECT CASE` statements. It provided rapid indexing into line tables, which executed very quickly because it mapped well to the CPU's computed jump instructions. Modern BASIC++ includes both `SELECT CASE` and `ON...GOTO` for maximum flexibility.
...
2. To test standard routing, type:
   ...
   300 PRINT "C" : END
   ```
   Type `RUN`. Output must be `B`.
3. To test fall-through, type:
   ```
   10 ON 4 GOTO 100, 200
   20 PRINT "Fall"
   30 END
   100 PRINT "No"
   ```
   Type `RUN`. Output must be `Fall`.
4. To test bounds error, type:
   ```
   10 ON 0 GOTO 100, 200
   ```
