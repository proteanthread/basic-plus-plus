...
*   **`line_number`** *(Optional)*: An integer literal representing a valid, existing line number to return to.
*   **`label`** *(Optional)*: An alphanumeric label to return to.

### 2. Description & Usage
The `RETURN` command concludes a subroutine previously invoked by a `GOSUB` or `ON...GOSUB` statement. When executed, it pops the top address from the internal call stack and branches execution to that address, effectively resuming the program right after the invoking `GOSUB`. 

If an optional `line_number` or `label` is provided, the interpreter still pops the stack to balance it, but execution branches to the specified target instead of the original return address. This is sometimes used for error handling or complex state machine exits, though it can severely hamper readability.

**Usage:**
Place `RETURN` at the logical end of your subroutines to transfer control back to the caller.
...
**RETURN with Target (Advanced):**
```basic
10 GOSUB 100
20 PRINT "This line is skipped."
30 PRINT "Resumed here."
40 END
100 PRINT "In Subroutine"
110 RETURN 30
```
...
The `RETURN` instruction is identified in the Lexer by the `KW_RETURN` token.

**Parser Expectations:**
The parser optionally matches an integer literal or string literal label. If present, it embeds the target into the AST node.

**Struct Mutations:**
*   **`vm->call_stack`**: The top frame `vm_frame_t` is popped. 
*   **`vm->stack_pointer`**: Decremented by 1.
*   **`vm->pc` (Program Counter):** Mutated to point to the popped return address OR the explicit target if provided.

**Execution Paths & Error Handling:**
1. Check `vm->stack_pointer`. If it is 0, throw `ERR_RETURN_WITHOUT_GOSUB`.
2. Extract the `return_address` from `vm->call_stack[vm->stack_pointer - 1]`.
3. Decrement `vm->stack_pointer`.
4. If an explicit target is provided, resolve it. If unresolved, throw `ERR_UNDEFINED_LINE`.
5. Set `vm->pc` to the chosen target.

**What to do if it breaks:**
If `ERR_RETURN_WITHOUT_GOSUB` fires unexpectedly, trace the execution path to see if the program counter naturally incremented into the subroutine code without branching. Ensure an `END` or `STOP` statement guards subroutines.
...
In traditional BASIC (like GW-BASIC), `RETURN <line>` was often used to exit error handlers or state machines, though it was considered a bad practice due to unstructured spaghetti code. BASIC++ preserves this capability for 100% backward compatibility but internally manages the stack cleanly to prevent resource leaks that plagued older implementations.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe` (Windows) or `baspp-console` (Linux).
2. To test `RETURN WITHOUT GOSUB`, type:
   ```
   10 PRINT "A"
   20 RETURN
   ```
   Type `RUN`. Ensure it throws `ERR_RETURN_WITHOUT_GOSUB`.
3. To test standard `RETURN`, type:
   ```
   10 GOSUB 100
   20 PRINT "Back"
   30 END
   ...
   110 RETURN
   ```
   Type `RUN`. Expected output: `Sub` then `Back`.
4. To test `RETURN <line>`, type:
   ```
   10 GOSUB 100
   20 PRINT "Fail"
   30 END
   100 RETURN 30
   ```
   Type `RUN`. Ensure "Fail" is skipped and execution safely ends.
