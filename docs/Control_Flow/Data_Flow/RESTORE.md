...
*   **`line_number`**: (Optional) An integer literal representing a valid line number in the current program where `DATA` statements exist.
*   **`label`**: (Optional) An alphanumeric label representing a specific section of the program.

### 2. Description & Usage
The `RESTORE` command alters the internal `DATA` pointer, changing the sequence of where the next `READ` statement will fetch its values from. 

**Usage:**
When called without arguments, `RESTORE` resets the internal data pointer back to the very first `DATA` constant in the program. When called with a line number or label, it sets the data pointer to the first `DATA` constant at or immediately following that specific line/label. This allows developers to reuse `DATA` sets, or categorize different `DATA` sets into logical blocks and retrieve them selectively.

**Edge Cases & Boundary Conditions:**
*   **Invalid Line/Label:** If a `line_number` or `label` is provided that does not exist in the program, an `ERR_UNDEFINED_LINE` or `ERR_UNDEFINED_LABEL` is thrown at runtime.
*   **No Data After Restore Point:** If `RESTORE` is called to a valid line number, but no `DATA` statements exist on or after that line, the pointer is set to EOF (End of File). The next `READ` command will immediately trigger an `ERR_OUT_OF_DATA`.
*   **Restore Without Data:** If the program contains no `DATA` statements at all, `RESTORE` executes successfully but the pointer remains null. 

### 3. Code Examples

**Basic RESTORE (Reset to Beginning):**
```basic
10 READ A, B
20 RESTORE
30 READ C, D
...
50 DATA 1, 2, 3, 4
```
*Output: ` 1 2 1 2 ` (Values 3 and 4 are never read)*

**RESTORE to a Specific Line Number:**
```basic
10 RESTORE 50
20 READ X, Y
30 PRINT X; Y
...
The `RESTORE` instruction is identified in the Lexer by the `KW_RESTORE` token.

**Parser Expectations:**
The parser optionally expects a single argument resolving to an integer literal (line number) or string (label). It compiles this into an `AST_RESTORE` node.

**Struct Mutations:**
*   **Data Pointer (`rt->data_ptr`):** Mutated to point to the `DATA` stream element corresponding to the beginning of the program, or the specific line/label provided.

**Execution Paths & Error Handling:**
1. Check if the `AST_RESTORE` node has an argument.
2. If no argument, set `rt->data_ptr = rt->data_head`.
3. If an argument exists, consult the global `line_map` or `label_map`.
4. If the target does not exist, throw `ERR_UNDEFINED_LINE`.
5. If the target exists, traverse the AST sequentially from that target node until the first `AST_DATA` node is found.
6. Set `rt->data_ptr` to the head of the data constants associated with that `AST_DATA` node. If no subsequent `DATA` node exists, set `rt->data_ptr = NULL`.

**What to do if it breaks:**
If `RESTORE` to a specific line number fails to reset the data pointer correctly, check the AST traversal logic inside the `AST_RESTORE` executor in `runtime.c`. The executor must sequentially search forward in the AST from the requested line number until it encounters the next `DATA` statement. Ensure the global `line_map` index is providing the correct starting node.
...
The ability to `RESTORE` to a specific line number was introduced in extended BASICs (like Microsoft BASIC) to give developers the flexibility to group static data hierarchically, rather than forcing a strict global sequential access pattern. BASIC++ fully supports this ECMA-116 and QBASIC capability.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe` (Windows) or `baspp-console` (Linux).
2. To test standard reset, type:
   ```
   10 READ A
   20 RESTORE
   30 READ B
   ...
   50 DATA 100, 200
   ```
   Type `RUN`. Output must be `100 100`.
3. To test RESTORE to line number, type:
   ```
   10 RESTORE 40
   20 READ A
   30 PRINT A
   ...
   50 DATA 60
   ```
   Type `RUN`. Output must be `50`.
4. To test RESTORE to an invalid line, type:
   ```
   10 RESTORE 999
   ```
