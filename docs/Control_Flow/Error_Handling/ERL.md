...
## 1. Syntax & Parameters
`ERL`
- No parameters. `ERL` is invoked as a read-only system variable containing an integer value.
...
If an error is triggered in direct (immediate) mode, or if a program has not encountered any errors during its execution cycle, `ERL` evaluates to `0`. Furthermore, `ERL` is read-only; attempts to assign a value to it (e.g., `LET ERL = 100`) will result in a Syntax Error. `ERL` works symbiotically with the `ERR` system variable, which holds the corresponding error code.
...
30 X = 10 / 0
40 PRINT "This line will not execute."
50 END

1000 REM --- Error Handler ---
1010 PRINT "A critical error occurred!"
1020 PRINT "Error Code (ERR): "; ERR
1030 PRINT "Error Line (ERL): "; ERL
1040 IF ERL = 30 THEN PRINT "Division by zero detected on line 30."
1050 RESUME NEXT
```

## 4. Internal C-Source Mapping
- `source/core/eval.c`: Evaluates `TOK_ERL` during expression parsing and pushes the `state->last_error_line` to the evaluation stack.
- `source/runtime/exec.c`: Intercepts thrown exceptions and commits the current execution line to `last_error_line`.
- `source/parser/parser_expr.c`: Handles the parsing logic for `KW_ERL`.
- `source/lexer/lexer.c`: Contains the lexical definition mapping the string `"ERL"` to `KW_ERL`.

## 5. Implementation Details
The BASIC++ interpreter persistently tracks the currently executing line number within the active runtime state (`state->last_error_line` or `rt->last_err_line`). When a runtime error exception is raised (e.g., via `error_raise`), the execution loop intercepts the failure before branching to the registered error trap routine. During this interception, the current line number is captured and finalized into the `last_error_line` state struct. 

When the expression evaluator encounters the `ERL` system variable, it directly queries this state struct and pushes the stored 16-bit or 32-bit integer onto the evaluation stack as a `TYPE_INTEGER`. Because the evaluator yields a literal integer evaluation for `ERL`, the AST parser explicitly forbids mutations against it.

If `ERL` breaks or fails to yield the correct line number, verify that `exec.c` is correctly snapshotting the line number prior to the stack unwind triggered by the error state. If `ERL` is returning 0 unexpectedly, verify that the error wasn't triggered in a zero-line context (like direct mode).
...
In historic interpreters such as GW-BASIC and QBASIC, `ERL` returned the closest preceding line number. If a program was written entirely without line numbers (as was common in QBASIC), `ERL` was notoriously unreliable and often returned 0 or an unrelated line. In BASIC++, `ERL` has been engineered to strictly return the exact executing line number of the failure, ensuring complete ECMA-116 and Microsoft compatibility while operating safely within modern 64-bit architectures.

## 8. Manual Testing Guide
To manually verify the integrity of the `ERL` keyword:
...
2. Enter the following test program:
   ...
   20 ERROR 11
   30 END
   100 PRINT "Error on line: "; ERL
   110 RESUME NEXT
   ```
3. Type `RUN` and press Enter.
4. The console must output: `Error on line:  20`.
5. This confirms that the `ERROR` statement successfully mutated the internal `last_error_line` state, and the `ERL` variable successfully retrieved it.
