...
## 2. Description & Usage
Returns the system-level error message string corresponding to the last error encountered. While `ERR` returns the numeric error code, `ERR$` provides the explicit human-readable description of that error. It is essential for modern logging, debugging, and dynamic error output where relying on a manual lookup table is insufficient.

## 3. Code Examples
```basic
10 ON ERROR GOTO 100
20 OPEN "I", 1, "NONEXISTENT.TXT"
30 END
...
- Function: `stdlib_core_err_str` in `source/core/stdlib_core.c`
- Registration: `builtins.c` under `KW_ERR_STR`

## 5. Implementation Details
The function accesses the global runtime error state (`rt->err_code`) and queries the error registry to retrieve the string description associated with the current `ERR` value.
**Edge Cases & Boundary Conditions:**
- If no error has occurred since the program started (i.e., `ERR` is `0`), `ERR$` safely returns a default string `"Uncategorized Error"` or an empty string, rather than referencing a null pointer.
- The string returned is deeply allocated inside the temporary string pool, ensuring it does not overwrite the global immutable error registry map in `error_db.c`.
- **Troubleshooting**: If `ERR$` triggers an `OUT OF MEMORY` error in rare scenarios, the garbage collector may have failed to free previous string pool objects. This usually means your error trap loop is recursively generating strings without returning control flow to clear the transient scratch pool.

## 6. Cross-References / See Also
- `ERR`, `ERL`, `ERROR`, `ON ERROR GOTO`

## 7. Historical Context
In traditional GW-BASIC, programmers had to refer to the printed manual for error code meanings. `ERR$` is a modern addition in BASIC++ that allows dynamic string representation of error codes, bringing the language closer to modern exception-handling conventions.

## 8. Manual Testing Guide
**Interactive Test:**
1. Launch `basicpp-console.exe`.
2. Type `ERROR 53: PRINT ERR$` and verify it prints "File not found" or the exact system equivalent message.
...
110 IF ERR$ = "Division by zero" THEN PRINT "SUCCESS!"
120 RESUME NEXT
```
