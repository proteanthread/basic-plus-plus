...
*   **`error_code`**: An integer expression representing the error code to be triggered. Standard error codes match classic GW-BASIC (e.g., 5 for Illegal Function Call, 11 for Division by Zero), but users can specify custom values.

### 2. Description & Usage
The `ERROR` statement forcefully simulates a runtime exception or triggers a user-defined error condition. When executed, it behaves exactly as if a built-in system error had occurred, populating the `ERR` and `ERL` variables and branching execution to the active error handler (if one exists).

**Usage:**
`ERROR` is primarily used for testing error-handling routines (by simulating standard errors) or for throwing custom domain-specific errors in large applications. If an `ON ERROR GOTO` handler is active, execution branches to it. If no handler is active, the program halts and prints the standard error message (or "Uncatchable error" for unknown codes) to the console.

**Edge Cases & Boundary Conditions:**
*   **Out of Bounds Error Codes:** Passing very large integers or negative integers might result in unexpected behavior or map to internal system exceptions. BASIC++ should cleanly handle or constrain these.
*   **No Active Handler:** Triggering `ERROR` without an `ON ERROR GOTO` handler will immediately terminate the running program.
*   **Floating-Point Error Codes:** If `error_code` evaluates to a float, it is typically truncated or rounded to an integer before being thrown.
...
```basic
10 ON ERROR GOTO 50
20 PRINT "Simulating division by zero..."
30 ERROR 11
40 END
50 PRINT "Caught error: "; ERR; " on line "; ERL
60 RESUME NEXT
```
...
50 END
ErrorHandler:
60 IF ERR = 200 THEN PRINT "Custom error: Value must be positive!"
70 RESUME 20
```
...
The `ERROR` instruction is parsed by the Lexer as `KW_ERROR`.

**Parser Expectations:**
The parser expects a single argument resolving to a numeric expression following the `ERROR` keyword. This is validated during the AST generation phase.

**Struct Mutations:**
*   **`vm->error_code`**: Set to the evaluated value of `error_code`.
*   **`vm->error_line`**: Set to the current line number where `ERROR` was executed.
*   **`vm->pc` (Program Counter)**: Modified to point to the error handler target if one is active, otherwise the execution halts.

**Execution Paths & Error Handling:**
1. Evaluate the `error_code` expression. Convert it to a strict integer.
2. Call `vm_throw_error(vm, evaluated_code)`.
3. The VM checks if `vm->on_error_target` is set.
4. If set, it updates `vm->pc` and sets the `vm->in_error_handler` flag.
5. If not set, it prints the error to the console, performs resource cleanup, and halts the VM back to the prompt.

**What to do if it breaks:**
If `ERROR` causes a segfault, check `vm_throw_error` for safe handling of arbitrary integers, ensuring array-out-of-bounds does not occur when mapping the error code to a built-in string message. Ensure `vm_throw_error` properly handles floating-point casting constraints.
...
In GW-BASIC, `ERROR` was the definitive way to test `ON ERROR` handlers or create software-based interrupts in business logic. BASIC++ retains full compatibility with this behavior, allowing script authors to define robust, domain-specific error state machines.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe` (Windows) or `baspp-console` (Linux).
2. To test throwing an error without a handler, type:
   10 ERROR 11
   Type `RUN`. Ensure the console prints a "Division by zero" error and halts.
3. To test with a handler, type:
   10 ON ERROR GOTO 40
   20 ERROR 255
   30 END
   40 PRINT ERR
   50 RESUME NEXT
   Type `RUN`. Output must be `255` followed by a clean program exit.
