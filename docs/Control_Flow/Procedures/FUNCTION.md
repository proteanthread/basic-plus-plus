...
*   **`name`**: The alphanumeric identifier for the function. It implies the return type (e.g., `name$` for string return, `name` for numeric).
*   **`parameter_list`**: An optional, comma-separated list of variables passed to the function.
*   **`statements`**: The body of the function.
*   **`return_expression`**: The value assigned to the function's name, which acts as the return value.

### 2. Description & Usage
The `FUNCTION` block defines a structured, multi-line procedure that returns a value. It provides isolated local variable scoping and parameter passing, representing modern structured programming in BASIC.

**Usage:**
Unlike `DEF FN`, `FUNCTION` blocks can contain loops, branching, and complex logic. The function is invoked by using its name in an expression (e.g., `Y = MyFunc(X)`). The return value is established by assigning a value to the function's name within its body.

**Edge Cases & Boundary Conditions:**
*   **No Return Assignment:** If execution reaches `END FUNCTION` without assigning a value to the function's name, it returns a default value (0 for numeric, `""` for string).
*   **Scope Isolation:** Variables inside a `FUNCTION` are entirely local to that invocation unless explicitly defined as `SHARED`. Global variables cannot be accessed directly without passing them as parameters or declaring them `SHARED`.
*   **Early Exit:** `EXIT FUNCTION` immediately terminates the function, returning whatever value was last assigned to the function's name.
...
The `FUNCTION` definition is identified by the `KW_FUNCTION` token in the Lexer.

**Parser Expectations:**
The parser treats `FUNCTION` ... `END FUNCTION` as an isolated block. It registers the signature in `vm->proc_registry` (if not already handled by a `DECLARE`) and creates a local variable symbol table attached to the block.

**Struct Mutations:**
*   **`vm->call_stack`:** Modified during execution to hold local state, returning to the expression evaluator upon completion.
*   **`rt->return_value`:** Used to ferry the evaluated result back to the calling expression.

**Execution Paths & Error Handling:**
1. A function call is encountered inside an expression.
2. The VM looks up the AST node for the `FUNCTION` in `vm->proc_registry`.
3. Pushes a new frame to `vm->call_stack`.
4. Initializes local parameters from passed arguments.
5. Executes the function body.
6. If `EXIT FUNCTION` or `END FUNCTION` is hit, evaluates the variable matching the function name.
7. Pops the stack frame, returning the value to the expression evaluator.

**What to do if it breaks:**
Scope leaks are the primary concern. If a local variable inside a `FUNCTION` accidentally overwrites a global variable in `vm_procedures.c`, ensure that the symbol table lookup strictly bounds variable resolution to the current `call_stack` frame before falling back to global scope (and only if `SHARED`).
...
Introduced in QuickBASIC / QBASIC to provide true structured procedural programming, replacing the single-line limitation of `DEF FN` and standardizing return value mechanics in complex applications.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe` (Windows) or `baspp-console` (Linux).
2. To test basic functionality, type:
   ```
   10 PRINT GetTen()
   20 END
   30 FUNCTION GetTen()
   ...
   50 END FUNCTION
   ```
   Type `RUN`. Output must be `10`.
3. To test scoping isolation, type:
   ```
   10 X = 5
   20 PRINT TestFunc()
   30 PRINT X
   40 END
   50 FUNCTION TestFunc()
   60 X = 99
   70 TestFunc = X
   80 END FUNCTION
   ```
   Type `RUN`. Output must be `99` then `5`.
