...
### 1. Syntax & Parameters
```basic
SUB <name> [([<parameter_list>])]
  [statements]
END SUB
```

*   **`name`**: The alphanumeric identifier for the subroutine.
*   **`parameter_list`**: An optional, comma-separated list of variables passed to the subroutine.
*   **`statements`**: The body of the subroutine containing the logic to be executed.

### 2. Description & Usage
The `SUB` block defines a structured, multi-line procedure that does not return a value (analogous to a `void` function in C). It is used for modularizing code, performing actions, and mutating state via pass-by-reference parameters.

**Usage:**
A `SUB` is invoked either with the `CALL` keyword or implicitly by its name. It provides a secure, locally-scoped environment, preventing variables inside the subroutine from accidentally overwriting global variables of the same name unless explicitly requested via `SHARED`.

**Edge Cases & Boundary Conditions:**
*   **Variable Scope:** Variables are local to the `SUB` unless declared `SHARED`.
*   **Early Exit:** `EXIT SUB` immediately terminates the subroutine, returning control to the statement following the caller.
*   **Recursive Calls:** A `SUB` can call itself. The interpreter handles recursive depth by allocating new local variable frames on the `call_stack`. Extreme recursion will trigger `ERR_OUT_OF_MEMORY`.

### 3. Code Examples

**Basic Subroutine:**
```basic
DECLARE SUB DrawLine (Length)
...
The `SUB` statement is processed when the Lexer identifies `KW_SUB`.

**Parser Expectations:**
The parser parses the `SUB` definition, storing the block of statements in the AST and registering the prototype in `vm->proc_registry`. The block is effectively skipped during normal sequential execution; it is only executed when invoked by a `CALL` or implicit call node.

**Struct Mutations:**
*   **`vm->call_stack`:** Used to track the return address `vm->pc` and local symbol tables.
*   **`vm->proc_registry`:** The dictionary holding all defined subprograms and functions.

**Execution Paths & Error Handling:**
1. A call jumps `vm->pc` to the first node inside the `SUB`.
2. A new frame is created on `vm->call_stack`.
3. Arguments are mapped to parameters (creating reference pointers for variables passed).
4. Normal sequential execution proceeds within the block.
5. Encountering `EXIT SUB` or `END SUB` triggers a stack pop, restoring the caller's `vm->pc` and symbol table environment.

**What to do if it breaks:**
If a `SUB` causes a crash on return, check `vm_procedures.c` to ensure `END SUB` is properly popping the `call_stack` and restoring the `pc` without off-by-one errors in instruction alignment. Ensure parameter references are correctly deallocated or unlinked.
...
Prior to structured BASIC variants (QBASIC), developers used `GOSUB` paired with line numbers. `SUB` modernized BASIC by introducing named procedures, explicit parameters, and local scope, shifting the paradigm away from global variables and line-number spaghetti.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe` (Windows) or `baspp-console` (Linux).
2. To test a basic sub call, type:
   ```
   10 CALL MySub()
   20 END
   30 SUB MySub()
   ...
   50 END SUB
   ```
   Type `RUN`. Output must be `Inside SUB`.
3. To test early exit, type:
   ```
   10 CALL ExitTest()
   20 END
   30 SUB ExitTest()
   ...
   70 END SUB
   ```
   Type `RUN`. Output must be `A`, and `B` must not print.
