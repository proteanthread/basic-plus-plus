...
*   **`variable`**: A numeric scalar variable (typically integer or floating-point) used as the loop counter.
*   **`start_value`**: A numeric expression representing the initial value assigned to the `variable`.
*   **`end_value`**: A numeric expression representing the termination threshold.
*   **`step_value`**: (Optional) A numeric expression representing the increment added to the `variable` each iteration. Defaults to 1 if omitted.

### 2. Description & Usage
The `FOR` statement initiates a definite loop that repeats a block of code a specific number of times. It initializes the loop counter and evaluates the termination condition. Execution proceeds into the loop block, and upon encountering the corresponding `NEXT` statement, the counter is incremented by the `step_value`.

**Usage:**
`FOR...NEXT` loops are fundamental for iterating over arrays, executing deterministic sequences, and mathematical computations. They offer a highly structured alternative to `GOTO` loops, ensuring that initialization, condition checking, and incrementing are tightly coupled and less prone to manual errors.

**Edge Cases & Boundary Conditions:**
*   **Zero Step:** If `step_value` evaluates to 0, it creates an infinite loop unless explicitly broken via `EXIT FOR` or a branching statement. The interpreter yields during each iteration to prevent UI freeze.
*   **Start Exceeds End:** If `start_value` > `end_value` (with a positive step) or `start_value` < `end_value` (with a negative step), the loop block is entirely skipped, and execution resumes immediately after the corresponding `NEXT` statement.
*   **Variable Mutation:** The loop variable can be manually mutated inside the loop block. This affects the termination condition natively.
...
The `FOR` instruction is identified in the Lexer by the `KW_FOR` token.

**Parser Expectations:**
The parser expects an assignment sequence, the `KW_TO` token, and optionally the `KW_STEP` token. It constructs an AST node (`AST_FOR`) containing references to the variable, start expression, end expression, and step expression. It also binds a linkage to the corresponding `NEXT` statement during AST resolution.

**Struct Mutations:**
*   **`vm->loop_stack`:** Pushes a new loop frame containing the variable pointer, end value, step value, and the AST node pointer of the loop start.

**Execution Paths & Error Handling:**
1. Evaluate `start_value`, `end_value`, and `step_value`.
2. Assign `start_value` to `variable`.
3. Check if the initial condition prevents entry. If so, jump PC to the instruction following `NEXT`.
4. Push loop context to `vm->loop_stack`.
5. If the stack overflows, throw `ERR_OUT_OF_MEMORY` or `ERR_LOOP_STACK_OVERFLOW`.

**What to do if it breaks:**
If loops iterate incorrectly, verify the AST linkage in `parser_loops.c` correctly identifies the matching `NEXT`. For logic errors, inspect `vm_loops.c` to ensure `step_value` sign properly dictates the comparison operator (<= vs >=) during the boundary check.
...
The `FOR` statement has existed since the original Dartmouth BASIC (1964). In GW-BASIC, loop contexts were stored on a fixed-size internal stack, making deep nesting risky. BASIC++ utilizes dynamic allocation for the `loop_stack` but retains strict ECMA-116 semantics.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe` (Windows) or `baspp-console` (Linux).
2. To test standard iteration, type:
   ```basic
   10 FOR I = 1 TO 3
   20 PRINT I
   30 NEXT I
   ```
   Type `RUN`. Output must be `1`, `2`, `3`.
