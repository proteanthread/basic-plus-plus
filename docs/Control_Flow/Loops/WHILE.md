...
*   **`condition`**: A numeric expression that evaluates to true (non-zero) or false (zero).

### 2. Description & Usage
The `WHILE` statement initiates a pre-test loop. It evaluates the `condition` before entering the loop block. If the condition is true, execution proceeds to the statements inside the block. When a `WEND` statement is encountered, execution jumps back to the `WHILE` statement to re-evaluate the condition. If the condition is false, execution branches immediately to the statement following the corresponding `WEND`.

**Usage:**
`WHILE...WEND` loops are ideal for situations where the number of iterations is unknown beforehand, such as reading from a file until EOF, or waiting for user input. It provides a structured mechanism superior to using `IF...THEN GOTO` constructs.

**Edge Cases & Boundary Conditions:**
*   **Infinite Loops:** If the `condition` never evaluates to false and no `EXIT` is invoked, the loop runs infinitely. BASIC++ ensures UI responsiveness via `vm_yield_if_needed()`.
*   **Never Enters:** If the initial `condition` evaluation is false, the loop block is completely bypassed.
*   **Jumping into Blocks:** Branching directly into a `WHILE...WEND` block bypassing the `WHILE` statement results in an `ERR_WEND_WITHOUT_WHILE` upon hitting the `WEND`.
...
The `WHILE` instruction is identified in the Lexer by the `KW_WHILE` token.

**Parser Expectations:**
The parser requires a single conditional expression. It constructs an AST node (`AST_WHILE`) and defers linkage until it encounters the matching `WEND` in the same block scope.

**Struct Mutations:**
*   **`vm->loop_stack`:** Pushes a new loop frame (of type `LOOP_WHILE`) containing the AST node pointer to quickly return upon encountering `WEND`.

**Execution Paths & Error Handling:**
1. Evaluate the `condition` expression.
2. If false (0), jump the PC directly to the instruction after the paired `WEND`.
3. If true (non-zero), push loop context to `vm->loop_stack` and continue execution.
4. Stack overflows result in `ERR_LOOP_STACK_OVERFLOW`.

**What to do if it breaks:**
If the loop fails to terminate, verify that the condition expressions are evaluated correctly in `vm_loops.c` and that the variables involved are actually mutated. Check block nesting depth if `ERR_WEND_WITHOUT_WHILE` triggers unexpectedly.
...
`WHILE...WEND` was popularized by GW-BASIC and QBASIC as an essential structured programming addition to classic BASIC. BASIC++ implements it precisely to those specifications, alongside the more robust `DO...LOOP` constructs.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe` (Windows) or `baspp-console` (Linux).
2. Type:
   ```basic
   10 X=1: WHILE X<3: PRINT X: X=X+1: WEND
   ```
