...
### 1. Syntax & Parameters
`ON KEY(<n>) GOSUB <line_number | label>`
`KEY(<n>) ON | OFF | STOP`

*   **`n`**: An integer expression (1 to 20) representing the specific key to trap. Typically 1-10 are function keys (F1-F10), 11-14 are arrow keys.
*   **`line_number | label`**: The target subroutine to execute when the event fires.

### 2. Description & Usage
The `ON KEY(...) GOSUB` statement sets up an asynchronous event trap. When the specified key is pressed, the program automatically interrupts its sequential execution and branches to the specified subroutine (as if an implicit `GOSUB` was called). The event trapping is only active when enabled via `KEY(n) ON`.

**Usage:**
This feature allows programs to handle user input (like ESC or function keys) in the background without constantly polling `INKEY$` in a busy loop. 

**Edge Cases & Boundary Conditions:**
*   **Disabled Traps:** If `KEY(n) OFF` is active, the key press is ignored by the event system and goes to the standard keyboard buffer.
*   **Suspended Traps (`STOP`):** If `KEY(n) STOP` is active, the event is remembered but the subroutine is not called until `KEY(n) ON` is executed.
*   **Target Missing:** If the subroutine label is undefined, `ERR_UNDEFINED_LABEL` or `ERR_UNDEFINED_LINE` is thrown during parsing or runtime linking.
*   **Nested Traps:** Event subroutines can be interrupted by other events, increasing the call stack depth. Ensure subroutines end with `RETURN` to prevent stack overflows.

### 3. Code Examples

**Basic Key Trapping:**
```basic
10 ON KEY(1) GOSUB HandleF1
20 KEY(1) ON
30 PRINT "Press F1 to trigger event. Press CTRL+C to exit."
40 GOTO 40
50 HandleF1:
60 PRINT "F1 was pressed!"
70 RETURN
```
...
10 ON KEY(1) GOSUB HandleF1
20 KEY(1) STOP
30 PRINT "Pressing F1 now will queue the event but not trigger it."
40 SLEEP 5
50 KEY(1) ON : REM Now it will trigger if it was pressed.
60 END
70 HandleF1:
80 PRINT "Queued event fired!"
90 RETURN
```
...
The `ON KEY` instruction creates an `AST_ON_KEY` node, while `KEY(n) ON` creates an `AST_KEY_TRAP_STMT` node.

**Struct Mutations:**
*   **`vm->key_traps[n].target`**: Set to the AST node pointer of the target subroutine.
*   **`vm->key_traps[n].state`**: Set to `TRAP_ON`, `TRAP_OFF`, or `TRAP_STOP`.
*   **`vm->key_traps[n].flagged`**: A boolean flag set to `true` when the physical key is pressed during a `TRAP_STOP` or `TRAP_ON` state.

**Execution Paths & Error Handling:**
1. **Setup:** The `ON KEY` statement maps the `n` to the target.
2. **Polling Loop:** Inside `vm_yield_if_needed()`, the VM checks the `flagged` status of all active key traps.
3. **Execution:** If a trap is flagged and the state is `TRAP_ON`, the VM clears the flag, pushes the current `vm->pc` to `vm->call_stack`, and sets `vm->pc` to the trap target.
4. If a trap triggers but the stack is full, `ERR_OUT_OF_MEMORY` is thrown.

**What to do if it breaks:**
If key events are firing twice or locking up the VM, ensure that `vm_yield_if_needed()` properly clears the `flagged` boolean *before* invoking the subroutine branch. Otherwise, an infinite loop of trap triggering will occur. Check `vm_events.c` to ensure key codes map accurately to the 1-20 trap indexes.
...
Event trapping was a major selling point of Advanced BASIC (BASICA) and GW-BASIC over simpler implementations. It provided a crude form of multitasking. BASIC++ implements this using cooperative yielding inside the main execution loop, maintaining identical semantics to the MS-DOS era but backed by a non-blocking asynchronous event queue.
...
2. Type:
   ...
   ```
3. Type `RUN`.
4. Press `F1`. You should see `HIT!` printed asynchronously while the infinite loop at line 30 continues.
