...
### 1. Syntax & Parameters
`ON TIMER(<n>) GOSUB <line_number | label>`

*   **`n`**: An integer expression representing the timer interval in seconds (from 1 to 86400).
*   **`line_number | label`**: The target subroutine to execute when the timer fires.

### 2. Description & Usage
The `ON TIMER(...) GOSUB` statement initializes a periodic timer event. When enabled via the `TIMER ON` statement, the virtual machine will automatically branch to the specified subroutine every `n` seconds.

**Usage:**
Useful for creating background tasks like updating a clock on the screen, periodically saving data, or driving game loops at a fixed tick rate.

**Edge Cases & Boundary Conditions:**
*   **Range Limit:** `n` must be strictly between 1 and 86400 (one day). Values outside this range throw `ERR_ILLEGAL_FUNCTION_CALL`.
*   **Timer Overlap:** If the subroutine takes longer than `n` seconds to execute, the next timer event will queue up but will not interrupt the current execution of the trap itself. Once it returns, it may immediately fire again.
*   **Disabled Trap:** If `TIMER OFF` is issued, the timer stops tracking elapsed time.
*   **Suspended Trap:** If `TIMER STOP` is issued, the timer continues counting, but the subroutine won't fire until `TIMER ON` is executed.

### 3. Code Examples

**Periodic Clock Update:**
```basic
10 ON TIMER(1) GOSUB UpdateClock
20 TIMER ON
...
Parsed as `AST_ON_TIMER` node.

**Struct Mutations:**
*   **`vm->timer_trap.target`**: Subroutine AST node pointer.
*   **`vm->timer_trap.interval_ms`**: `n` converted to milliseconds (`n * 1000`).
*   **`vm->timer_trap.last_tick_ms`**: An internal uint64_t timestamp storing the absolute system time of the last triggering.

**Execution Paths & Error Handling:**
1. During `ON TIMER(n)`, the VM calculates `n * 1000` and stores it.
2. The current monotonic system time is saved into `last_tick_ms`.
3. In `vm_yield_if_needed()`, the VM gets the current monotonic time.
4. If `(current_time - last_tick_ms) >= interval_ms` and state is `TRAP_ON`, it triggers the implicit `GOSUB`.
5. It then updates `last_tick_ms = current_time` to reset the interval.
6. Validation checks ensure `n` > 0 and `n` <= 86400.

**What to do if it breaks:**
If the timer drifts or fires inconsistently, check the underlying OS time API used by `vm_yield_if_needed()`. Ensure that `last_tick_ms` is being updated relative to the monotonic clock, not the wall-clock time, to prevent bugs related to daylight savings or NTP adjustments.
...
In GW-BASIC, `ON TIMER` was limited to 1-86400 seconds. BASIC++ strictly mirrors this limitation. It operates using standard cooperative polling rather than true hardware interrupts, ensuring safe execution within the single-threaded virtual machine constraints.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe` (Windows) or `baspp-console` (Linux).
2. Type:
   ```
   10 ON TIMER(2) GOSUB 100
   20 TIMER ON
   30 GOTO 30
   100 PRINT "Tick!"
   110 RETURN
   ```
3. Type `RUN`.
4. Observe the word `Tick!` printing exactly every 2 seconds.
