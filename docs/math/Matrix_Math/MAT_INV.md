...
## 1. Syntax & Parameters
`MAT <arrayA> = INV(<arrayB>)`

* `<arrayA>`: The target 2D array that will store the inverted matrix.
* `<arrayB>`: The source 2D array to be inverted. It must be a square matrix.

## 2. Description & Usage
The `MAT INV` statement computes the multiplicative inverse of a square matrix `<arrayB>` and stores the result in `<arrayA>`. If `<arrayB>` is multiplied by `<arrayA>`, the result is the identity matrix. `<arrayA>` must be dimensioned to the exact same size as `<arrayB>`.

The maximum matrix size supported for inversion is 15x15. If the matrix is singular (cannot be inverted, e.g., determinant is 0), an `ERR_HOW` is raised.
...
* Condition: `pi_mat_match_ident(lex, "INV")`

## 5. Implementation Details
* **Parser Expectations:** Matches `INV`, consumes a left parenthesis, array name, and right parenthesis.
* **Dimensionality Check:** Both `<arrayA>` and `<arrayB>` must be 2D square matrices of the exact same size. Maximum size is 15x15 (`b->size[0] > 15`). Exceeding this size raises `ERR_SORRY`.
* **Algorithm:** Uses Gauss-Jordan elimination with partial pivoting. Builds an augmented matrix `[B | I]` into a local `double work[16][32]` array.
* **Singularity Check:** If the pivot approaches 0 (`pivot > -1e-12 && pivot < 1e-12`), it treats the matrix as singular and raises `ERR_HOW`.
* **Troubleshooting:**
  * `ERR_HOW`: Verify arrays are 2D, square, the same size, and that the source matrix is invertible (not singular).
  * `ERR_SORRY`: The matrix is too large (greater than 15x15).
...
* **ECMA-116:** Standardized matrix inversion syntax.
* **GW-BASIC / QBASIC:** Unsupported natively.

## 8. Manual Testing Guide
To manually test `MAT INV`:
...
2. Enter:
   ...
   RUN
   ```
3. Verify the output displays the inverted matrix (e.g., -2, 1, 1.5, -0.5).
