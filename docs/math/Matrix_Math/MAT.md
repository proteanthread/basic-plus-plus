...
### 1. Syntax & Parameters
`MAT array = array | MAT array = expr`
- Parameters correspond to mathematical expressions evaluated as numeric types.

### 2. Description & Usage
Performs matrix operations. Evaluates the input and calculates the result mathematically.
...
### 3. Code Examples
```basic
10 MAT A = B ' Output: Matrix assignment
```
...
- **Lexer**: `source/parser/lexer.c` (`KW_MAT`)
- **AST Node**: `source/parser/ast.c` (`STMT_MAT`)
- **Runtime Evaluation**: `source/math/parser_math.c -> pi_parse_mat()`

### 5. Implementation Details
The runtime extracts the numeric arguments, enforces bounds checking, and utilizes the C `<math.h>` standard library to compute the operation. Precision is maintained as standard double precision internally. Type-safety guidelines enforce standard casting to prevent memory faults.
...
Standard mathematical functions derived from GW-BASIC, QBASIC, and IEEE-754.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type: `10 MAT A = B` and press ENTER.
3. Verify the output correctly evaluates to the expected value.
