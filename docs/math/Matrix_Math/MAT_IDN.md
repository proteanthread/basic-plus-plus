...
* `<array>`: The name of a pre-dimensioned 2D numeric array (matrix) that will be transformed into an identity matrix.

## 2. Description & Usage
The `MAT IDN` statement initializes a square matrix (a 2D array where the number of rows equals the number of columns) as an identity matrix. An identity matrix has `1`s on the main diagonal (where row index equals column index) and `0`s everywhere else. 

If the target array is not a 2D array or is not square, a runtime error (HOW) is raised.
...
* Condition: `pi_mat_match_ident(lex, "IDN")`

## 5. Implementation Details
* **Parser Expectations:** The parser expects the `MAT` keyword, followed by an array name, an equals sign `=`, and the `IDN` identifier.
* **Dimensionality Check:** The target array must be a 2D array (`arr->dims != 2` triggers an `ERR_HOW`).
* **Square Check:** The number of rows must equal the number of columns (`arr->size[0] != arr->size[1]` triggers an `ERR_HOW`).
* **Mutation:** Iterates through rows and columns. If `row == col`, assigns a float value of `1.0`; otherwise, `0.0`.
* **Troubleshooting:** If `ERR_HOW` is raised, check that the array was properly dimensioned as a 2D square matrix before calling `MAT IDN`.
...
* **ECMA-116:** Fully compliant with the standard BASIC specification for matrix initialization.
* **GW-BASIC / QBASIC:** GW-BASIC does not support matrix math natively, requiring manual loops. BASIC++ includes this as an extended feature.

## 8. Manual Testing Guide
To manually test `MAT IDN`:
...
2. Enter the following lines:
   ...
   RUN
   ```
3. Verify that the output shows `1`s on the diagonal and `0`s elsewhere.
