...
## 1. Syntax & Parameters
`result = TIM(component_id)`
- **Parameters**: component_id (0=min, 1=hr, 2=yday, 3=yr)
- **Return Value**: Float

## 2. Description & Usage
TIM(x) returns specific time components based on the argument. TIM(0)=minute (0-59), TIM(1)=hour (0-23), TIM(2)=day of year (1-366), TIM(3)=year (e.g. 2026). Default fallback returns the raw epoch time.
This keyword provides access to system time values, ensuring compatibility across supported platforms while delivering high precision where applicable.

## 3. Code Examples
```basic
10 PRINT "Current TIM: "; TIM(1)
20 SLEEP 1
30 PRINT "Done."
```
...
Implemented in builtins_math.c as builtin_tim. Uses localtime() internally. It is evaluated dynamically at runtime by the parser (`parser_expr.c`) or built-in function handler (`builtins.c`), mapping to the C standard library `time()` and `localtime()` functions, or the virtual device `vdev_get_time()`.
...
Originally popularized by Commodore BASIC (in the case of `TI$`) and HP BASIC (in the case of `TIM`), these extensions have been implemented in BASIC++ to maximize compatibility and enhance timekeeping capabilities beyond the standard `TIMER` functionality.

## 8. Manual Testing Guide
To manually test this keyword, open a command prompt and execute:
```bash
basicpp-console.exe -c "PRINT TIM(1)"
```
Verify that the output accurately reflects the current system time component.
