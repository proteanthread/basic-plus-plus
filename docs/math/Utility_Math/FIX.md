...
### 2. Description & Usage
Truncates a numeric value toward zero. It simply strips the fractional part of a number without rounding. For positive numbers, it is identical to `INT`. For negative numbers, it behaves differently (e.g., -3.7 becomes -3, whereas `INT` makes it -4).
...
- **Lexer**: `source/parser/lexer.c` (`KW_FIX`)
- **AST Node**: `source/parser/ast.c` (`FUNC_FIX`)
- **Runtime Evaluation**: `source/math/builtins_math.c` -> `builtin_fix()`

### 5. Implementation Details
The `builtin_fix()` function utilizes the standard C `<math.h>` `trunc()` function. It evaluates the provided argument and returns a new numeric value stripped of its fractional component. No type conversion to actual C integers happens here; it remains a double-precision float without a fraction.
...
Included in GW-BASIC and QBASIC for scenarios where simple truncation (rather than floor rounding) was required.

### 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Enter: `PRINT FIX(-9.9)`
3. Verify output is `-9`.
