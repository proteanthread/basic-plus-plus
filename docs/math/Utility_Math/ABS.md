...
### 2. Description & Usage
Returns the absolute (positive) value of the specified mathematical expression. If the value is negative, it is converted to positive. If it is already positive or zero, it remains unchanged.
...
### 3. Code Examples
```basic
10 PRINT ABS(-5.5) ' Prints 5.5
20 PRINT ABS(10)   ' Prints 10
```
...
- **Lexer**: `source/parser/lexer.c` (`KW_ABS`)
- **AST Node**: `source/parser/ast.c` (`FUNC_ABS`)
- **Runtime Evaluation**: `source/math/builtins_math.c` -> `builtin_abs()`

### 5. Implementation Details
The `builtin_abs()` function evaluates the child AST node and extracts the numeric value. It then wraps the standard C `<math.h>` function `fabs()` to compute the absolute value. Since BASIC++ uses double-precision floats for most arithmetic, `fabs()` ensures precision is not lost. The result is returned as a new Value node.
...
Standard in all BASIC dialects, including GW-BASIC and QBASIC. Behaves identically.

### 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Enter: `PRINT ABS(-99.9)`
3. Verify output is `99.9`.
