...
## 2. Description & Usage
Rounds a numeric expression to a specified number of decimal places. Positive places round to decimals, zero to integer, negative to powers of ten.
This function operates safely on scalar numbers and handles standard type coercions implicitly.

## 3. Code Examples
```basic
10 PRINT ROUND(3.14159, 2)
20 PRINT ROUND(1234, -2)
```

## 4. Internal C-Source Mapping
Implemented in `builtins_math.c` via the `builtin_round()` function.
This function integrates into the `FCAT_MATH` built-in registry.

## 5. Implementation Details
Calculates `factor = 10^places` with a limit of 15 powers. Uses `floor(x * factor + 0.5) / factor` to achieve standard half-up rounding.
...
ECMA-116 standard. GWBASIC typically relied on manual rounding techniques like `INT(X * 100 + .5) / 100`.

## 8. Manual Testing Guide
Run `basicpp-console.exe`.
Type `PRINT ROUND(2.718, 2)` and verify it outputs `2.72`.
