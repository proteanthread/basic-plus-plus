...
**Parameters:**
- n : Any valid numeric expression.

## 2. Description & Usage
Returns the arctangent of a numeric expression.
Returns the principal value of the arctangent of n. Result is in radians by default, or degrees if OPTION ANGLE DEGREES is active.
Edge Cases:
...
File: `source/math/builtins_math.c`, Line: 131, Function: `builtin_atn`

## 5. Implementation Details
Delegates to `atan()` from the C math library. If `state->angle_degrees` is true, the result is multiplied by `RAD_TO_DEG`.
...
- TAN, SIN, COS, ACOS, ASIN

## 7. Historical Context
Present in GW-BASIC, QBASIC, and fully compliant with ECMA-116 standard.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`
2. Type `PRINT ATN(0)` and press Enter.
