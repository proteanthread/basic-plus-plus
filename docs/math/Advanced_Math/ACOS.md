...
**Parameters:**
- n : Numeric expression in the range -1.0 to 1.0.

## 2. Description & Usage
Returns the arccosine (inverse cosine) of a numeric expression.
Used in trigonometry to find an angle given its cosine. The result is returned in radians unless OPTION ANGLE DEGREES is active.
Edge Cases:
...
File: `source/math/builtins_math.c`, Line: 474, Function: `builtin_acos`

## 5. Implementation Details
Delegates to the standard C math library `acos()` function. Expects arguments in the domain [-1, 1]. Values outside this domain may return NaN or trigger a runtime error.
...
Not native to GW-BASIC. Common in extended dialects and fully compliant with advanced math standards.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`
2. Type `PRINT ACOS(1)` and press Enter.
3. Verify the output is `0`.
