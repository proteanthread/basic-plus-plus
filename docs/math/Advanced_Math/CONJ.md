...
For a complex number z = a + bi, returns a - bi (negates the imaginary part). For real numbers, returns the number itself.
Edge Cases:
...
File: `source/math/builtins_math.c`, Line: 210, Function: `builtin_conj`

## 5. Implementation Details
Checks if the value is complex. If so, returns a new complex BValue with negated imaginary part. Otherwise, returns the original value.
...
2. Type `Z = COMPLEX(1, 2)` and press Enter.
3. Type `PRINT IMAG(CONJ(Z))` and press Enter. Verify output is `-2`.
