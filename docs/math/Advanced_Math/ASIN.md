...
## 2. Description & Usage
Returns the arcsine (inverse sine) of a numeric expression.
Used in trigonometry to find an angle given its sine. The result is returned in radians unless OPTION ANGLE DEGREES is active.
Edge Cases:
...
File: `source/math/builtins_math.c`, Line: 465, Function: `builtin_asin`

## 5. Implementation Details
Delegates to the standard C math library `asin()` function. Expects arguments in the domain [-1, 1]. Values outside this domain may return NaN or trigger a runtime error.
...
Historically found in SUPER BASIC (Tymshare) and extended math dialects.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`
2. Type `PRINT ASIN(0)` and press Enter.
