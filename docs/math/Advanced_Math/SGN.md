...
## 2. Description & Usage
Returns the mathematical sign of the expression: -1 for negative, 0 for zero, and 1 for positive.
...
Implemented in `builtins_math.c` via the `builtin_sgn()` function.
This function integrates into the `FCAT_MATH` built-in registry.

## 5. Implementation Details
Delegates to `bval_sgn()` to determine if the internal structure represents a positive, negative, or zero value.
...
A staple in all major dialects, including GWBASIC, QBASIC, and ANSI BASIC.

## 8. Manual Testing Guide
Run `basicpp-console.exe`.
Type `PRINT SGN(-42)` and verify it outputs `-1`.
