...
**Parameters:**
- `z, w`: Numeric expression(s).

## 2. Description & Usage
Raises a complex number to a complex power (z^w).
Computes z^w using the formula e^(w * ln(z)). Works with real numbers but treats them as complex numbers with 0 imaginary part.

## 3. Code Examples
```basic
PRINT CPOW(COMPLEX(0,1), COMPLEX(0,1))
```

## 4. Internal C-Source Mapping
Implemented in: `builtins_math.c, builtin_cpow`.

## 5. Implementation Details
Extracts real and imaginary parts of z and w. Computes ln(z), multiplies by w, and exponentiates using exp() and trigonometric functions.
Always returns a numeric (float or integer or complex). Ensures type-safety through the BValue API.
If arguments are of incorrect type, standard BASIC++ type conversions or errors apply.

## 6. Cross-References / See Also
COMPLEX, EXP, LOG

## 7. Historical Context
Modern extension for complex number math.

## 8. Manual Testing Guide
1. Open terminal and run `basicpp-console.exe`
2. Type `PRINT CPOW(COMPLEX(0,1), COMPLEX(0,1))` to verify output matches expected mathematical result.
3. Use invalid inputs (e.g. strings) to test type checking: `PRINT CPOW("test")`.
