...
## 1. Syntax & Parameters
`MED(expression1, expression2, [...])`
- **expression**: A numeric value or variable.

## 2. Description & Usage
Returns the median (middle value) of a series of numeric expressions. If an even number of arguments is provided, it returns the arithmetic average of the two middle values.
This function operates safely on scalar numbers and handles standard type coercions implicitly.

## 3. Code Examples
```basic
10 PRINT MED(5, 2, 9)
20 PRINT MED(1, 10, 3, 8)
```

## 4. Internal C-Source Mapping
Implemented in `builtins_math.c` via the `builtin_med()` function.
This function integrates into the `FCAT_MATH` built-in registry.

## 5. Implementation Details
Uses a C89-compatible insertion sort algorithm on a local double array. Max 64 values are allowed due to a fixed buffer size (`MED_MAX_VALS 64`). If more are passed, the parser/evaluator caps it or it will trigger a safety boundary. The result is calculated as `vals[n / 2]` for odd counts, or `(vals[n / 2 - 1] + vals[n / 2]) / 2.0` for even counts.
...
A non-standard statistical extension found in some advanced dialects. Not present in typical GWBASIC/QBASIC, where users would have to write their own sorting subroutines.

## 8. Manual Testing Guide
Run `basicpp-console.exe`.
Type `PRINT MED(1, 5, 3)` and verify it outputs `3`.
Type `PRINT MED(10, 20, 30, 40)` and verify it outputs `25`.
