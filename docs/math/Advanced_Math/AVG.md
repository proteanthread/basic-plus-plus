...
**Parameters:**
- n1, n2, ... : A comma-separated list of numeric expressions.

## 2. Description & Usage
Returns the arithmetic average (mean) of a list of numeric expressions.
Calculates the sum of all arguments and divides by the total number of arguments. Useful for statistical calculations.
Edge Cases:
...
## 3. Code Examples
```basic
10 PRINT AVG(10, 20, 30)
20 X = AVG(5, 15)
30 PRINT X
```

## 4. Internal C-Source Mapping
File: `source/math/builtins_math.c`, Line: 390, Function: `builtin_avg`

## 5. Implementation Details
Iterates over the arguments, sums their float values using `bval_to_float()`, and divides by `argc`. Returns 0 if `argc` is 0.
...
Not native to GW-BASIC. An extension provided for convenience in statistical operations.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`
2. Type `PRINT AVG(2, 4, 6)` and press Enter.
3. Verify the output is `4`.
