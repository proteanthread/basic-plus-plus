...
Calculates e^z where z = a + bi. The result is e^a * (cos(b) + i*sin(b)).
Edge Cases:
...
File: `source/math/builtins_math.c`, Line: 259, Function: `builtin_cexp`

## 5. Implementation Details
Extracts real (a) and imaginary (b) components. Uses Euler's formula to compute the result and returns a new complex BValue.
...
2. Type `PRINT CEXP(COMPLEX(0, 0))` and press Enter.
3. Verify the output shows the real part as 1.
