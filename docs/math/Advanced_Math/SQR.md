...
## 2. Description & Usage
Returns the mathematical square root of a positive expression. If a negative value is passed, it generally produces an error unless in a complex context.
...
Implemented in `builtins_math.c` via the `builtin_sqr()` function.
This function integrates into the `FCAT_MATH` built-in registry.

## 5. Implementation Details
Delegates to `bval_sqr()`.
...
Standard in all BASIC dialects.

## 8. Manual Testing Guide
Run `basicpp-console.exe`.
Type `PRINT SQR(16)` and verify it outputs `4`.
