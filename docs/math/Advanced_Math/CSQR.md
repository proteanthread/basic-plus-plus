...
**Parameters:**
- `z`: Numeric expression(s).

## 2. Description & Usage
Returns the square root of a complex number.
Computes the principal square root of z = a + bi.

## 3. Code Examples
```basic
PRINT CSQR(COMPLEX(-1, 0))
```

## 4. Internal C-Source Mapping
Implemented in: `builtins_math.c, builtin_csqr`.

## 5. Implementation Details
Calculates magnitude r and angle t = atan2(b, a). Returns sqrt(r)*(cos(t/2) + i*sin(t/2)).
...
Complex math extension.

## 8. Manual Testing Guide
1. Open terminal and run `basicpp-console.exe`
2. Type `PRINT CSQR(COMPLEX(-1, 0))` to verify output matches expected mathematical result.
3. Use invalid inputs (e.g. strings) to test type checking: `PRINT CSQR("test")`.
