...
## 2. Description & Usage
Returns the complex argument (phase angle) of a complex number in radians.
Returns the angle between the positive real axis and the line joining the origin to the complex point in the complex plane.
Edge Cases:
...
File: `source/math/builtins_math.c`, Line: 296, Function: `builtin_carg`

## 5. Implementation Details
Extracts real (a) and imaginary (b) components. Returns `atan2(b, a)`. For real numbers, b=0.
...
2. Type `PRINT CARG(COMPLEX(1, 1))` and press Enter.
3. Verify the output is approximately `0.785398` (pi/4).
