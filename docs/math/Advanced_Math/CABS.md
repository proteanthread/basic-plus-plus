...
## 2. Description & Usage
Returns the complex absolute value (magnitude) of a complex number.
Calculates the magnitude of a complex number z = a + bi as sqrt(a^2 + b^2). For real numbers, behaves exactly like ABS.
Edge Cases:
...
File: `source/math/builtins_math.c`, Line: 223, Function: `builtin_cabs`

## 5. Implementation Details
Checks if the value is complex. If so, computes `sqrt(real^2 + imag^2)`. Otherwise, delegates to `fabs()`.
...
2. Type `PRINT CABS(COMPLEX(3, 4))` and press Enter.
