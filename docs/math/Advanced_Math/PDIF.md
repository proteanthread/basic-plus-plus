...
## 1. Syntax & Parameters
`PDIF(expression1, expression2)`
- **expression**: A numeric value or variable.

## 2. Description & Usage
Returns the positive difference between two numeric expressions. If the first argument is greater than the second, it returns `arg1 - arg2`. Otherwise, it returns `0.0`.
...
Implemented in `builtins_math.c` via the `builtin_pdif()` function.
This function integrates into the `FCAT_MATH` built-in registry.

## 5. Implementation Details
Calculates `diff = bval_to_float(&args[0]) - bval_to_float(&args[1])`. Returns `diff > 0.0 ? diff : 0.0`.
...
A mathematical extension present in some scientific BASIC environments. It removes the need to write `IF A > B THEN RES = A - B ELSE RES = 0`.

## 8. Manual Testing Guide
Run `basicpp-console.exe`.
Type `PRINT PDIF(10, 3)` and verify it outputs `7`.
Type `PRINT PDIF(3, 10)` and verify it outputs `0`.
