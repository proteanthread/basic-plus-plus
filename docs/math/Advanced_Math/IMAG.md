...
If the input is not a complex number, it returns 0.

## 3. Code Examples
```basic
PRINT IMAG(COMPLEX(3, 4))
```

## 4. Internal C-Source Mapping
Implemented in: `builtins_math.c, builtin_imag`.

## 5. Implementation Details
Checks if bval_is_complex. If so returns v.cval.imag, else 0.
...
2. Type `PRINT IMAG(COMPLEX(3, 4))` to verify output matches expected mathematical result.
3. Use invalid inputs (e.g. strings) to test type checking: `PRINT IMAG("test")`.
