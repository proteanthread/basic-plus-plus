...
**Parameters:**
- `x`: Numeric expression(s).

## 2. Description & Usage
Returns the base-2 logarithm of a number.
Computes log base 2 using log(x) / ln(2).
...
Implemented in: `builtins_math.c, builtin_log2`.

## 5. Implementation Details
Computed as log(x) / 0.69314718055994530942 for C89 compatibility.
...
Useful for bitwise math and modern extensions.

## 8. Manual Testing Guide
1. Open terminal and run `basicpp-console.exe`
2. Type `PRINT LOG2(8)` to verify output matches expected mathematical result.
3. Use invalid inputs (e.g. strings) to test type checking: `PRINT LOG2("test")`.
