...
## 2. Description & Usage
Returns the smallest numeric value among a list of given numeric arguments.
This function operates safely on scalar numbers and handles standard type coercions implicitly.

## 3. Code Examples
```basic
10 PRINT MIN(50, 10, 100, -5)
20 X = MIN(A, B, C)
```

## 4. Internal C-Source Mapping
Implemented in `builtins_math.c` via the `builtin_min()` function.
This function integrates into the `FCAT_MATH` built-in registry.

## 5. Implementation Details
Iterates sequentially over all arguments from index 0 to `argc-1`, comparing each converted double value against the running minimum. If `argc` is 0, behavior may be undefined or handled by the parser limiting minimum arguments.
...
Included in standard ECMA-116 and modern BASIC implementations. GWBASIC lacked a built-in MIN function, requiring developers to write `IF A < B THEN MIN = A ELSE MIN = B`.

## 8. Manual Testing Guide
Run `basicpp-console.exe`.
Type `PRINT MIN(15, 7, 99)` and verify it outputs `7`.
