...
Implemented in `builtins_math.c` via the `builtin_pi()` function.
...
Standard constant in many modern dialects. In classic GWBASIC, users often manually defined `PI = 3.14159`.

## 8. Manual Testing Guide
Run `basicpp-console.exe`.
Type `PRINT PI` and verify it outputs roughly `3.14159`.
