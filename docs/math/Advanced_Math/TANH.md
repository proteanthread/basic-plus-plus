...
Implemented in `builtins_math.c` via the `builtin_tanh()` function.
This function integrates into the `FCAT_MATH` built-in registry.

## 5. Implementation Details
Directly invokes the C standard library function `tanh()`.
...
Advanced mathematical extension, typically found in HP/True BASIC variants.

## 8. Manual Testing Guide
Run `basicpp-console.exe`.
Type `PRINT TANH(0)` and verify it outputs `0`.
