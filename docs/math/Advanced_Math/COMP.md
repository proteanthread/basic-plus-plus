...
**Parameters:**
- a, b : Numeric expressions to compare.

## 2. Description & Usage
Compares two numeric values and returns a status indicator.
Returns -1 if a < b, 0 if a = b, and +1 if a > b. Useful for custom sorting routines or conditional branching.
Edge Cases:
...
File: `source/math/builtins_math.c`, Line: 524, Function: `builtin_comp`

## 5. Implementation Details
Converts both arguments to float. Evaluates conditions `a < b` and `a > b` to return the appropriate status float.
...
Extension feature, not native to standard GW-BASIC.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`
2. Type `PRINT COMP(5, 10)` and press Enter.
3. Verify the output is `-1`.
