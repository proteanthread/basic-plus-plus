...
## 1. Syntax & Parameters
`REAL(complex_expression)`
- **expression**: A numeric value or variable.

## 2. Description & Usage
Extracts the real part of a complex number. If passed a standard non-complex numeric, it returns the numeric value itself.
...
Implemented in `builtins_math.c` via the `builtin_real()` function.
This function integrates into the `FCAT_MATH` built-in registry.

## 5. Implementation Details
Checks if the provided BValue is a complex type using `bval_is_complex`. If so, returns `args[0].v.cval.real`. Otherwise, casts the argument to float and returns it.
...
Crucial for dialects that support native complex numbers (like advanced HP BASICs or True BASIC).

## 8. Manual Testing Guide
Run `basicpp-console.exe`.
Type `PRINT REAL(COMPLEX(5, 7))` and verify it outputs `5`.
