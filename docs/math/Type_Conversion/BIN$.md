...
## 1. Syntax & Parameters
`BIN$(v)`
- **Parameters**: v: The numeric value to convert (rounded to integer).

## 2. Description & Usage
Converts a numeric value to its binary string representation.
...
Implemented by `builtin_bins` in `source/strings/builtins_string.c`. Registration occurs in `source/functions/builtins.c`.
