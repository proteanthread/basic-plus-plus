...
## 1. Syntax & Parameters
`NUM(s$)`
- **Parameters**: s$: The string of bytes to convert.

## 2. Description & Usage
Converts string bytes to a numeric value.
...
Implemented by `builtin_num` in `source/strings/builtins_string.c`. Registration occurs in `source/functions/builtins.c`.
