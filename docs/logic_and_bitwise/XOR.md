...
## 2. Description & Usage
The XOR operator evaluates to true if exactly one of the expressions is true, but not both. For numeric values, it performs a bitwise exclusive OR. Each bit of the result is 1 if the corresponding bits of the operands differ, and 0 if they are the same.
...
A = 1
B = 2
IF (A=1) XOR (B=1) THEN PRINT "Only one is true"
' Output: Only one is true
' A=1 is True (-1). B=1 is False (0). -1 XOR 0 is -1 (True).
`

## 4. Internal C-Source Mapping
Implemented in: parser_expr.c (Handling OP_XOR inside the expression parsing and evaluation loop).

## 5. Implementation Details
In parser_expr.c, XOR maps to OP_XOR. The operands are evaluated to integers via bval_to_int(), and the C bitwise XOR (^) is applied (left ^ right). Issues with XOR typically stem from precedence errors rather than evaluation logic.

## 6. Cross-References / See Also
AND, OR, NOT, EQV, IMP

## 7. Historical Context
Functions exactly as in GW-BASIC and QBASIC, acting as a bitwise exclusive OR.

## 8. Manual Testing Guide
To verify XOR manually:
1. Open terminal and launch asicpp-console.exe.
2. Enter PRINT 5 XOR 3 and verify the bitwise result.
