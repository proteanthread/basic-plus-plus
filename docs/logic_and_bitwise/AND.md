...
## 2. Description & Usage
The AND operator evaluates to true (non-zero) if and only if both expressions evaluate to true. When evaluating numerical values, it performs a bitwise AND operation on the integer representations of the operands. This means each bit of the result is 1 if both corresponding bits of the operands are 1, otherwise it is 0. If expressions are boolean or relational (e.g., A=1), they evaluate to true (usually -1) or false (0), making the bitwise AND function perfectly as a logical AND.
...
A = 1
B = 2
IF A=1 AND B=2 THEN PRINT "True"
' Output: True
' Since A=1 (-1) and B=2 (-1), -1 AND -1 results in -1 (True).
`

## 4. Internal C-Source Mapping
Implemented in: parser_expr.c (Handling OP_AND inside the expression parsing and evaluation loop).

## 5. Implementation Details
In parser_expr.c, the AND operator corresponds to OP_AND. During evaluation, operands are converted to integers via bval_to_int(). A standard C bitwise AND (&) is applied to the two integers (left & right). If this behavior breaks, verify that both operands are being successfully cast to integers before the & operation is applied.

## 6. Cross-References / See Also
OR, NOT, XOR, EQV, IMP

## 7. Historical Context
Maintains exact parity with GW-BASIC and QBASIC, where logical operators are inherently bitwise on 16-bit or 32-bit signed integers.

## 8. Manual Testing Guide
To verify AND manually:
1. Open terminal and launch asicpp-console.exe.
2. Enter PRINT 5 AND 3 and verify the bitwise result.
