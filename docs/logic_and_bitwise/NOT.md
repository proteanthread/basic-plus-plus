...
## 2. Description & Usage
The NOT operator performs logical negation and bitwise complement. It takes a single operand. For numerical values, it inverts all bits (bitwise NOT). For boolean/relational comparisons that return -1 (True) or 0 (False), applying NOT to 0 results in -1, and applying NOT to -1 results in 0 (in a 2's complement system, NOT 0 is -1, and NOT -1 is 0), so it perfectly doubles as a logical NOT.
...
PRINT NOT 0
' Output: -1
' Explanation: All bits 0 become all bits 1, which represents -1 in two's complement integer.
`

**Logical Example:**
`asic
A = 5
IF NOT (A = 3) THEN PRINT "A is not 3"
' Output: A is not 3
' A=3 is False (0). NOT 0 is -1 (True).
`

## 4. Internal C-Source Mapping
Implemented in: parser_expr.c (Handling OP_NOT inside the expression parsing and evaluation loop).

## 5. Implementation Details
In parser_expr.c, NOT maps to OP_NOT and is evaluated as a unary operator. The operand is cast to an integer using bval_to_int(), and the C bitwise NOT (~) is applied (~val). If issues arise, verify the unary parsing path properly assigns higher precedence to NOT over binary logical operators.

## 6. Cross-References / See Also
AND, OR, XOR, EQV, IMP

## 7. Historical Context
Consistent with GW-BASIC and QBASIC, returning the two's complement bitwise inversion.

## 8. Manual Testing Guide
To verify NOT manually:
1. Open terminal and launch asicpp-console.exe.
2. Enter PRINT NOT 0 and verify the bitwise result.
