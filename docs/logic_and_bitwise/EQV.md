...
## 2. Description & Usage
The EQV operator is the logical opposite of XOR. It evaluates to true if both expressions are true or both are false. For numeric values, it performs a bitwise equivalence (XNOR). Each bit of the result is 1 if the corresponding bits of the operands are the same, and 0 if they differ.
...
' EQV result:  ...1111 1001 (which is -7 in two's complement 32-bit/64-bit depending on type)
`
...
A = 1
B = 1
IF (A=1) EQV (B=1) THEN PRINT "Both match"
' Output: Both match
' A=1 is True (-1). B=1 is True (-1). -1 EQV -1 is -1 (True).
`

## 4. Internal C-Source Mapping
Implemented in: parser_expr.c (Handling OP_EQV inside the expression parsing and evaluation loop).

## 5. Implementation Details
In parser_expr.c, EQV corresponds to OP_EQV. The operands are converted to integers via bval_to_int(). The evaluation is effectively the bitwise complement of XOR: ~(left ^ right). If it breaks, check for signed/unsigned mismatch in the complement operation.

## 6. Cross-References / See Also
XOR, AND, OR, NOT, IMP

## 7. Historical Context
Included in GW-BASIC and QBASIC as a bitwise XNOR operation. Fully implemented here for strict standard compatibility.

## 8. Manual Testing Guide
To verify EQV manually:
1. Open terminal and launch asicpp-console.exe.
2. Enter PRINT 5 EQV 3 and verify the bitwise result.
