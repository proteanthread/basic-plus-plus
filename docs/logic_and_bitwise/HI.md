...
## 1. Syntax & Parameters
`HI(<integer_expression>)`

* `<integer_expression>`: A numeric expression that evaluates to an integer (typically a 16-bit integer).

## 2. Description & Usage
The `HI` function returns the high-order (most significant) byte of a 16-bit integer expression. It isolates the upper 8 bits (bits 8-15) of the given integer and returns them as a new integer value. This is typically used in low-level bitwise operations, memory manipulations, or when interacting with hardware and binary file structures.

## 3. Code Examples
```basic
10 N = &H1234
20 PRINT HI(N)
' Output: 18 (which is &H12)
```
...
* **Parser/Evaluator:** Recognized as a pure math function (`FSAFE_PURE`, `FCAT_MATH`). Accepts exactly one argument and returns an integer (`FRET_INT`).
* **Bitwise Logic:** Extracts the value by casting the argument to a `long`, bit-shifting right by 8 bits (`val >> 8`), and masking with `0xFF` (`& 0xFF`).
* **Data Types:** Converts the internal `BValue` to an integer via `bval_to_int()`. Floating-point values passed will be truncated before processing.
* **Troubleshooting:** Ensure that the input expression is within the bounds of a standard 16-bit or 32-bit integer. For values larger than 16 bits, it only returns bits 8-15.
...
* **GW-BASIC / QBASIC:** Not a standard GW-BASIC or QBASIC function. Usually achieved using `INT(X / 256) AND 255`. BASIC++ provides this as a convenient built-in for systems programming.
...
2. Enter:
   ...
   PRINT HI(&HABCD)
   ```
3. The expected output is `171` (which is the decimal equivalent of `&HAB`).
