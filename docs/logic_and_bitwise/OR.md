...
## 2. Description & Usage
The OR operator evaluates to true if at least one of the expressions evaluates to true. For numeric values, it performs a bitwise inclusive OR operation. This means each bit of the result is 1 if at least one of the corresponding bits of the operands is 1. If expressions are boolean/relational, they evaluate to -1 or 0, so bitwise OR effectively functions as a logical OR.
...
A = 1
B = 5
IF A=1 OR B=2 THEN PRINT "True"
' Output: True
' Since A=1 (-1) and B=2 (0), -1 OR 0 results in -1 (True).
`

## 4. Internal C-Source Mapping
Implemented in: parser_expr.c (Handling OP_OR inside the expression parsing and evaluation loop).

## 5. Implementation Details
In parser_expr.c, the OR operator corresponds to OP_OR. Operands are cast to integers with bval_to_int(), and the C bitwise OR (|) is applied (left | right). To fix regressions, ensure precedence is correct relative to AND and arithmetic operators.

## 6. Cross-References / See Also
AND, NOT, XOR, EQV, IMP

## 7. Historical Context
Behaves identically to classic BASIC dialects (GW-BASIC/QBASIC), applying bitwise inclusive OR.

## 8. Manual Testing Guide
To verify OR manually:
1. Open terminal and launch asicpp-console.exe.
2. Enter PRINT 5 OR 3 and verify the bitwise result.
