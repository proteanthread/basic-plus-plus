Remove-Item -Path 'c:\Users\rtdos\GitHub\basic-plus-plus\docs\Creating_Dialects.md' -ErrorAction SilentlyContinue
Remove-Item -Path 'c:\Users\rtdos\GitHub\basic-plus-plus\docs\Dialect_Architecture.md' -ErrorAction SilentlyContinue
Remove-Item -Path 'c:\Users\rtdos\GitHub\basic-plus-plus\docs\Dialect_Audit_Overview.md' -ErrorAction SilentlyContinue
Remove-Item -Path 'c:\Users\rtdos\GitHub\basic-plus-plus\docs\Mixing_Dialects.md' -ErrorAction SilentlyContinue
Remove-Item -Path 'c:\Users\rtdos\GitHub\basic-plus-plus\docs\Specific_Machine_Dialects.md' -ErrorAction SilentlyContinue

$files = Get-ChildItem -Path 'c:\Users\rtdos\GitHub\basic-plus-plus\docs' -Recurse -Filter '*.md'
foreach ($f in $files) {
    $content = Get-Content $f.FullName -Raw
    $original = $content
    # Delete entire lines that are DIALECT command entries
    $content = $content -replace '(?m)^.*\*\*DIALECT\*\*.*$\r?\n?', ''
    $content = $content -replace '(?m)^.*\*\*DIALECT\$\*\*.*$\r?\n?', ''
    ...
    # Delete lines that are links to deleted dialect guides
    ...
    # Delete lines that contain 'DIALECT LOAD' as a command example
    $content = $content -replace '(?m)^.*`DIALECT LOAD.*$\r?\n?', ''
    $content = $content -replace '(?m)^.*DIALECT LOAD.*$\r?\n?', ''
    
    # Additional line removals based on new constraints
    $content = $content -replace '(?m)^.*BppDialect.*$\r?\n?', ''
    $content = $content -replace '(?m)^.*dialect\.c.*$\r?\n?', ''
