...
*   **`n`**: A numeric expression for the length of the string to generate. Bound between 0 and 255.
*   **`char`**: Either an integer representing the ASCII code (0-255) of the character to repeat, or a string whose first character will be used as the repeating pattern.

## 2. Description & Usage
The `STRING$` function is a highly versatile string generation utility that creates a new string composed of a single repeating character. 

If `char` is an integer, it uses the corresponding ASCII character. If `char` is a string variable or literal, it extracts only the very first character of that string. If the provided string is empty, `STRING$` will fail gracefully and return an empty string. The length `n` is strictly clamped between 0 and 255.

## 3. Code Examples

**Using an ASCII Code:**
```basic
10 REM Print 10 asterisks (ASCII 42)
20 PRINT STRING$(10, 42)
```
*Output:*
```text
**********
...
**Edge Case - Empty String Reference:**
```basic
10 REM Empty string behavior
20 A$ = ""
30 PRINT "START" + STRING$(5, A$) + "END"
```
*Output:*
```text
STARTEND
...
*   **Function:** `builtin_string_func(BValue *args, int argc, void *rt)`
*   **Category:** `FCAT_STRING`
*   **Safety:** `FSAFE_STATE` (Allocates from the string pool)

## 5. Implementation Details
`STRING$` maps to `builtin_string_func` in the C implementation.
1.  **Length Clamping:** Just like `SPACE$`, the `n` parameter is processed via `bval_to_int(&args[0])` and clamped: `if (n < 0) n = 0; if (n > 255) n = 255;`.
2.  **Type Introspection:** The function checks if the second argument (`args[1]`) is a string using `bval_is_string`. 
    *   If it is a string, it checks for a valid length (`> 0`) and a non-null data pointer. If valid, `ch = args[1].v.sval.data[0]`. If invalid (empty string), the function preemptively aborts and returns an empty string (`bval_string(NULL, 0)`).
    *   If the second argument is not a string, it falls back to parsing it as an integer: `ch = (char)bval_to_int(&args[1])`.
3.  **Pool Allocation & Mutation:** It calls `strpool_alloc(&state->strpool, n)`. If successful, it loops and sets `buf[i] = ch`, and returns the constructed `BValue`. If memory fails, it aborts safely.

## 6. Cross-References / See Also
*   `SPACE$`: Generates strings consisting exclusively of space characters.
*   `CHR$`: Returns a single character based on its ASCII code.
*   `ASC`: Retrieves the ASCII code of a string's first character.

## 7. Historical Context
`STRING$` matches the dual-overload behavior seen in Microsoft GW-BASIC and QBASIC, where the second parameter could interchangeably be a numeric ASCII value or a string. The 255-character hard limit is maintained for strict backward compatibility, preventing legacy scripts from allocating oversized arrays of characters that would have overflowed 8-bit memory constraints.

## 8. Manual Testing Guide
To manually test the dual-overload nature of `STRING$`:
1.  Start the interpreter: `basicpp-console.exe --log --debug`
2.  Test the numeric parameter branch: `PRINT STRING$(10, 65)` and expect `AAAAAAAAAA`.
3.  Test the string parameter branch: `PRINT STRING$(5, "B")` and expect `BBBBB`.
4.  Test the length clamping logic: `PRINT LEN(STRING$(500, "-"))` and ensure it returns `255`.
5.  Test the empty string edge condition: `PRINT LEN(STRING$(10, ""))` and ensure it returns `0` without causing a segmentation fault.
6.  Review the generated `.LOG` file for clean string pool interactions.
