...
### 1. Syntax & Parameters
`VAL(string_expr$)`
- **`string_expr$`**: A required string expression containing numerical characters to be parsed into a numeric equivalent.

### 2. Description & Usage
The `VAL` function evaluates a string representation of a number and converts it into its corresponding numeric value (`integer` or `float`). 

The conversion scans the string from left to right, parsing valid numeric characters (digits, decimal points, and sign indicators) and halts execution at the very first non-numeric character encountered (including spaces following a number). If the very first non-space character is not part of a valid numeric structure, `VAL` silently returns `0`.

**Common Use Cases:**
- Deserializing textual input or string data into mathematical variants.
- Stripping garbage characters from mixed alphanumeric prefixes.
- Converting user input (`INPUT`, `LINE INPUT`) logically into math boundaries.
...
```basic
100 DATA "100.5", "200", "ERROR", "300.99A"
110 TOTAL = 0
120 FOR I = 1 TO 4
130   READ S$
140   TOTAL = TOTAL + VAL(S$)
150 NEXT I
...
- **Lexer**: `source/parser/lexer.c` (`KW_VAL`)
- **AST Node**: `source/parser/ast.c` (`FUNC_VAL`)
- **Runtime Evaluation**: `source/strings/builtins_string.c -> builtin_val()`
- **Core Parsing Logic**: `source/core/value.c -> bval_val()`

### 5. Implementation Details
The execution flow begins when `VAL` is evaluated, dispatching control to `bval_val()` in the core subsystem.

**Architectural Execution & Safe C-RTL Bounding:**
1. **Validation & Isolation:** The engine verifies the operand is a valid string (`bval_is_string(v)`). If a numeric value is passed directly, `ERR_WHAT` (Type Mismatch) is raised.
2. **Buffer Preparation:** To interact cleanly with the standard C runtime without mutating the origin memory, the string is copied to an isolated stack buffer (`char buf[MAX_LINE_LENGTH + 1]`). Strings exceeding `MAX_LINE_LENGTH` are truncated automatically to prevent stack overflows. The buffer is safely NUL-terminated.
3. **Dual-Pass Parsing Strategy:** 
   - *Integer Pass:* The engine first invokes `strtol()` parsing in base-10. If the parser consumes the entire string (or stops cleanly on a space), an integer `BValue` (`VAL_INT`) is generated and returned.
   - *Float Pass:* If `strtol()` halts explicitly at a decimal point (`.`), the engine recognizes floating-point semantics and falls back to `strtod()` to parse the entire literal, yielding a float `BValue` (`VAL_DOUBLE`).
4. **Failure State:** If parsing fails utterly, the function resolves to integer `0`.
...
The implementation logic mirrors GW-BASIC exactly. In classic BASIC, `VAL("123ABC")` yields `123`, but `VAL("ABC123")` yields `0`. By utilizing C17's `strtol` and `strtod`, BASIC++ guarantees determinism mirroring this exact truncation behavior.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe` from the command prompt.
2. Execute float parsing: Type `10 PRINT VAL("3.14159TEXT")` and press ENTER.
3. Verify output matches ` 3.14159`.
4. Execute unparseable zeroing: Type `20 PRINT VAL("TEXT3.14")` and press ENTER.
5. Verify output matches ` 0`.
6. Verify type constraint: Type `30 PRINT VAL(100)` and verify it trips a Type Mismatch.
