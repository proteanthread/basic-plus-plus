...
### 1. Syntax & Parameters
`LTRIM$(target$)`
- `target$`: The original string expression to strip leading whitespace from.

### 2. Description & Usage
The `LTRIM$` function removes all leading space characters (` `) from the left side of the provided string and returns the cleaned string.
...
- **Runtime Evaluation**: `source/strings/builtins_string.c -> builtin_ltrims()`

### 5. Implementation Details
Iterates an index pointer `i` starting from 0, skipping spaces until it hits a non-space character or the end of the string.

### In-Depth Architectural Narrative:
- **Struct Mutations:** It calls `strpool_store(&state->strpool, s + i, len - i)` returning a `BValue` referencing the shifted pointer. This operation is pointer-driven and skips unnecessary copying loops.
...
- **Modification Constraints:** 
  ...
  - *What CANNOT be changed:* The pointer arithmetic (`s + i`) must be strictly bound by `len` to prevent out-of-bounds reads.
...
`LTRIM$` was part of the original Microsoft GW-BASIC and QuickBASIC standard library, historically utilized to format numbers implicitly cast to strings, as `STR$` frequently padded positive numbers with a leading space for a sign placeholder.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type: `PRINT "*" + LTRIM$("   test   ") + "*"` and press ENTER.
