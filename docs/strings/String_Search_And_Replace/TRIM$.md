...
### 1. Syntax & Parameters
`TRIM$(target$)`
- `target$`: The original string expression to strip whitespace from.

### 2. Description & Usage
The `TRIM$` function removes all leading and trailing space characters (` `) from the provided string and returns the cleaned string.

### Detailed Behavioral Edge Cases and Constraints:
- **Null or Empty Target String:** If the string is empty (`""`), it safely returns an empty string.
- **Strings with Only Spaces:** If the string is entirely comprised of spaces, the function strips all characters and returns an empty string (`""`).
- **Internal Spaces:** Spaces occurring between non-space characters are never removed.
- **Tab Characters:** Only ASCII space characters (`CHR$(32)`) are evaluated and trimmed by this function. Tab characters (`CHR$(9)`) are left intact.
- **Memory Allocation:** The result is placed directly into the string pool by offsetting pointers from the original string data, avoiding duplicate evaluations.

### 3. Code Examples
```basic
10 A$ = "   HELLO WORLD   "
20 PRINT ">" + TRIM$(A$) + "<" ' Output: >HELLO WORLD<
```

**Complex Integration Example:**
```basic
100 INPUT "Enter your name (don't worry about spaces): ", N$
110 N$ = TRIM$(N$)
120 PRINT "Welcome, " + N$ + "!"
```
...
- **Runtime Evaluation**: `source/strings/builtins_string.c -> builtin_trims()`

### 5. Implementation Details
The runtime extracts the string value. It maintains a `left` index (starting at 0) and a `right` index (starting at `len`). It loops `while (left < right && s[left] == ' ')` incrementing `left`, and `while (right > left && s[right-1] == ' ')` decrementing `right`.

### In-Depth Architectural Narrative:
- **Struct Mutations:** Instead of creating a temporary buffer and copying memory, `builtin_trims()` efficiently leverages the original string pointer. It calculates the substring boundary and allocates exactly `right - left` bytes in the string pool via `strpool_store(&state->strpool, s + left, right - left)`. This ensures highly efficient O(N) evaluation with zero heap defragmentation.
- **Error States & Fault Tolerance:** If an invalid type is passed, the runtime gracefully returns a null string.
- **Modification Constraints:** 
  - *What CAN be changed:* Additional whitespace checking macros (e.g., `isspace(s[left])`) could be implemented to include tabs and carriage returns.
  - *What CANNOT be changed:* The pointer offset arithmetic must not breach the bounds of the original array structure, or else out-of-bounds pointer dereferencing will crash the interpreter.
...
`TRIM$` was introduced in later BASIC dialects (like QBASIC) as a convenience wrapper combining `LTRIM$` and `RTRIM$`. Older dialects required `LTRIM$(RTRIM$(A$))`.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type: `PRINT "*" + TRIM$("   test   ") + "*"` and press ENTER.
