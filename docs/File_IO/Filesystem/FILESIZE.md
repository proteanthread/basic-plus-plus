...
## 2. Description & Usage
Returns the exact total size of the specified file in bytes. Unlike `LOF`, which requires a file to be currently opened via an active channel, `FILESIZE` queries the filesystem directly. It is designed for checking file constraints before allocating memory buffers.

## 3. Code Examples
```basic
10 F$ = "ARCHIVE.ZIP"
20 IF EXISTS(F$) THEN PRINT F$; " is "; FILESIZE(F$); " bytes."
```

## 4. Internal C-Source Mapping
- Function: `builtin_filesize` in `source/io/builtins_fileio.c`
- Registration: `builtins.c` under `KW_FILESIZE`

## 5. Implementation Details
Performs a `stat` query via the Virtual Filesystem layer.
**Edge Cases & Boundary Conditions:**
- Files larger than 2.1 GB (2^31 - 1 bytes) may trigger arithmetic overflow if the variable assigning the result is not a double-precision float (`#`) or 64-bit integer, depending on the architecture compile flags.
- Requesting the size of a directory or special device node (e.g., `/dev/null` on Linux) will return `0` or trigger `Bad file mode` depending on VFS policy.
- **Troubleshooting**: If it throws `TYPE MISMATCH`, ensure the file path is a valid string `$` literal.

## 6. Cross-References / See Also
- `LOF`, `EXISTS`, `FILEMOD$`

## 7. Historical Context
GW-BASIC rigidly required files to be opened with `OPEN` and then queried using `LOF`. `FILESIZE` is a modern addition allowing direct disk-level querying without burning precious file handle channels.

## 8. Manual Testing Guide
**Interactive Test:**
1. Launch `basicpp-console.exe`.
2. Type `PRINT FILESIZE("basicpp-console.exe")` and verify it returns a large non-zero integer.

**Scripted Test (.BAS):**
```basic
10 OPEN "O", 1, "size.tmp"
20 PRINT #1, "HELLO"
30 CLOSE 1
40 S = FILESIZE("size.tmp")
50 PRINT "Size is: "; S; " bytes."
60 KILL "size.tmp"
```
