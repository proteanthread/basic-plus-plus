...
## 2. Description & Usage
Returns the "Length Of File"—the total byte size of the currently opened file on the specified channel. It is used to allocate buffers for binary reads or calculate progress percentages during file processing.

## 3. Code Examples
```basic
10 OPEN "I", 1, "IMAGE.DAT"
20 L = LOF(1)
30 PRINT "The image is "; L; " bytes large."
40 CLOSE 1
```

## 4. Internal C-Source Mapping
- Function: `builtin_lof` in `source/io/builtins_fileio.c`
- Registration: `builtins.c` under `KW_LOF`

## 5. Implementation Details
Unlike `FILESIZE` which queries the disk, `LOF` retrieves the length directly from the active `FileChannel` object managed by the VFS.
**Edge Cases & Boundary Conditions:**
- If called on a `COM` (serial) or `TCP` (network) stream, `LOF` traditionally returns the number of bytes waiting in the receive buffer (acting identical to `LOC`).
- Calling `LOF` on an unopened channel produces Error 52 `Bad file number`.
- **Troubleshooting**: If you need the file size without opening it, use `FILESIZE` instead to avoid consuming a channel.

## 6. Cross-References / See Also
- `EOF`, `LOC`, `FILESIZE`, `OPEN`

## 7. Historical Context
A direct equivalent to the GW-BASIC `LOF` function. In older systems, files were allocated in 128-byte sectors, meaning `LOF` sometimes returned a multiple of 128 rather than the exact byte length. BASIC++ guarantees byte-exact measurements.

## 8. Manual Testing Guide
**Interactive Test:**
1. Launch `basicpp-console.exe`.
2. Type `OPEN "O", 1, "L.TMP": PRINT #1, "12345": CLOSE 1`
3. Type `OPEN "I", 1, "L.TMP": PRINT LOF(1): CLOSE 1`
4. Verify it prints `7` (5 chars + CRLF).
...
30 CLOSE 1
40 OPEN "I", 1, "lof.tmp"
50 IF LOF(1) > 0 THEN PRINT "FILE HAS DATA"
60 CLOSE 1
70 KILL "lof.tmp"
```
