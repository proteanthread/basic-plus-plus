...
### 2. Description & Usage
Examines the user's program currently in memory and performs a comprehensive multi-pass static analysis to identify potential errors, warnings, structural anomalies, and unreachable code without actually executing the program. 
**Usage:** It acts as an advanced built-in linting tool to ensure that code blocks are structurally sound. It checks variable usage, unreachable jumps, missing line numbers for `GOTO` and `GOSUB`, and mismatching `FOR/NEXT` or `WHILE/WEND` loops.
**Edge Cases:** If a `GOTO` target is dynamically computed (which is rare but possible in some dialects), `CHECK` may flag it as unreachable. Programs with complex nested loops that exit prematurely might trigger warnings about unbalanced stacks. Empty programs will pass checks silently.
**Behavior Constraints:** `CHECK` performs static analysis only; it does not execute code, evaluate expressions, or modify the program state or variables.
...
```
*Execution:* Typing `CHECK` will output a warning that line 40 does not exist and that line 30 is unreachable code.
...
30 WEND
```
*Execution:* `CHECK` will flag a mismatched loop structure because `WEND` is found instead of `NEXT I`.

### 4. Internal C-Source Mapping
- `source/debug.c` (`pi_parse_check`)
- `source/check.h` and `source/check.c` (`check_program`)

### 5. Implementation Details
The `CHECK` command performs multiple sequential passes over the `rt->program` Abstract Syntax Tree (AST): 
- **Pass 1:** Validates all literal `GOTO` and `GOSUB` targets against the internal line number index.
- **Pass 2:** Matches `FOR` statements with corresponding `NEXT` statements, ensuring variable names match if provided.
- **Pass 3:** Matches `WHILE` and `WEND` pairs.
- **Pass 4:** Analyzes variable initialization and usage, looking for uninitialized reads.
- **Pass 5:** Balances `DATA` and `READ` statements across the program.
- **Pass 6:** Validates `DEF FN` function definitions and calls.
- **Pass 7:** Evaluates overall reachability and calculates cyclomatic complexity for the program.
**Struct Mutations:** None. The operation is entirely read-only with respect to `BASIC_VM_STATE` and the AST. It only mutates local analysis state variables.
**Error States:** Does not halt the program or generate runtime errors; instead, it streams formatted warnings and errors to standard output.
**What can/cannot be changed:** The order of the passes is important for performance but not strictly required. The static analysis is isolated from the runtime evaluator to prevent accidental state modification.
...
Unlike standard GW-BASIC or QBASIC which relied strictly on runtime errors (e.g., encountering a missing label during execution), BASIC++ introduces `CHECK` as a pre-execution safety net. It is heavily inspired by modern IDE linters and brings true static analysis to the BASIC environment.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type `10 GOTO 100` and press ENTER.
3. Type `CHECK` and press ENTER.
4. Verify that the static analyzer reports a missing line target for line 100.
