...
### 2. Description & Usage
The `DUMP` command provides a low-level, comprehensive output of the BASIC++ virtual machine's internal state.
**Usage:** It is an advanced diagnostic and debugging tool that outputs the current status of the symbol table, memory allocations, string pool usage, the call stack, and internal VM registers. It is incredibly useful for compiler developers and advanced users diagnosing memory leaks or variable state corruption.
**Edge Cases:** Calling `DUMP` on a massive program with thousands of variables will generate an enormous amount of console output. It may cause the console buffer to overflow. 
**Behavior Constraints:** `DUMP` is a read-only command. It pauses REPL interaction while printing but does not change the state of the program.
...
30 DUMP
```
*Execution:* Prints the internal VM state, including the fact that `A` is a numeric variable holding 100, and `B$` is a string variable pointing to the string pool containing "TEST".

### 4. Internal C-Source Mapping
- `source/parser/debug.c` (Parsing `KW_DUMP`)
- `source/virtual/dump.c` (Implementation of `vm_dump_state`)

### 5. Implementation Details
The `DUMP` instruction is matched by the `KW_DUMP` token.
**Struct Mutations:** It performs a read-only traversal of the `rt` (Runtime State) struct, including `rt->sym_table` (variables), `rt->string_pool` (allocated strings), and `rt->call_stack` (active `GOSUB` returns).
**Error States:** If internal pointers are corrupted, `DUMP` could theoretically segfault, but it implements safe-read boundaries to gracefully report corrupted nodes instead of crashing.
**What can/cannot be changed:** The output format of `DUMP` is strictly plain text designed for debugging. Do not introduce pagination or interactive prompts into `DUMP`, as it is frequently redirected to log files.
**Architecture:** It iterates through the hash map of the symbol table and prints the raw byte representation and typing information of `EVAL_RESULT` nodes.
...
While standard BASIC dialects lacked a native `DUMP` command, forcing users to manually `PRINT` variables or use `DEF SEG` and `PEEK`, BASIC++ provides `DUMP` to align with modern virtual machine introspection tools.

### 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type `A = 42` and press ENTER.
3. Type `DUMP` and press ENTER.
4. Verify that the console outputs the internal VM state, and that the variable `A` is listed with a value of 42.
