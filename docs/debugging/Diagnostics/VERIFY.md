...
## 1. Syntax & Parameters
`VERIFY "path/to/file.BAS"`

- `"path/to/file.BAS"`: A required string literal or string variable representing the path to the BASIC source file to be analyzed.

## 2. Description & Usage
The `VERIFY` command performs a complete static analysis of a specified BASIC source file without loading it into the active program workspace. This allows the user to audit external files for syntax errors, structural integrity, and missing line numbers safely.
**Usage:** Ideal for Continuous Integration (CI/CD) pipelines or when you want to validate a program's correctness before running or merging it.
**Edge Cases & Boundary Conditions:** 
- If the file path is empty (`""`), it returns an `ERR_FILE_NOT_FOUND` or `ERR_BAD_FILE_NAME`.
- If the file exceeds `MAX_PROGRAM_LINES`, it returns an `ERR_OUT_OF_MEMORY`.
- Parsing complex string paths with escape characters or very long paths might trigger buffer overflow protections if not handled cleanly by the OS layer.
**Syntax Combinations:** Can be used dynamically with string variables, e.g., `A$ = "TEST.BAS" : VERIFY A$`.

## 3. Code Examples
```basic
10 REM Basic Verification
20 VERIFY "TEST.BAS"
30 REM Using variables
40 F$ = "MODULE1.BAS"
50 VERIFY F$
```

## 4. Internal C-Source Mapping
Implemented in `debug.c` (`pi_parse_verify`) and `check.h`.

## 5. Implementation Details
The `VERIFY` command dynamically allocates a temporary `ProgramStore` structure of size `MAX_PROGRAM_LINES`. 
**Execution Paths:** The Lexer identifies `KW_VERIFY`. The string argument is evaluated and passed to the VFS (Virtual File System) to resolve the absolute path. A temporary program buffer is allocated. The target file is read and parsed into this buffer. `run_static_analysis()` is executed on this temporary AST. 
**Struct Mutations:** The temporary `ProgramStore` is heavily mutated during parsing, but the main runtime `ProgramStore` (pointed to by `rt->program`) remains completely isolated and untouched. Upon completion (success or error), the temporary `ProgramStore` is freed via `free_program_store()`.
**Troubleshooting:** If `VERIFY` crashes, verify that `MAX_PROGRAM_LINES` is respected and the system has sufficient heap memory for the temporary allocation.

## 6. Cross-References / See Also
CHECK, LOAD, TRACE

## 7. Historical Context
A unique feature to BASIC++ designed for CI/CD pipelines and batch testing of external scripts before execution, ensuring that bad code isn't inadvertently run or loaded over working programs. Not found in standard GW-BASIC or QBASIC.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Create a dummy file containing a deliberate error: `10 PRINT "HELLO" : 20 GOTO 500 : SAVE "DUMMY.BAS"`.
3. Clear memory with `NEW`.
4. Type `VERIFY "DUMMY.BAS"`. The interpreter should output a missing line error for 500.
5. Type `LIST`. The memory should be completely empty, proving `VERIFY` did not load `DUMMY.BAS`.
