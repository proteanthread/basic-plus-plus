...
## 2. Description & Usage
The `TROFF` command explicitly disables program tracing that was previously enabled by the `TRACE` (or `TRON`) command.
**Usage:** Used to stop the console from being flooded with line numbers once the flow of execution through a specific section of code has been verified.
**Edge Cases & Boundary Conditions:** 
- Executing `TROFF` when tracing is already off has no effect and does not throw an error.
- Safe to use inside error-handling routines to ensure tracing is disabled during catastrophic failures.
**Syntax Combinations:** Paired directly with `TRACE ON`. `100 TRACE ON ... 200 TROFF`.
...
30 PRINT "Skipped"
50 TROFF
60 PRINT "Tracing disabled."
```

## 4. Internal C-Source Mapping
- `source/parser/troff_parser.c`
- `source/virtual/troff_vm.c`

## 5. Implementation Details
The `TROFF` instruction simply clears the tracing flag in the VM state.
**Execution Paths:** The Lexer identifies `KW_TROFF`. The AST evaluator executes the corresponding node.
**Struct Mutations:** Clears the `TRACE_MODE_ACTIVE` bit in the `rt->state` struct. 
**Troubleshooting:** If tracing continues after `TROFF`, ensure no subsequent `TRACE ON` commands are being executed inadvertently (e.g., inside an interrupt or `ON TIMER` handler) and that the struct bitwise clear operation `rt->state &= ~TRACE_MODE_ACTIVE;` is properly synchronized if multi-threading is ever introduced.

## 6. Cross-References / See Also
TRACE, DEBUG

## 7. Historical Context
The exact counterpart to `TRON`, carrying forward the identical semantics from GW-BASIC and QBASIC, ensuring backward compatibility for legacy scripts.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type `10 TRACE ON : 20 PRINT "A" : 30 TROFF : 40 PRINT "B"`.
3. Type `RUN`.
4. Verify that the line number `[20]` is printed, but `[40]` is NOT printed, confirming `TROFF` successfully disabled tracing.
