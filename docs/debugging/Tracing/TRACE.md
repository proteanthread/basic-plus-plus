...
- `ON`: Enables line tracing.
- `OFF`: Disables line tracing (equivalent to `TROFF`).

## 2. Description & Usage
The `TRACE` command enables the tracing of program execution. When active, the interpreter prints the line number of each statement as it is executed.
**Usage:** Highly useful for tracking the flow of control, especially through complex `GOTO`, `GOSUB`, or `WHILE` loops.
**Edge Cases & Boundary Conditions:** 
- If `TRACE` is executed in immediate mode, it applies to the next `RUN` command.
- Rapid execution of a tight loop with `TRACE ON` will flood the console and may cause buffer lag.
**Syntax Combinations:** Can be selectively enabled and disabled around problematic code blocks: `10 TRACE ON : GOSUB 100 : TRACE OFF`.
...
- `source/parser/trace_parser.c`
- `source/virtual/trace_vm.c`

## 5. Implementation Details
The `TRACE` instruction intercepts the standard execution loop to output the current program counter's line number.
**Execution Paths:** Parses `KW_TRACE`. Depending on the arguments, it evaluates to enabling the trace state.
**Struct Mutations:** Sets the `TRACE_MODE_ACTIVE` bit inside the `rt->state` struct. During the VM's main loop in `vm_run()`, if `TRACE_MODE_ACTIVE` is true, a console output routine is called to print `[Line X]` before evaluating `rt->pc->ast`.
**Troubleshooting:** If line numbers are not printing, verify that `console.h` redirection is correctly receiving the trace output, and that `TRACE_MODE_ACTIVE` is not being prematurely cleared by an error handler.

## 6. Cross-References / See Also
TROFF, DEBUG

## 7. Historical Context
Directly analogous to the classic `TRON` (Trace On) command found in GW-BASIC and QBASIC, modernized for the BASIC++ virtual machine architecture.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type `10 TRACE ON : 20 X = 1 : 30 X = X + 1 : 40 TRACE OFF`.
3. Type `RUN`.
4. You should see the line numbers `[20]`, `[30]`, and `[40]` printed to the console as the program executes.
