...
- `ON`: Enables interactive debugging mode.
- `OFF`: Disables interactive debugging mode.

## 2. Description & Usage
The `DEBUG` command toggles the interactive debugging environment in BASIC++. When `DEBUG ON` is executed, the interpreter enters a specialized state where users can step through code, inspect variables, and monitor memory usage in real-time.
**Usage:** Used for isolating logical flaws, memory leaks, or incorrect variable states during execution.
**Edge Cases & Boundary Conditions:** 
- Issuing `DEBUG ON` while already in debug mode has no effect.
- Using `DEBUG` inside a tight loop without a breakpoint can significantly slow down execution due to internal logging overhead.
**Syntax Combinations:** Can be placed directly inside a program (e.g., `100 DEBUG ON : GOSUB 500 : DEBUG OFF`) to trace only specific subroutines.
...
- `source/parser/debug_parser.c`
- `source/virtual/debug_vm.c`
- Defined in `debug.h`

## 5. Implementation Details
The `DEBUG` instruction modifies the core virtual machine state flags.
**Execution Paths:** The parser identifies `KW_DEBUG` followed by `KW_ON` or `KW_OFF`. 
**Struct Mutations:** It sets or clears the `DEBUG_MODE_ACTIVE` bitfield inside the `rt->state` (BASIC_VM_STATE) structure. When active, the main execution loop (in `vm_run`) invokes the `debug_hook()` function before executing every AST node.
**Troubleshooting:** If the debugger fails to catch breakpoints, ensure the `rt->state` mutation successfully flipped the `DEBUG_MODE_ACTIVE` flag, and verify that `debug_hook()` is correctly registered in the VM function pointer table.

## 6. Cross-References / See Also
TRACE, TROFF, STOP

## 7. Historical Context
Derived from early debug mechanisms but expanded significantly for BASIC++. While standard dialects often relied only on `TRON`/`TROFF`, `DEBUG` introduces a modern, interactive stepping environment.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type `10 A = 1 : 20 DEBUG ON : 30 A = 2 : 40 DEBUG OFF`.
3. Type `RUN`.
4. The execution should pause at line 30, presenting the debug prompt.
5. Use debug commands (e.g., `STEP` or `CONTINUE`) to resume execution.
