...
> [!WARNING]
> **PLANNED / FUTURE** — The LOG: device is not yet implemented as a
> built-in device. LOG: is a user-creatable pattern that can be registered
> via `vdev_register()` in a user slot (VDEV_USER+). It is NOT a built-in
> device like CON: or ERR:.
...
=====================================================================
  ...
  ---- Generated C Runtime ----
    ...
    static void basic_print(const char *s)
    {
        fputs(s, stdout);
    }

    static void basic_error(const char *s)
    {
        fputs(s, stderr);
    }
    ...
    static void logger(const char *fmt, ...)
    {
        ...
        #ifdef BASIC_LOG_FILE
        {
            va_list ap;
            FILE *lf = fopen(BASIC_LOG_FILE, "a");
            if (lf) {
                time_t t = time(NULL);
  ...
  ---- PRINT Mapping ----
    ...
    BASIC                       C Output
    -------------------------   ---------------------------
    PRINT "Hello"               basic_print("Hello\n");
    PRINT #2, "Error"           basic_error("Error\n");
    FPRINT #9, "Log msg"        logger("Log msg\n");
  ...
  Becomes:
    ...
    #define BASIC_LOG_FILE "session.log"
  ...
  abstract I/O layer that can be swapped at link time:
    ...
    /* vdev.h — platform-independent I/O interface */
    void vdev_init(void);
    void vdev_print(const char *s);
    void vdev_error(const char *s);
    void vdev_log(const char *s);
    int  vdev_input(char *buf, int max);
    void vdev_cls(void);
    void vdev_flush(void);

  Default implementation (vdev_stdio.c):
    Uses printf/fprintf/fgets — works everywhere.

  Network implementation (vdev_net.c):
    Uses socket send/recv — for irata.online.

  Embedded implementation (vdev_uart.c):
