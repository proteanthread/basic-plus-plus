...
## 2. Description & Usage
Allows a user to completely rewrite how the parser interprets a given keyword by overriding it with a text macro or alternative syntax without modifying the original user code. Powerful for metaprogramming and dynamic replacements.

**Advanced NLP Depth: Edge Cases & Boundary Conditions:**
- **Lexical Precedence**: When OVERRIDE interacts with other language constructs, the parser resolves it strictly before variable evaluation. If an identifier matches exactly, it triggers the routine.
- **Boundary Conditions**: Calling OVERRIDE with maximum argument lengths (e.g., strings over 255 characters or nesting limits) must be handled by the allocator. Out-of-bounds parameters will throw a SYNTAX ERROR or TYPE MISMATCH rather than overflowing.
...
OVERRIDE PRINT "?"
OVERRIDE LIST
OVERRIDE BEEP "PRINT \"BEEP!\""
```

**Advanced Execution Path Example:**
```basic
' Demonstrating edge case and boundary condition for OVERRIDE
ON ERROR GOTO ErrorHandler
PRINT "Executing OVERRIDE..."
' Insert OVERRIDE usage here conditionally
END
ErrorHandler:
  PRINT "Trapped error in OVERRIDE: "; ERR
  RESUME NEXT
```

## 4. Internal C-Source Mapping
Implemented in `config/parser_config.c` (`pi_parse_override`) and `config/override.c`.

## 5. Implementation Details
Maintains an `override_table` indexed by `KeywordId`. When the parser processes a statement, it checks if an override exists. If active, the parser substitutes the original token's semantic meaning with the configured override text. Protected keywords (like OVERRIDE, SCOPE, ALIAS, KEYWORD, REM) cannot be overridden to prevent unrecoverable parser corruption. Blocked at security LEVEL 2+.

**Execution Paths & Struct Mutations:**
- **Struct Mutation**: The internal C struct representing the interpreter state (often `struct basic_ctx`) undergoes targeted mutations. Variables, string pools, or stack pointers are updated atomically. If OVERRIDE fails mid-execution, the AST node evaluator unwinds state to prevent memory leaks.
- **Execution Path**: The lexer tokenizes OVERRIDE, passing it to the parser, which constructs an AST node. The interpreter loops and matches the statement ID, invoking the underlying C function for OVERRIDE.
- **Troubleshooting Scenarios**: If OVERRIDE behaves unexpectedly, ensure no memory corruption occurred in previous lines. Check for pointer alignment issues (especially on 64-bit systems) where `intptr_t` conversions might have truncated. Recompile and run the SELFTEST.

## 6. Cross-References / See Also
ALIAS, KEYWORD, SCOPE

## 7. Historical Context
Provides macro-like rewriting capabilities to standard BASIC, enabling deep syntactic sugar customization far beyond standard ECMA-116. GW-BASIC lacked any concept of runtime token substitution.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type `OVERRIDE BEEP "PRINT \"BEEP!\""`.
3. Type `BEEP` and verify that it prints the text rather than executing the native hardware BEEP command.
4. Type `OVERRIDE BEEP CLEAR`.
