...
- **Boundary Conditions**: Calling ALIAS with maximum argument lengths (e.g., strings over 255 characters or nesting limits) must be handled by the allocator. Out-of-bounds parameters will throw a SYNTAX ERROR or TYPE MISMATCH rather than overflowing.
...
```basic
' Demonstrating edge case and boundary condition for ALIAS
ON ERROR GOTO ErrorHandler
PRINT "Executing ALIAS..."
' Insert ALIAS usage here conditionally
END
ErrorHandler:
  PRINT "Trapped error in ALIAS: "; ERR
  RESUME NEXT
```

## 4. Internal C-Source Mapping
Implemented in `config/parser_config.c` (`pi_parse_alias`), `alias_lang.c`, and `alias_lang.h`.

## 5. Implementation Details
Modifies the lexical tokenization process. When the lexer encounters an identifier, it first checks the active alias tables (both global and program-scoped) before falling back to the native keyword list. Alias tables have a fixed capacity (`MAX_ALIASES`). Overloading or exceeding this capacity triggers an error. Language packs are predefined sets of aliases loaded sequentially. Alias bindings persist either globally or within the program scope depending on the execution context.

**Execution Paths & Struct Mutations:**
- **Struct Mutation**: The internal C struct representing the interpreter state (often `struct basic_ctx`) undergoes targeted mutations. Variables, string pools, or stack pointers are updated atomically. If ALIAS fails mid-execution, the AST node evaluator unwinds state to prevent memory leaks.
- **Execution Path**: The lexer tokenizes ALIAS, passing it to the parser, which constructs an AST node. The interpreter loops and matches the statement ID, invoking the underlying C function for ALIAS.
- **Troubleshooting Scenarios**: If ALIAS behaves unexpectedly, ensure no memory corruption occurred in previous lines. Check for pointer alignment issues (especially on 64-bit systems) where `intptr_t` conversions might have truncated. Recompile and run the SELFTEST.

## 6. Cross-References / See Also
OVERRIDE, OPTION, KEYWORD

## 7. Historical Context
A novel feature for BASIC++ that significantly extends the static keyword definitions of ECMA-116 and QBASIC, allowing for true multilingual and custom-dialect programming without recompiling the interpreter.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type `ALIAS PRINT = "SAY"`.
3. Type `SAY "Testing"` and verify it behaves identically to PRINT.
4. Type `ALIAS LIST` to view the mapping.
