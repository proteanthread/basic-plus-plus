...
## 2. Description & Usage
An introspection and configuration command that allows viewing and modifying internal parser properties for specific keywords at runtime. Useful for dynamic syntax adjustments, allowing users to override keyword traits like spacing and context.

**Advanced NLP Depth: Edge Cases & Boundary Conditions:**
- **Lexical Precedence**: When KEYWORD interacts with other language constructs, the parser resolves it strictly before variable evaluation. If an identifier matches exactly, it triggers the routine.
- **Boundary Conditions**: Calling KEYWORD with maximum argument lengths (e.g., strings over 255 characters or nesting limits) must be handled by the allocator. Out-of-bounds parameters will throw a SYNTAX ERROR or TYPE MISMATCH rather than overflowing.
...
KEYWORD LIST
KEYWORD PRINT PROP 1
```

**Advanced Execution Path Example:**
```basic
' Demonstrating edge case and boundary condition for KEYWORD
ON ERROR GOTO ErrorHandler
PRINT "Executing KEYWORD..."
' Insert KEYWORD usage here conditionally
END
ErrorHandler:
  PRINT "Trapped error in KEYWORD: "; ERR
  RESUME NEXT
```

## 4. Internal C-Source Mapping
Implemented in `config/parser_config.c` (`pi_parse_keyword`).

## 5. Implementation Details
Accesses the internal keyword property tables. Modifying properties can change how the parser handles argument expectations, precedence, or dialect affinity for a given `KeywordId`. Like SCOPE, modification is blocked at security LEVEL 2+. Uses `keyword_prop_describe` and `keyword_prop_clear` to manipulate internal properties.

**Execution Paths & Struct Mutations:**
- **Struct Mutation**: The internal C struct representing the interpreter state (often `struct basic_ctx`) undergoes targeted mutations. Variables, string pools, or stack pointers are updated atomically. If KEYWORD fails mid-execution, the AST node evaluator unwinds state to prevent memory leaks.
- **Execution Path**: The lexer tokenizes KEYWORD, passing it to the parser, which constructs an AST node. The interpreter loops and matches the statement ID, invoking the underlying C function for KEYWORD.
- **Troubleshooting Scenarios**: If KEYWORD behaves unexpectedly, ensure no memory corruption occurred in previous lines. Check for pointer alignment issues (especially on 64-bit systems) where `intptr_t` conversions might have truncated. Recompile and run the SELFTEST.

## 6. Cross-References / See Also
ALIAS, OVERRIDE

## 7. Historical Context
An advanced metacircular feature in BASIC++ that exposes the interpreter's internal lexical mappings directly to the user. It allows the language itself to mutate its own core traits dynamically.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type `KEYWORD PRINT DESCRIBE`.
3. Verify that the properties for the PRINT keyword are output to the console.
