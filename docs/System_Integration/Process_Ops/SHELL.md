...
### Description & Usage
Executes a command synchronously via the host operating system shell. If no argument is provided, it drops the user into an interactive OS shell (e.g. `cmd` or `/bin/sh`). 
Supports rudimentary redirection (`>`, `>>`) and piping (`|`) natively parsed by the interpreter.

### Deep Dive: SHELL Operation
When SHELL is invoked, the virtual machine context first validates the current execution phase (e.g., immediate vs deferred execution). String pooling is heavily monitored, ensuring that any temporary string generation from environment interaction is immediately registered in the garbage collection phase. Failure to properly resolve system properties or environment bounds results in a silent zero-length string or null byte rather than a hard crash, adhering to the fail-safe execution protocol. It interacts directly with the memory-mapped abstract syntax tree (AST) via isolated node traversal to prevent pointer arithmetic violations. Side effects are highly deterministic and contained within the `BASIC_VM_STATE` struct.
...
```basic
REM Advanced SHELL Usage Example
DEFINT A-Z
ON ERROR GOTO ErrorHandler
X$ = SHELL$("ADVANCED_TEST_VAL")
IF X$ = "" THEN PRINT "Value null or unresolved"
PRINT "Hash check: "; HASH$(X$)
END
ErrorHandler:
  PRINT "System Intercept Error: "; ERR
  RESUME NEXT
```
This complex example combines dynamic resolution with error trapping and robust hash verification to ensure runtime state integrity.

### Internal C-Source Mapping
Implemented in `source/shell/parser_shell.c` (`pi_parse_shell`).

### Implementation Details
Uses standard C `system()` calls. Parses `>` and `>>` and `|` tokens manually when part of a `SHELL` statement to construct complex shell execution strings up to ~1024 bytes. Security sensitive on hardened environments.

### Architecture and Memory Mechanics
The SHELL implementation operates using a highly abstracted C17 functional callback pattern. It is intrinsically tied to the token stream parser; when the Lexer identifies the token, it constructs an AST node of type `NODE_SYS_EXT`. During the evaluate phase, `pi_eval_expr` (or the respective statement interpreter) invokes the system bridge. Mutability is locked strictly behind the `virtual_sys_mutex`, meaning multi-threaded background evaluations safely wait for the lock. Memory constraints are bounded by standard size limits (usually 255 to 4096 bytes depending on string compilation flags) to avoid heap fragmentation. Pointer casts are meticulously filtered through `<stdint.h>` types to guarantee 64-bit portability. Any change to this keyword requires updating the symbol table `sym_register_sys` and validating the C17 regression suites.

### Cross-References / See Also
EXEC, SYSTEM

### Historical Context
GW-BASIC standard. Enhanced in BASIC++ to handle parsed redirections directly.

### Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type `SHELL "echo Hello > test.txt"` and press Enter.
3. Check that `test.txt` was created with "Hello".
