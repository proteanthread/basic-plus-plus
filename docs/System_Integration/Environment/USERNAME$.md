...
**Parameters:**
None. `USERNAME$` is a parameterless system string function.

## 2. Description & Usage

The `USERNAME$` function retrieves the login name of the current user executing the BASIC++ interpreter as a string.

This is highly useful for creating personalized greetings in scripts, logging user actions, mapping user-specific paths (like extracting configuration files from a home directory), or constructing security and permission-checking logic based on the user identity.

**Note on Command Form:** 
BASIC++ also supports a command statement form without the string suffix (`USERNAME`), which simply prints the current username directly to the console output. The `USERNAME$` function form is used when you need to assign the value to a variable or evaluate it within an expression.
...
**Example 2: Constructing a User-Specific Path**
```basic
CONF_PATH$ = "C:\Users\" + USERNAME$ + "\config.ini"
PRINT "Loading configuration from: " + CONF_PATH$
```

**Example 3: Command Statement Form**
```basic
' Prints the username directly to the screen
USERNAME 
```

## 4. Internal C-Source Mapping

- **Lexer/Parser:** `source/parser/parser_expr.c` (Expression parsing block) and `source/help/parser_help.c` (Command form `pi_parse_username`)
- **Tokens:** `KW_USERNAME_FUNC`

## 5. Implementation Details

- **Execution Path (Function Form):** During expression evaluation in `parser_expr.c`, encountering `KW_USERNAME_FUNC` triggers an OS-level query to retrieve the active user's name. The resulting string is pushed onto the evaluation stack.
- **OS API Interaction:** 
  - On Windows (`_WIN32`), it uses the Win32 API `GetUserNameA()`.
  - On POSIX/Linux, it looks up the `USER` environment variable via `getenv("USER")`.
  - In the command statement form (`pi_parse_username`), it first checks `getenv("USERNAME")`, then `getenv("USER")`. 
  - If all retrieval methods fail or the user lacks an identity block, it defaults to the fallback string `"user"`.
- **Memory Management:** The retrieved string is duplicated and allocated within the BASIC++ garbage-collected string pool (`strpool_store`).
- **Troubleshooting:** If the username returns as `"user"` unexpectedly, it indicates the OS environment variables are missing (on Linux/POSIX) or the Win32 API call `GetUserNameA` failed on Windows, possibly due to restrictive execution permissions or running in a headless/service context with no defined user identity.
...
Legacy BASIC interpreters like GW-BASIC or QBASIC ran on single-user, non-networked operating systems (MS-DOS) which lacked the concept of user accounts or logins. `USERNAME$` is a modern addition to BASIC++ required for interacting with modern multi-user operating systems like Windows 11, Linux, and BSD.

## 8. Manual Testing Guide

1. Open `basicpp-console.exe` (Windows) or `baspp-console` (Linux).
2. Type `PRINT USERNAME$` and press Enter.
3. Validate that the printed string matches your active OS login name (e.g., `rtdos` or `Administrator`).
4. Type the command statement form `USERNAME` and press Enter to verify it prints the identical value.
