...
**Parameters:**
None. `HOSTNAME$` is a parameterless system string function.

## 2. Description & Usage

The `HOSTNAME$` function returns the network name (hostname) of the local computer running the BASIC++ interpreter as a string.

This is highly useful for diagnostic scripts, networking initialization, logging, or uniquely identifying the execution environment when BASIC++ scripts are distributed across multiple machines in a cluster or LAN. 

**Note on Command Form:** 
BASIC++ also supports a command statement form without the string suffix (`HOSTNAME`), which simply prints the hostname directly to the console output. The `HOSTNAME$` function form is used when you need to assign the value to a variable or evaluate it within an expression.
...
```basic
H$ = HOSTNAME$
PRINT "This script is running on: " + H$
```

**Example 2: Conditional Execution by Host**
```basic
IF HOSTNAME$ = "DEV-SERVER-01" THEN
    PRINT "Development environment detected. Bypassing login."
    GOSUB 5000 ' Dev mode init
END IF
```

**Example 3: Command Statement Form**
```basic
' Prints the hostname directly to the screen
HOSTNAME 
```

## 4. Internal C-Source Mapping

- **Lexer/Parser:** `source/parser/parser_expr.c` (Expression parsing block) and `source/help/parser_help.c` (Command form `pi_parse_hostname`)
- **Tokens:** `KW_HOSTNAME_FUNC`

## 5. Implementation Details

- **Execution Path (Function Form):** During expression evaluation in `parser_expr.c`, encountering `KW_HOSTNAME_FUNC` triggers an inline system call to retrieve the computer's name. The resulting string is pushed onto the evaluation stack.
- **OS API Interaction:** 
  - On Windows (`_WIN32`), it uses the Win32 API `GetComputerNameA()`.
  - On POSIX/Linux, it uses the standard `gethostname()` system call.
  - As a fallback (especially in the command parser form `pi_parse_hostname`), it may look up the `COMPUTERNAME` or `HOSTNAME` OS environment variables. If all methods fail, it defaults to `"localhost"`.
- **Memory Management:** The retrieved string is allocated within the BASIC++ garbage-collected string pool (`strpool_store`).
- **Troubleshooting:** If the hostname returns as `"localhost"` unexpectedly, it indicates the OS network layer is unconfigured, or the underlying system calls (`GetComputerNameA` or `gethostname`) failed due to lack of permissions or OS misconfiguration.
...
Legacy BASIC interpreters like GW-BASIC or QBASIC had no built-in awareness of networking or hostnames, as they were originally built for standalone single-tasking DOS systems. `HOSTNAME$` is a modern addition in BASIC++ to support network-aware execution environments.

## 8. Manual Testing Guide

1. Open `basicpp-console.exe` (Windows) or `baspp-console` (Linux).
2. Type `PRINT HOSTNAME$` and press Enter.
3. Validate that the printed string matches your computer's actual network name (you can verify this by typing `hostname` in a standard command prompt or terminal).
4. Type the command statement form `HOSTNAME` and press Enter to verify it prints the identical value.
