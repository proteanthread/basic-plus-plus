...
## 2. Description & Usage
Returns the current vertical line (row) position of the text cursor on the console or virtual terminal. The value ranges from `1` (top of screen) to the maximum height of the terminal (e.g., `24` or `25`).

## 3. Code Examples
```basic
10 LOCATE 10, 5
20 L = CSRLIN
30 PRINT "I am on line "; L
```

## 4. Internal C-Source Mapping
- Function: `builtin_csrlin` in `source/io/builtins_console.c`
- Registration: `builtins.c` under `KW_CSRLIN`

## 5. Implementation Details
The system queries the active `VirtualConsole` subsystem for the `cursor_y` coordinate. It adds `1` before returning, as internal representations are 0-indexed while the BASIC language is strictly 1-indexed.
**Edge Cases & Boundary Conditions:**
- If the program operates in a purely graphical mode (`SCREEN 13` etc.) without a dedicated text layer, `CSRLIN` computes the equivalent text line based on the global `FONT_HEIGHT` parameter.
- **Troubleshooting**: If `CSRLIN` continually returns `24` or `25` without updating, the terminal might be locked in a scrolling loop. Clear the screen with `CLS`.

## 6. Cross-References / See Also
- `POS`, `LOCATE`, `CLS`

## 7. Historical Context
A core GW-BASIC terminal control variable. Used frequently in tandem with `POS(0)` to save and restore the cursor position in text-mode user interfaces.

## 8. Manual Testing Guide
**Interactive Test:**
1. Launch `basicpp-console.exe`.
2. Type `CLS : LOCATE 15, 1 : PRINT CSRLIN`
3. Verify it prints `15`.

**Scripted Test (.BAS):**
```basic
10 CLS
20 LOCATE 5, 10
30 IF CSRLIN = 5 THEN PRINT "CSRLIN IS CORRECT!"
40 LOCATE 20, 1
```
