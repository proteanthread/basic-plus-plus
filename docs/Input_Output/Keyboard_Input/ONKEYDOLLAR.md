...
## 2. Description & Usage
The `ONKEY$` built-in string variable functions as an event-aware counterpart to `INKEY$`. It reads a single character from the keyboard buffer in a non-blocking manner, returning immediately to the caller. 

Just like `INKEY$`, if a key has been pressed, it returns a 1-character string containing the corresponding character. If no key has been pressed, it returns a null string (`""`). The inclusion of this keyword is primarily for symmetry with event-driven paradigms (`ON KEY`) and operates exactly the same as `INKEY$` during standard expression evaluation.

## 3. Code Examples
```basic
10 ' Wait for any key using ONKEY$
20 PRINT "Waiting for a key press..."
30 K$ = ONKEY$
...
- **Lexer/Tokens:** Evaluated in `source/lexer.h` and tokenized as `KW_ONKEY`.
- **Parsing/Execution:** Implemented alongside `INKEY$` in `source/parser/parser_expr.c` (`pi_parse_expression_bval_internal`).
- **Device Abstraction:** Leverages the exact same `vdev_inkey()` routine from `source/virtual/vdev.c`.
- **Help Registration:** Registered in `source/standalone/help/help_core.c`.

## 5. Implementation Details
During the parsing of expressions in `parser_expr.c`, the execution path for `KW_ONKEY` is logically unified with `KW_INKEY`. Both tokens trigger a call to the Virtual Device Layer's keyboard polling function, `vdev_inkey()`.

`vdev_inkey()` retrieves the next character from the keyboard buffer without blocking execution. In SDL GUI modes, this reads from the SDL event queue (`gw_sdl2_get_key()`); in standard text modes, it leverages `_kbhit()` and `_getch()` on Windows, or returns 0 if unsupported on a specific platform.

If a valid character is read (an integer `> 0`), it is processed into a 1-character C string, which is then dynamically stored into the memory-safe string pool via `strpool_store`. A string `BValue` with length 1 is formulated and pushed onto the execution stack. Conversely, if no key was ready, a `BValue` representing a zero-length string (`NULL`) is generated and pushed.

## 6. Cross-References / See Also
- `INKEY$`: The standard non-blocking single-char keyboard read function.
- `ON KEY`: Used to define event-driven interrupt handling for specific keys.
- `INPUT$`: Blocking read for a specified number of characters.

## 7. Historical Context
Unlike `INKEY$`, which was universally standard across 8-bit and 16-bit Microsoft BASICs (including GW-BASIC and QBasic), `ONKEY$` is an extended construct. While classic dialects used `ON KEY(n) GOSUB` for hardware interrupt-like keystroke detection, `ONKEY$` acts as a functional string alias to `INKEY$` inside standard expressions for systems bridging event-polling with continuous string retrieval.

## 8. Manual Testing Guide
To manually verify the functionality of `ONKEY$`, follow these steps:
1. Open your system terminal and execute `basicpp-console.exe`.
2. Input the following test program:
   `10 A$ = ONKEY$`
   `20 IF A$ = "" THEN 10`
   `30 PRINT "You triggered ONKEY$ with: "; A$`
3. Type `RUN` and press Enter. The interpreter will pause visually, polling the input buffer continuously.
4. Press a recognizable key on the keyboard (e.g., `X`).
5. The terminal should immediately output `You triggered ONKEY$ with: X` and complete execution, successfully demonstrating its non-blocking read operation.
