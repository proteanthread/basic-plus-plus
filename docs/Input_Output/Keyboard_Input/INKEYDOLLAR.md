...
## 2. Description & Usage
The `INKEY$` built-in string variable reads a single character from the keyboard buffer. Unlike `INPUT$` or `INPUT`, `INKEY$` is non-blocking, meaning it immediately returns control to the program even if no key was pressed.

If a key was pressed, it returns a 1-character string containing the pressed character. If the keyboard buffer is empty, it returns a null string (`""` or an empty string with length 0). It is primarily used within tight application loops to poll for user input asynchronously without halting program execution, making it indispensable for real-time applications such as games or dynamic menus.

## 3. Code Examples
```basic
10 ' Simple loop waiting for the user to press 'Q'
20 PRINT "Press 'Q' to quit..."
30 K$ = INKEY$
40 IF K$ = "" THEN GOTO 30
50 IF K$ = "Q" OR K$ = "q" THEN GOTO 80
...
- **Lexer/Tokens:** Evaluated in `source/lexer.h` and tokenized as `KW_INKEY`.
- **Parsing/Execution:** Implemented in `source/parser/parser_expr.c` (`pi_parse_expression_bval_internal`).
- **Device Abstraction:** Uses `vdev_inkey()` defined in `source/virtual/vdev.c`.
- **Help Registration:** Registered in `source/standalone/help/help_core.c`.

## 5. Implementation Details
During expression parsing in `parser_expr.c`, `INKEY$` calls the Virtual Device Layer via `vdev_inkey()`. 
- In SDL environments (when `NO_SDL2` is not defined and SDL is active), `vdev_inkey()` polls the SDL event queue with `gw_sdl2_poll_events()` and fetches the key via `gw_sdl2_get_key()`.
- In standard Windows console environments, it utilizes `_kbhit()` from `<conio.h>` to check for available keys in the buffer and consumes one using `_getch()`.
- For other environments, it defaults to returning 0 if no platform-specific non-blocking I/O is supported.

If a valid character is returned (an integer `> 0`), it is cast to a `char` and a temporary 2-byte buffer is allocated. The character is stored, null-terminated, and safely inserted into the string pool (`strpool_store`). A `VAL_STRING` BValue is created with length 1 and pushed onto the execution stack. If no key is detected (`0` returned), a `NULL` string of length 0 is returned.

## 6. Cross-References / See Also
- `ONKEY$`: Event-aware counterpart. Evaluates exactly like `INKEY$`.
- `INPUT`: Blocks and waits for a full string of input from the user.
- `INPUT$`: Blocks and waits for a specified number of characters.

## 7. Historical Context
`INKEY$` is an iconic keyword that originated in early Microsoft BASICs (like MBASIC, GW-BASIC, and QBasic). It has always served as the primary mechanism for asynchronous keyboard polling. Our implementation retains full backward compatibility with historic behaviors while expanding backend support to interface seamlessly with modern SDL2 event loops or the standard Windows console.

## 8. Manual Testing Guide
To manually verify the functionality of `INKEY$`, follow these steps:
1. Open a terminal and run `basicpp-console.exe`.
2. Type `10 A$ = INKEY$` and press Enter.
3. Type `20 IF A$ = "" THEN 10` and press Enter.
4. Type `30 PRINT "You pressed: "; A$` and press Enter.
5. Type `RUN` and press Enter. The program will appear to hang, but it is actively polling the keyboard buffer.
6. Press any visible key on your keyboard (e.g., the letter `J`).
7. The program should immediately print `You pressed: J` and terminate, returning to the `Ok` prompt.
