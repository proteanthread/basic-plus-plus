...
- **BEEP**: Instantly emits a single audible beep (the BEL character `\a` or an SDL synthesized tone) from the PC speaker or active audio device.
- **BEEP ON**: Enables the global error-beeping subsystem. When enabled, the system will beep **twice** on any syntax or runtime error, and **once** when waiting for user input via `INPUT`. This is the default state of the interpreter.
- **BEEP OFF**: Silences the global error-beeping subsystem. Errors will be printed silently, and `INPUT` prompts will no longer emit a waiting beep.

## 2. Description & Usage

The `BEEP` command controls the rudimentary auditory feedback mechanism of the BASIC++ interpreter. 

By default, BASIC++ boots with `BEEP ON` active. This provides legacy-authentic feedback for errors and input waits. Emitting a manual `BEEP` instruction inside a loop can be used as a simple metronome or alert mechanism.

**Edge Cases & Behavior Constraints:**
- **Console Environment:** If running `basicpp-console.exe` on a terminal that suppresses the standard BEL character (`\a`), `BEEP` might result in a visual flash (if the terminal supports visual bells) or nothing at all.
- **SDL Environment:** If running the full GUI `basicpp.exe`, `BEEP` utilizes the underlying SDL2 audio subsystem (`gw_sdl2_beep()`) to generate a clean, synthetic square-wave beep.

## 3. Code Examples

**Manual Beep:**
```basic
10 PRINT "Process complete."
20 BEEP
```

**Disabling Error Beeps:**
```basic
10 BEEP OFF
20 PRINT 10 / 0  ' Throws a divide-by-zero error, but remains silent.
```

**Enabling Error Beeps:**
```basic
10 BEEP ON
20 INPUT "What is your name? ", N$ ' Emits a single beep when prompting for input.
```

## 4. Internal C-Source Mapping

- **Parser:** `source/core/stmtreg_builtins.c` (`pi_parse_beep()`).
- **Runtime Execution:** `source/core/interp.c` (`TOK_BEEP`).
- **Error Control Flow:** `source/core/errors.c` (`error_set_beep()`, `error_get_beep()`).
- **Input Control Flow:** `source/io/parser_io.c`.

## 5. Implementation Details

The `BEEP` command acts dynamically depending on whether the `ON` or `OFF` tokens follow it.

During lexing, `BEEP` resolves to `KW_BEEP`. 
If it is parsed standalone, it executes immediately in the VM as `TOK_BEEP`. 
If the parser detects an `ON` or `OFF` modifier, it calls `error_set_beep(1)` or `error_set_beep(0)` respectively.

**Struct Mutations:** The state is stored in a static global `beep_on_error` flag residing inside `source/core/errors.c`.
**What can be changed:** The frequency and duration of the SDL synthetic beep can be adjusted in the SDL wrapper. The standard console BEL character (`0x07`) cannot be changed as it is an ASCII standard.
**What to expect:** When `BEEP ON` is active, `error_raise()` intercepts the fault and intentionally fires `vdev_beep()` twice before terminating execution. `parser_io.c` intercepts `stdin` waits and fires `vdev_beep()` once.

## 6. Cross-References / See Also
- `SOUND` - Play a specific frequency for a specific duration.
- `PLAY` - Play a sequence of musical notes using MML (Music Macro Language).

## 7. Historical Context
In original GW-BASIC and QBASIC, `BEEP` simply fired the PC speaker. BASIC++ expands this with the `BEEP ON` and `BEEP OFF` toggles, which allow developers to completely mute the otherwise intrusive system-level error and input feedback�a much-requested feature for modern 64-bit environments.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Type `BEEP OFF`.
3. Type `PRINT 1 / 0` (You should see the error, but hear NO beep).
4. Type `BEEP ON`.
5. Type `PRINT 1 / 0` (You should see the error, and hear TWO beeps).
6. Type `INPUT A$` (You should hear ONE beep).
