...
Pulls and returns the next pending message from the specified endpoint, channel, or user mailbox queue.

## 2. Syntax

```basic
RECEIVE$(source$ [, timeout_ms])
```

## 3. Code Example

```basic
10 Val = RECEIVE$(source$ [, timeout_ms])
20 PRINT "Result: "; Val
```
