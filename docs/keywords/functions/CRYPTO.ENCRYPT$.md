...
# `CRYPTO.ENCRYPT$` Keyword Reference
...
Encrypts plaintext using symmetric key and returns hex-encoded ciphertext.
...
```basic
10 Val = CRYPTO.ENCRYPT$(key$, plainte10t$)
20 PRINT "Result: "; Val
```
...
| Name | CRYPTO.ENCRYPT$ |
| Category | Security & Encryption |
| Syntax | CRYPTO.ENCRYPT$(key$, plaintext$) |
