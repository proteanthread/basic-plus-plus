...
First-class Last In, First Out stack buffer operation function.

## 2. Syntax

```basic
LIFO(op$, [val]) | LIFO.PUSH(val) | LIFO.POP()
```

## 3. Code Example

```basic
10 Val = LIFO(op$, [val]) | LIFO.PUSH(val) | LIFO.POP()
20 PRINT "Result: "; Val
```
