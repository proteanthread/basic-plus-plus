...
# `FUJI.JSON.GET$` Keyword Reference
...
Queries remote JSON endpoint via FujiNet and extracts matching value.

## 2. Syntax

```basic
FUJI.JSON.GET$(url$, json_path$)
```

## 3. Code Example

```basic
10 Val = FUJI.JSON.GET$(url$, json_path$)
20 PRINT "Result: "; Val
```
