...
Returns the arithmetic mean of numeric arguments, set elements, or an array (alias for AVG).

## 2. Syntax

```basic
MEAN(val1, val2 [, ...]) or MEAN(arr) or MEAN{...} or MEAN[arr]
```

## 3. Code Example

```basic
10 Val = MEAN(val1, val2 [, ...]) or MEAN(arr) or MEAN{...} or MEAN[arr]
20 PRINT "Result: "; Val
```
