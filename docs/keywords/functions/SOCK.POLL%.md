...
Polls a socket handle for readability or writability events.

## 2. Syntax

```basic
SOCK.POLL%(handle% [, mask% [, timeout_ms%]])
```

## 3. Code Example

```basic
10 Val = SOCK.POLL%(handle% [, mask% [, timeout_ms%]])
20 PRINT "Result: "; Val
```
...
| Name | SOCK.POLL% |
| Category | Hardware & Network |
| Syntax | SOCK.POLL%(handle% [, mask% [, timeout_ms%]]) |
