...
### Description & Usage
Alias for PWD; print the current working directory to the console. This command evaluates system state immediately and outputs the string directly to standard output. It does not return a string value that can be assigned to a variable (for that, use `CWD$`).
...
- Parser: `pi_parse_cwd` in `source/help/parser_help.c` (or equivalent location).
...
### Historical Context
These commands were introduced in BASIC++ to provide quick POSIX/DOS-like shell introspection without requiring `PRINT CWD$()`.

### Manual Testing Guide
Execute `basicpp-console.exe -c "CWD"` in your terminal. Ensure that the correct string is output without any runtime evaluation errors.
