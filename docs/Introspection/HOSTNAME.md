...
### Description & Usage
Print the system hostname to the console. This command evaluates system state immediately and outputs the string directly to standard output. It does not return a string value that can be assigned to a variable (for that, use `HOSTNAME$`).
...
- Parser: `pi_parse_hostname` in `source/help/parser_help.c` (or equivalent location).
- Execution: Direct console output using `gw_printf`.

### Implementation Details
The command invokes platform-specific APIs (`gethostname`, `GetUserName`, `getcwd`, etc.) to retrieve the required environment string, and immediately flushes the result to the attached output buffer. It bypasses the AST expression stack entirely, preventing side-effects on `RuntimeState *rt`.

### Cross-References
- `HOSTNAME$` (String Variable)
- `INFO`
- `ENVIRON`

### Historical Context
These commands were introduced in BASIC++ to provide quick POSIX/DOS-like shell introspection without requiring `PRINT HOSTNAME$()`.

### Manual Testing Guide
Execute `basicpp-console.exe -c "HOSTNAME"` in your terminal. Ensure that the correct string is output without any runtime evaluation errors.
