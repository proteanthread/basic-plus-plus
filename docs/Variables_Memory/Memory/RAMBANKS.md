...
## 1. Syntax & Parameters
`--init-rambanks <N>` (Command Line Switch)
- `N`: Number of RAM banks to allocate (16-bit unsigned integer, max 65535).
- If the flag is omitted, the interpreter defaults to `64` RAM banks (providing sixty-four 1MB chunks of memory).

## 2. Description & Usage
RAMBANKs provide a virtualized segmented memory abstraction layer for BASIC++. They function by allocating discrete chunks of memory (banks) that are managed by the interpreter's internal `MemorySystem`. These banks allow users and internal subsystems to read/write raw memory bytes through built-in keywords such as `PEEK`, `POKE`, `BPEEK`, and `BPOKE`.

RAMBANKs integrate directly with the interpreter's 'mock' BIOS (which can be configured to mimic IBM PC, PCjr, XT, or AT architectures). The 'mock' BIOS manages memory mapping across the allocated banks to simulate hardware interrupts, video memory mapping (like CGA/MDA buffers), and system ROM/RAM footprints without risking illegal memory access on the modern 64-bit host OS. 

Because we target modern systems (Windows 11, Linux, BSD), the number of RAM banks is fully dynamic. However, to maintain stability and integration with the legacy 16-bit abstraction of the mock BIOS, the maximum hard upper limit of allocated RAM banks is capped at 65,535 (a 16-bit unsigned integer).

## 3. Code Examples
```basic
REM Example of writing and reading across RAMBANKS
DEFINT A-Z
REM Switch to a specific memory segment (e.g. video RAM mapped in mock BIOS)
DEF SEG = &HB800 

REM Poke a character into the active segment/bank
POKE 0, 65

REM Read the value back out
A = PEEK(0)
PRINT "Read back: "; A
```

## 4. Internal C-Source Mapping
Implemented within `source/core/memory.c`, `source/memory.h`, `source/core/main.c`, and mapped into virtual space via `source/virtual/vmem.c`. Command line parsing occurs in `source/core/main.c`.

## 5. Implementation Details
The RAMBANK system was refactored to dynamically allocate `mem->banks` as a pointer (`RamBank *banks`) via `calloc`, scaling efficiently to user requirements rather than maintaining a hard-coded array. The global variable `g_init_rambanks` controls this sizing, defaulting to `64`.

Bounds checking is enforced strictly at the command line parser level (capping at 65535) and during initialization in `rambank_init()` to prevent memory exhaustion (OOM). On interpreter shutdown, `rambank_shutdown()` calls `free(mem->banks)` to cleanly discard the allocated memory across all targets.
...
Historically, 8-bit and 16-bit systems (like the IBM PC with GW-BASIC) had strict physical memory constraints (e.g., the 640K barrier or expanded/extended memory paging systems like EMS/XMS). The RAMBANK feature provides a modern, infinitely safer virtual equivalent that respects legacy PEEK/POKE behavior by mapping these virtual addresses back into dynamic safe allocations.

## 8. Manual Testing Guide
1. Launch the console interpreter with a custom bank size: `basicpp-console.exe --init-rambanks 1000` (Windows) or `./baspp-console --init-rambanks 1000` (Linux).
2. Within the interpreter, execute `DEF SEG` to target high memory, and issue `POKE` commands.
3. Exit the interpreter. It should shut down cleanly without exit codes or segmentation faults.
4. Run without the flag to test the 64-bank default initialization.
