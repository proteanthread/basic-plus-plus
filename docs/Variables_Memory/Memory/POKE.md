...
- **Byte Overflow Restrictions**: If `value` exceeds 255 (e.g., `POKE 1000, 300`), the VM will perform a modulo wrap `value AND 255` or throw an `Overflow` error depending on strictness flags.
