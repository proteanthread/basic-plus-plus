...
## 1. Syntax & Parameters
`value = PEEK(offset)`
- `offset`: An integer representing the memory address offset to read from within the currently mapped segment or bank.

## 2. Description & Usage
Reads a single byte (8 bits) directly from the specified memory `offset`. Unlike modern high-level languages, `PEEK` offers direct, low-level memory interrogation. It is essential for interacting with the mock BIOS, checking virtual hardware registers, or manipulating raw graphics buffers.

## 3. Code Examples
```basic
10 DEF SEG = &HB800
20 CHAR = PEEK(0)
30 PRINT "Character at top-left of VRAM: "; CHAR
```

## 4. Internal C-Source Mapping
- Function: `builtin_peek` in `source/variables/builtins_mem.c`
- Registration: `builtins.c` under `KW_PEEK`

## 5. Implementation Details
`PEEK` interacts with the Virtual Memory Management subsystem (`vm_core.c`). It resolves the address by adding the given `offset` to the current `DEF SEG` base.
**Edge Cases & Boundary Conditions:**
- **Boundary Verification**: The memory address is rigidly bounds-checked against the allocated virtual sandbox. If an offset results in an address exceeding the maximum sandbox size (e.g., beyond the 1MB or 64MB limit depending on `SIZE` configuration), it throws an `Out of range` or `Access denied` error. It will **never** cause a host-OS level Segmentation Fault.
- **Troubleshooting**: If `PEEK` is returning unexpected values, ensure your `DEF SEG` is correctly assigned. If `DEF SEG` was never declared, it defaults to the start of the data segment.

## 6. Cross-References / See Also
- `POKE`, `PEEKB`, `DEF SEG`

## 7. Historical Context
In the 1980s, `PEEK` was the primary tool for bypassing BASIC to read hardware. In BASIC++, it reads a strictly sandboxed, virtualized memory layer to ensure host stability.

## 8. Manual Testing Guide
**Interactive Test:**
1. Launch `basicpp-console.exe`.
2. Type `POKE 1000, 255 : PRINT PEEK(1000)`
3. Verify it prints `255`.

**Scripted Test (.BAS):**
```basic
10 POKE 500, 123
20 V = PEEK(500)
30 IF V = 123 THEN PRINT "PEEK OK!"
```
