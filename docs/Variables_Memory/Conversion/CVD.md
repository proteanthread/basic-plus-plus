...
## 1. Syntax & Parameters
`double_val# = CVD(8_byte_str$)`
- `8_byte_str$`: A string containing exactly 8 bytes of raw binary data.

## 2. Description & Usage
Converts an 8-byte binary string into a Double-Precision floating-point numeric value (`#`). Used extensively when reading binary IEEE 754 float data structures from random access files or raw network sockets.

## 3. Code Examples
```basic
10 OPEN "R", 1, "DATA.BIN", 8
20 FIELD 1, 8 AS D_STR$
30 GET 1, 1
40 D_VAL# = CVD(D_STR$)
50 PRINT "Recovered Double: "; D_VAL#
60 CLOSE 1
```

## 4. Internal C-Source Mapping
- Function: `builtin_cvd` in `source/variables/builtins_conv.c`
- Registration: `builtins.c` under `KW_CVD`

## 5. Implementation Details
The VM safely executes a `memcpy()` from the string's character array directly into an 8-byte `double` type.
**Edge Cases & Boundary Conditions:**
- **Length Constraint Exception**: If the supplied string is less than 8 bytes long, it triggers an `Illegal function call` error to prevent reading garbage memory outside the string bounds.
- If the string is longer than 8 bytes, only the first 8 bytes are converted; the rest are safely ignored.
- **Troubleshooting**: `TYPE MISMATCH` will occur if a numeric literal is passed. Ensure the input is strictly string.

## 6. Cross-References / See Also
- `MKD$`, `CVI`, `CVS`, `FIELD`

## 7. Historical Context
A staple of GW-BASIC's Random Access File subsystem (`FIELD` statements). In modern implementations, it acts as a direct memory cast to IEEE 754 doubles, overriding legacy Microsoft Binary Format constraints.

## 8. Manual Testing Guide
**Interactive Test:**
1. Launch `basicpp-console.exe`.
2. Type `PRINT CVD(MKD$(12345.67))`
3. Verify it prints `12345.67`.
...
20 B$ = MKD$(A#)
30 C# = CVD(B$)
40 IF ABS(A# - C#) < 0.001 THEN PRINT "DOUBLE CONVERSION OK"
```
