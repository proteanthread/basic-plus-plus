...
## 1. Syntax & Parameters
`int_val% = CVI(2_byte_str$)`
- `2_byte_str$`: A string containing exactly 2 bytes of raw binary data.

## 2. Description & Usage
Converts a 2-byte binary string into a 16-bit Integer value (`%`). Utilized heavily for extracting short integer metadata from binary headers or `FIELD` file buffers.

## 3. Code Examples
```basic
10 S$ = CHR$(255) + CHR$(0)
20 V% = CVI(S$)
30 PRINT "Integer is: "; V%
```

## 4. Internal C-Source Mapping
- Function: `builtin_cvi` in `source/variables/builtins_conv.c`
- Registration: `builtins.c` under `KW_CVI`

## 5. Implementation Details
Extracts exactly 16 bits (2 bytes) from the string memory block.
**Edge Cases & Boundary Conditions:**
- Fails with `Illegal function call` if the string length is `0` or `1`, securely preventing an out-of-bounds heap read.
- The conversion strictly observes Little-Endian byte ordering, identical to x86 architectures.
- **Troubleshooting**: Ensure the data being extracted was originally created using `MKI$` to maintain byte-order parity across platforms.

## 6. Cross-References / See Also
- `MKI$`, `CVD`, `CVS`, `FIELD`

## 7. Historical Context
Required for decoding GW-BASIC random file buffers.

## 8. Manual Testing Guide
**Interactive Test:**
1. Launch `basicpp-console.exe`.
2. Type `PRINT CVI(MKI$(16000))`
3. Verify it prints `16000`.
...
20 Y$ = MKI$(X%)
30 Z% = CVI(Y$)
40 IF X% = Z% THEN PRINT "INTEGER CONVERSION OK"
```
