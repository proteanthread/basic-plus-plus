...
### Description & Usage
Returns a string representing the present working directory.

The `PWD$` function dynamically retrieves the configured present working directory of the local machine executing the BASIC++ interpreter. This is especially useful for system profiling, configuration selection, or diagnostic logic.

**Deep Dive: PWD$ Operation**
When `PWD$` is invoked, the virtual machine context validates the current execution phase. The returned string is immediately registered in the string pool for garbage collection tracking, preventing memory leaks during continuous execution or loops. If the operating system fails to resolve the value (e.g., restricted permissions or unsupported environment), the function gracefully returns a zero-length string (`""`) rather than crashing, ensuring fail-safe runtime execution.

### Code Examples
```basic
PRINT "Current present working directory: "; PWD$
```

```basic
REM Advanced PWD$ Usage Example
DEFINT A-Z
ON ERROR GOTO ErrorHandler
H$ = PWD$
IF H$ = "" THEN 
    PRINT "Warning: present working directory unresolved or null."
ELSE
    PRINT "Identity: "; H$
    PRINT "Hash: "; HASH$(H$)
END IF
...
Relies on OS-specific APIs. It is intrinsically tied to the token stream parser; when the Lexer identifies the `PWD$` token, it constructs an AST node of type `NODE_SYS_EXT`. During the evaluate phase, `pi_eval_expr` invokes the system bridge. Mutability is locked strictly behind `virtual_sys_mutex`, ensuring thread-safe background evaluations. Memory constraints are bounded by standard size limits (usually 255 to 4096 bytes) to avoid heap fragmentation. Any changes must maintain safe pointer operations via `<stdint.h>` types to guarantee 64-bit portability.
...
### Historical Context
`PWD$` is an extension in BASIC++ designed to support modern operating system environments, and is not present in legacy dialects like GW-BASIC or QBASIC.

### Manual Testing Guide
1. Launch `basicpp-console.exe` (Windows) or `./baspp-console` (Linux).
2. Type `PRINT PWD$` and press Enter.
3. The interpreter should output the current present working directory.
