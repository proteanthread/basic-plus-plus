...
## 1. Syntax & Parameters
`value = PEEKB(bank, offset)`
- `bank`: An integer specifying the memory bank.
- `offset`: The address offset within that bank.

## 2. Description & Usage
Performs an extended `PEEK` operation across partitioned memory banks. It allows reading data from expanded memory beyond the traditional 64KB segment limitation.

## 3. Code Examples
```basic
10 VAL = PEEKB(1, 1000)
20 PRINT "Value in Bank 1 at 1000 is "; VAL
```

## 4. Internal C-Source Mapping
- Function: `builtin_peekb` in `source/variables/builtins_mem.c`
- Registration: `builtins.c` under `KW_PEEKB`

## 5. Implementation Details
`PEEKB` overrides the `DEF SEG` constraints by manually instructing the VMM (Virtual Memory Manager) to map the specified `bank` and read the `offset` byte.
**Edge Cases & Boundary Conditions:**
- **Invalid Banks**: If `bank` refers to a non-existent bank (e.g., `PEEKB(99, 0)`), the memory manager safely halts execution with an `Out of range` exception. It rigorously validates the partition ID before mapping.
- **Syntax Parsing**: Since `PEEKB` uses two arguments, omitting the bank parameter (`PEEKB(100)`) will trigger a hard `WHAT?` syntax error during parsing.
- **Troubleshooting**: Ensure the system has been configured with adequate memory partitions before executing banked memory reads.

## 6. Cross-References / See Also
- `POKEB`, `PEEK`, `SIZE`

## 7. Historical Context
`PEEKB` (and `POKEB`) were frequently found in expanded BASIC dialects for 16-bit computers dealing with EMS (Expanded Memory Specification). It is naturally implemented natively in BASIC++ to handle large sandboxes.

## 8. Manual Testing Guide
**Interactive Test:**
1. Launch `basicpp-console.exe`.
2. Type `POKEB 1, 1500, 128 : PRINT PEEKB(1, 1500)`
3. Verify it outputs `128`.

**Scripted Test (.BAS):**
```basic
10 POKEB 2, 5000, 99
20 IF PEEKB(2, 5000) = 99 THEN PRINT "BANK PEEK OK"
```
