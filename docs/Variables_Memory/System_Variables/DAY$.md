...
```
**Parameters:**
- None. `DAY$` is a read-only String System Variable.

## 2. Description & Usage

Returns the current day of the week as a capitalized string (e.g., "Sunday", "Monday", ..., "Saturday").
...
Implemented as a string system variable keyword. The lexer recognizes the suffix `$` and produces `KW_DAY_FUNC`. During expression parsing, the C library `time()` and `localtime()` functions are used. The `tm_wday` field acts as an index into an array of week day strings. The resulting string is duplicated into the string pool (`strpool_store`) and pushed onto the value stack.
...
- DAY, MONTH, MONTH$, YEAR, DATE$

## 7. Historical Context

In traditional GW-BASIC and QBASIC, getting the weekday required parsing `DATE$` and using external algorithms to calculate the day of the week. BASIC++ introduces `DAY$` as a convenient, modern built-in string variable.
