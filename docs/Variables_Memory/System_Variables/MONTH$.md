...
```
**Parameters:**
- None. `MONTH$` is a read-only String System Variable.

## 2. Description & Usage

Returns the name of the current month as a capitalized string (e.g., "January", "February", ..., "December").
...
**Example:**
```basic
PRINT "We are in the month of "; MONTH$
```
*Expected Output:*
```text
...
Implemented as a string system variable. The lexer handles the string suffix to generate `KW_MONTH_FUNC`. During parsing, the current time is retrieved using C standard time functions. The `tm_mon` field serves as a 0-11 index into an internal C array of month names. The selected name is allocated into the interpreter's string pool and pushed to the value stack.
...
While most retro BASIC interpreters lacked a direct way to retrieve the month name without lookup arrays, BASIC++ provides `MONTH$` as a convenient feature avoiding boilerplate code.
