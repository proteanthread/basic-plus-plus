...
## 2. Description & Usage
HOURS returns the current hour of the system clock as an integer ranging from 0 to 23.
...
Implemented in parser_expr.c using localtime(&t)->tm_hour. It is evaluated dynamically at runtime by the parser (`parser_expr.c`) or built-in function handler (`builtins.c`), mapping to the C standard library `time()` and `localtime()` functions, or the virtual device `vdev_get_time()`.
...
```bash
basicpp-console.exe -c "PRINT HOURS"
```
