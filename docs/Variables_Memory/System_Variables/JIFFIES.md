...
## 2. Description & Usage
The JIFFIES function returns the elapsed system time multiplied by a specific video mode frequency. By default, it multiplies by `rt->jiffies_multiplier`. It can take an optional string argument such as "NTSC" (60.0), "PAL" (50.0), or "SECAM" (50.0) to simulate ticks for specific video hardware standards.
...
## 3. Code Examples
```basic
10 PRINT "NTSC Ticks: "; JIFFIES("NTSC")
20 PRINT "PAL Ticks: "; JIFFIES("PAL")
30 PRINT "Default Ticks: "; JIFFIES
```

## 4. Internal C-Source Mapping
- parser_expr.c (Lines 1324-1341)
- lexer.c (tokenization as KW_JIFFIES)

## 5. Implementation Details
Implemented in parser_expr.c under KW_JIFFIES. It queries `vdev_get_time()`, retrieves an optional string argument if parentheses are present, and adjusts the multiplier accordingly using `_stricmp`. Returns a `bval_float`. If the string argument is invalid, it silently falls back to the default multiplier. Stack pushes must be monitored for overflows during parsing.
...
An extension added to BASIC++ to aid in porting and emulating classic 8-bit games which relied on specific hardware interrupt frequencies (VBLANK intervals).

## 8. Manual Testing Guide
1. Run basicpp-console.exe
2. Type PRINT JIFFIES("NTSC") and PRINT JIFFIES("PAL")
3. Observe the proportional difference in output values.
4. Type PRINT JIFFIES to test the zero-argument fallback.
