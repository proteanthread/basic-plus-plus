...
## 2. Description & Usage
TICKS returns the system uptime in milliseconds. Often used for measuring elapsed time or as a high-resolution timer.
...
- `source/help/help.c
- `source/standalone/help/help_core.c`

## 5. Implementation Details
Implemented in parser_expr.c as KW_TICKS using vdev_get_time() to obtain a float representing milliseconds. It is evaluated dynamically at runtime by the parser (`parser_expr.c`) or built-in function handler (`builtins.c`), mapping to the C standard library `time()` and `localtime()` functions, or the virtual device `vdev_get_time()`.
...
```bash
basicpp-console.exe -c "PRINT TICKS"
```
