...
## 1. Syntax & Parameters
`TRUE`
- No parameters. `TRUE` is a read-only system variable.

## 2. Description & Usage
The `TRUE` system variable evaluates directly to a predefined integer representing boolean truth. Utilizing the `TRUE` keyword instead of hardcoding numeric values (such as `-1` or `1`) heavily enhances code readability and guarantees portability across varying BASIC dialects, particularly between GW-BASIC, QBASIC, and ECMA-116 profiles.

`TRUE` is predominantly used in conditional assignments, infinite loop constructions (e.g., `WHILE TRUE`), and explicitly setting logical state flags. It is a system-reserved constant, meaning that assignment attempts (e.g., `LET TRUE = 1`) will throw a definitive Syntax Error.

## 3. Code Examples
```basic
10 LET FLAG = TRUE
20 IF FLAG = TRUE THEN PRINT "The flag is active!"
30 WHILE TRUE
40   PRINT "Infinite loop (Press CTRL+C to break)"
50   BREAK
60 WEND
70 PRINT "Numeric value of TRUE is: "; TRUE
```

## 4. Internal C-Source Mapping
- `source/help/help.c`: Maps `TRUE` to its description and category `HCAT_SYSVAR`.
- `source/core/eval.c`: Evaluates `TOK_TRUE` and pushes the corresponding integer onto the evaluation stack.
- `source/codegen/bytecode.c`: PCODE jumps (`PCODE_JUMP_TRUE`) map execution pathways based on truthiness.

## 5. Implementation Details
`TRUE` acts as a hardcoded read-only system variable parsed at the expression level. Upon lexical analysis, `"TRUE"` is converted to a specific token (`TOK_TRUE` or `KW_TRUE`). During bytecode compilation and evaluation, this token is dynamically replaced with the system's active representation of truth.

Because BASIC++ supports multiple underlying architectures and dialect modes, `TRUE` prevents legacy programs from breaking if the internal representation of truth differs from the traditional all-bits-set (`-1`) implementation. If truthy operations fail to trigger branches, developers should audit the `PCODE_JUMP_TRUE` logic in `pcode_compiler.c` to ensure bitwise masking correctly traps the emitted `TRUE` token.
...
In traditional GW-BASIC and QBASIC, boolean `TRUE` was strictly evaluated as `-1` (all bits set to `1` in a signed integer), which allowed bitwise operators (`AND`, `OR`, `NOT`) to double as logical operators perfectly. Other programming languages (like C) evaluate truth as `1`. By providing a built-in `TRUE` constant, BASIC++ shields the programmer from dialectal variations in boolean representation, offering full ECMA-116 and QBASIC compatibility.

## 8. Manual Testing Guide
To manually verify the integrity of the `TRUE` keyword:
1. Open the console via `basicpp-console.exe` (Windows) or `./baspp-console` (Linux).
2. Enter the following test program:
   ```basic
   10 PRINT "The value of TRUE is: "; TRUE
   20 IF TRUE THEN PRINT "SUCCESS" ELSE PRINT "FAILED"
   ```
3. Type `RUN` and press Enter.
4. The console must output the numeric value of `TRUE` followed immediately by `SUCCESS`.
