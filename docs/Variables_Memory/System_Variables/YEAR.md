...
```
**Parameters:**
- None. `YEAR` is a read-only Numeric System Variable.

## 2. Description & Usage

Returns the current four-digit year as an integer (e.g., 2026).
Because it is a system variable, it does not require parentheses. It is read-only; you cannot assign a value to it (e.g., `YEAR = 1` will throw a syntax error).
...
IF YEAR > 2000 THEN PRINT "Welcome to the future!"
```
*Expected Output:*
```text
The year is 2026
...
Implemented as a numeric system variable. The lexer generates the `KW_YEAR` token. Inside `parser_expr.c`, `localtime()` populates the `tm_year` field. Because `tm_year` represents years since 1900, the parser adds 1900 to this value before pushing it onto the value stack as a 32-bit integer. It cannot be altered by the script.
If the C standard library's time functions fail or are unavailable on a specific embedded platform, the value may default to the system epoch (1970). The memory allocated for the string pool (for string variables) is managed by the garbage collector and freed appropriately. If you alter the system clock while a script is running, the next invocation of `YEAR` will reflect the updated clock.
...
GW-BASIC provided the year as part of the `DATE$` string. Providing `YEAR` directly as a four-digit integer is immune to Y2K-style two-digit truncation issues, aligning with modern programming standards.
...
- **Windows:**
  ```cmd
  basicpp-console.exe -c "PRINT YEAR"
  ```
- **Linux:**
  ```bash
  baspp-console -c "PRINT YEAR"
  ```
