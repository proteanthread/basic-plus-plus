...
## 2. Description & Usage
The DOW (Day Of Week) keyword returns a numeric value from 1 to 7 corresponding to the current day of the week. Depending on the dialect, 1 may represent Sunday or Monday. It is strictly a read-only variable/function.
...
```basic
10 W = DOW
20 IF W = 1 THEN PRINT "It is Sunday!"
```

## 4. Internal C-Source Mapping
- lexer.c (tokenization as KW_DOW)
- builtins_time.c / parser_expr.c (evaluation)

## 5. Implementation Details
Tokenized as KW_DOW. Evaluation relies on the `<time.h>` structure `tm_wday`. Must ensure that the return value correctly maps to the 1-7 range expected by the dialect (usually Sunday=1, Saturday=7). Any failure in evaluation will result in an ERR_SYNTAX if the keyword falls through parser_expr.c without an explicit handler.
...
Present in various specialized BASIC dialects to facilitate calendar and scheduling programs without complex string parsing of DATE$.

## 8. Manual Testing Guide
1. Run basicpp-console.exe
2. Type PRINT DOW
3. Verify the number aligns with the current actual day of the week.
4. Test edge cases on weekends.
