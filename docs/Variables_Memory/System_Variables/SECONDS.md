...
## 2. Description & Usage
SECONDS returns the current second of the system clock as an integer ranging from 0 to 59.
...
## 3. Code Examples
```basic
10 PRINT "Current SECONDS: "; SECONDS
20 SLEEP 1
30 PRINT "Done."
```
...
Implemented in parser_expr.c using localtime(&t)->tm_sec. It is evaluated dynamically at runtime by the parser (`parser_expr.c`) or built-in function handler (`builtins.c`), mapping to the C standard library `time()` and `localtime()` functions, or the virtual device `vdev_get_time()`.
...
```bash
basicpp-console.exe -c "PRINT SECONDS"
```
