...
```
**Parameters:**
- None. `DAY` is a read-only Numeric System Variable.

## 2. Description & Usage

Returns the current day of the month as an integer between 1 and 31.
Because it is a system variable, it does not require parentheses. It is read-only; you cannot assign a value to it (e.g., `DAY = 1` will throw a syntax error).
...
Implemented as a system variable keyword. The lexer produces `KW_DAY`. During expression parsing (`parser_expr.c`), `time()` and `localtime()` from the standard C library are called to retrieve the current date. The parser intercepts the `KW_DAY` token and pushes the `tm_mday` field onto the value stack as a 32-bit integer. The returned value is strictly immutable and cannot be assigned to.
If the C standard library's time functions fail or are unavailable on a specific embedded platform, the value may default to the system epoch (1970). The memory allocated for the string pool (for string variables) is managed by the garbage collector and freed appropriately. If you alter the system clock while a script is running, the next invocation of `DAY` will reflect the updated clock.

## 6. Cross-References / See Also

- DAY$, MONTH, MONTH$, YEAR, TIME$

## 7. Historical Context

Standard BASIC often used a generalized `DATE$` function to return the full date. The separate `DAY` numeric variable provides a more modular, ECMA-116 compliant approach to easily retrieve the current day without needing to substring parse a date string.
...
- **Windows:**
  ```cmd
  basicpp-console.exe -c "PRINT DAY"
  ```
- **Linux:**
  ```bash
  baspp-console -c "PRINT DAY"
  ```
