...
## 1. Syntax & Parameters
`MEM`
- No parameters. `MEM` acts as a read-only system variable/function.

## 2. Description & Usage
The `MEM` keyword returns the total amount of free, unallocated variable memory available to the BASIC program, measured in bytes. This allows the programmer to proactively check system resources before dimensioning large arrays or allocating substantial string buffers, thereby avoiding catastrophic "Out of Memory" runtime errors.

`MEM` specifically inspects the active variable memory pool (which scales based on platform constraints, e.g., 4 KB on embedded, 1 MB on 64-bit systems). If a program dynamically dimensions arrays, monitoring the `MEM` output will precisely reflect the corresponding drop in available bytes.

## 3. Code Examples
```basic
10 PRINT "Available memory at start: "; MEM; " bytes"
20 DIM A(1000)
30 PRINT "Available memory after array: "; MEM; " bytes"
40 IF MEM < 1024 THEN PRINT "Warning: Memory critically low!"
```

## 4. Internal C-Source Mapping
- `source/parser/parser_expr.c`: Evaluates `KW_MEM` and calculates available space via `size - used`.
- `source/lexer/lexer.c`: Contains the lexical mapping for the `"MEM"` keyword.
- `source/core/memory.c`: Manages the underlying structural pools referenced by `MEM` (`rt->memory->variable`).
- `source/config.h`: Defines the baseline sizes for `VARIABLE_MEMORY_SIZE`.

## 5. Implementation Details
During expression parsing, when the AST hits `KW_MEM`, the interpreter instantly queries the live runtime memory structures, specifically `rt->memory->variable.size` (the total allocated pool limit) minus `rt->memory->variable.used` (the bytes currently claimed). The result is pushed to the evaluation stack as a standard integer.

Because the calculation happens natively in the C-source at the moment the token is evaluated, `MEM` is completely accurate to the microsecond. If `MEM` yields negative numbers, verify that the `VARIABLE_MEMORY_SIZE` in `config.h` has not overflowed a 16-bit signed integer boundary without being properly cast to a larger type.
...
In historic microcomputers with extremely constrained RAM (e.g., 64K limits), querying free memory was a vital operation. GW-BASIC provided `FRE(0)` or `FRE("")` to return free bytes and simultaneously trigger garbage collection. `MEM` was introduced in subsequent dialects to strictly decouple the act of "querying free memory" from the expensive act of "garbage collecting the string pool." In BASIC++, `MEM` is instantaneous and strictly reports variable pool boundaries.

## 8. Manual Testing Guide
To manually verify the integrity of the `MEM` keyword:
1. Open the console via `basicpp-console.exe` (Windows) or `./baspp-console` (Linux).
2. Enter the following test program:
   ```basic
   10 M1 = MEM
   20 DIM TEST_ARRAY(500)
   30 M2 = MEM
   40 PRINT "Memory lost: "; M1 - M2; " bytes"
   ```
3. Type `RUN` and press Enter.
4. The console must output a positive integer representing the byte footprint of a 500-element default precision array.
