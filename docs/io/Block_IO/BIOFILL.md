...
## 1. Syntax & Parameters
`bytes_filled = BIOFILL(device_id, byte_value, num_bytes)`

## 2. Description & Usage
Rapidly floods a block device or memory buffer with a specific 8-bit `byte_value` (0-255) for `num_bytes`. It is primarily used for zeroing out memory banks, formatting raw block storage, or preparing graphics framebuffers. Edge Cases: If `byte_value` exceeds 255, it is automatically masked (`& 0xFF`). If `num_bytes` exceeds device capacity, it stops at the boundary.

## 3. Code Examples
```basic
10 REM Zero out 8KB of block storage
20 RES = BIOFILL(1, 0, 8192)
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_block.c` via `builtin_biofill()`.

## 5. Implementation Details
Parser Expectation: Three integers (Device ID, 8-bit Fill Value, Byte Count). Mutability: Heavily mutates the target device buffer. Execution Path: Resolves the VDev and invokes native `memset` for maximum throughput. Troubleshooting: If memory is corrupted elsewhere, verify `device_id` is actually the intended isolated VDev and not mapping to core memory.
...
Equivalent to the concept of `memset` in C, bringing high-speed memory initialization to BASIC++.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Open device 1.
3. Run `PRINT BIOFILL(1, 255, 100)`.
4. Read back using `BIOREAD$` to ensure it contains ASCII 255 characters.
