...
## 1. Syntax & Parameters
`bytes_copied = BIOCOPY(src_device_id, dest_device_id, num_bytes)`

## 2. Description & Usage
Executes a direct block-level memory copy from the source device to the destination device for `num_bytes`. This is the fastest way to clone memory banks, copy disk sectors, or transfer VDev buffers because the data never passes through the BASIC++ interpreter heap. Edge Cases: If `num_bytes` is larger than the destination capacity, it safely truncates the copy to prevent buffer overflows (CWE-120 protection).

## 3. Code Examples
```basic
10 REM Clone memory bank 1 to 2
20 COPIED = BIOCOPY(1, 2, 4096)
30 PRINT COPIED; " BYTES CLONED"
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_block.c` via `builtin_biocopy()`.

## 5. Implementation Details
Parser Expectation: Three integer arguments. Mutability: The destination device buffer is mutated and overwritten. Source remains read-only. Execution Path: Uses VDev abstraction layer to resolve buffer pointers and calls optimized `memcpy`. Boundary Checks: The VDev layer enforces strict bounds checking before copy execution. Troubleshooting: If `bytes_copied` is less than requested, check destination `BIOSIZE`.
...
A modern addition to BASIC++, superseding iterative `PEEK`/`POKE` loops from 8-bit BASICs.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Fill device 1 using `BIOFILL`.
3. Execute `PRINT BIOCOPY(1, 2, 1024)`.
4. Verify it prints 1024.
5. Use `BIOCOMPARE` to verify integrity.
