...
## 1. Syntax & Parameters
`data$ = BIOREAD$(device_id, num_bytes)`

## 2. Description & Usage
Reads raw binary data from a block device and returns it as a standard BASIC++ string. Since strings in BASIC++ are 8-bit clean and length-prefixed, they can safely contain NULL (`\0`) bytes, making this perfect for reading raw sectors, binary files, or network payloads. Edge Cases: Requesting a read larger than the String Pool capacity will trigger an Out of Memory (OOM) error.

## 3. Code Examples
```basic
10 REM Read boot sector
20 SECTOR$ = BIOREAD$(1, 512)
30 PRINT "SECTOR SIZE: "; LEN(SECTOR$)
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_block.c` via `builtin_bioread_str()`.

## 5. Implementation Details
Parser Expectation: Integer device ID, Integer byte count. Returns String. Mutability: Does not mutate the device (read-only), but mutates the String Pool by allocating a new string. Execution Path: Allocates exactly `num_bytes` in the String Pool, invokes VDev read, and populates the string payload. Troubleshooting: If OOM occurs, reduce chunk size and read in a loop.
...
Follows the strict string keyword suffix constraint (`$`) required by BASIC++ project rules.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Run `A$ = BIOREAD$(1, 10)`.
3. Run `PRINT LEN(A$)` to verify 10 bytes were read into the string pool.
