...
## 1. Syntax & Parameters
`status_code = BIOSTATUS(device_id)`

## 2. Description & Usage
Retrieves the internal health and readiness state of the block device. A return value of 0 indicates the device is ready for I/O operations. Non-zero values indicate specific hardware faults, unmounted volumes, or read-only locks. Edge Cases: Querying an invalid device ID returns a permanent offline status code.

## 3. Code Examples
```basic
10 IF BIOSTATUS(1) <> 0 THEN PRINT "BLOCK DEVICE OFFLINE": END
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_block.c` via `builtin_biostatus()`.

## 5. Implementation Details
Parser Expectation: Single integer argument. Mutability: Read-only. Execution Path: Directly reads `vdev->status_flags` from the virtual device abstraction layer. Troubleshooting: Check specific error codes in the technical manual if status is non-zero.
...
Unifies error checking across all block devices instead of relying on ON ERROR GOTO traps.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Run `PRINT BIOSTATUS(99)` to observe the error code for an invalid/unmounted device.
