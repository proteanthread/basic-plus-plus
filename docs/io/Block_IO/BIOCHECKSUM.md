...
## 1. Syntax & Parameters
`chksum = BIOCHECKSUM(data$)`
- `data$`: The string buffer or block of data to checksum. It must be a valid, initialized string.

## 2. Description & Usage
Calculates a CRC-16 (CCITT) checksum of the provided data string. This is extremely useful for verifying the integrity of data read from block devices or preparing data to be written to a block device. It ensures that large blocks of contiguous memory have not suffered from disk rot, transmission errors, or memory corruption.

## 3. Code Examples
```basic
10 OPEN "B", 1, "A:"
20 BIOREAD 1, 512, BUF$
30 CHK = BIOCHECKSUM(BUF$)
40 PRINT "CRC-16 is "; HEX$(CHK)
50 CLOSE 1
```

## 4. Internal C-Source Mapping
- Function: `builtin_biochecksum` in `source/io/builtins_bio.c`
- Registration: `builtins.c` under `KW_BIOCHECKSUM`

## 5. Implementation Details
The `BIOCHECKSUM` function uses a standard CRC-16-CCITT algorithm to compute a 16-bit checksum. It returns the result as an integer value.
**Edge Cases & Boundary Conditions:**
- If an empty string (`""`) is passed, the function deterministically returns `0` without causing a segmentation fault or out-of-bounds read.
- If a string exceeds the maximum string length pool (e.g., trying to checksum a string that is 65535 bytes in some segmented memory models), the runtime string memory allocator safely truncates it at the exact boundary of the string structure, preventing buffer overruns.
- **Troubleshooting**: If `TYPE MISMATCH` occurs, verify that the argument is explicitly a string (`$`). Passing raw memory addresses (like an array pointer) is not permitted; you must `PEEK$` or `BIOREAD$` into a string buffer first.

## 6. Cross-References / See Also
- `BIOREAD`, `BIOWRITE`, `BIOSTATUS`

## 7. Historical Context
Unlike standard GW-BASIC which lacked built-in block device CRC features, `BIOCHECKSUM` is introduced in BASIC++ to provide robust integrity checking for direct device memory manipulation.

## 8. Manual Testing Guide
**Interactive Test:**
1. Launch `basicpp-console.exe`.
2. Type `PRINT HEX$(BIOCHECKSUM("ABC"))` and hit Enter. Verify it produces a valid non-zero hex checksum value (e.g., `514A`).

**Scripted Test (.BAS):**
```basic
10 A$ = "BASIC++ BLOCK DATA"
20 B = BIOCHECKSUM(A$)
30 C = BIOCHECKSUM(A$)
40 IF B = C THEN PRINT "CHECKSUMS MATCH!" ELSE PRINT "DETERMINISM FAILED!"
```
