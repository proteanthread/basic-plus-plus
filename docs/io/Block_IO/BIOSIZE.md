...
## 1. Syntax & Parameters
`capacity = BIOSIZE(device_id)`

## 2. Description & Usage
Returns the total allocated capacity (in bytes) of the specified block device. This is crucial for boundary checking before executing `BIOCOPY` or `BIOFILL` to prevent truncation. Edge Cases: Returns 0 if the device is not initialized, closed, or mapped to a non-block pseudo-device.

## 3. Code Examples
```basic
10 REM Check block size
20 SIZ = BIOSIZE(1)
30 PRINT "DEVICE CAPACITY: "; SIZ; " BYTES"
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_block.c` via `builtin_biosize()`.

## 5. Implementation Details
Parser Expectation: Single integer argument. Mutability: Strictly read-only. Execution Path: Queries the VDev configuration struct (`vdev->capacity`). Troubleshooting: If it returns 0 unexpectedly, ensure the device was properly opened using the `OPEN` statement with a block driver.

## 6. Cross-References / See Also
- `BIOSTATUS`

## 7. Historical Context
Replaces old OS-specific file size interrupts from DOS days with a unified VDev capacity query.

## 8. Manual Testing Guide
1. Launch `basicpp-console.exe`.
2. Run `PRINT BIOSIZE(1)` without opening a device to see it gracefully return 0.
