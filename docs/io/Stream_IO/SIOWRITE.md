...
## 1. Syntax & Parameters
`bytes = SIOWRITE(channel, data$)`

## 2. Description & Usage
Writes string data continuously to the stream. Edge cases: Returns bytes successfully written if stream drops.

## 3. Code Examples
```basic
10 SIOWRITE 1, "HELLO STREAM"
```
...
Parser Expectation: Integer, String. Execution Path: Pushes to `fwrite`.
