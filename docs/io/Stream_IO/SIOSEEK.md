...
## 1. Syntax & Parameters
`status = SIOSEEK(channel, offset)`

## 2. Description & Usage
Moves the read/write cursor of the stream to the specified byte offset from the beginning. Edge Cases: Seeking past EOF may pad with NULLs or fail depending on the stream driver type.
...
Parser Expectation: Two integers. Mutability: Mutates stream state. Execution Path: Calls native `fseek` or custom VDev handler. Troubleshooting: Non-seekable streams (like network pipes) will return error.

## 6. Cross-References / See Also
- `SIOAVAIL`

## 7. Historical Context
Equivalent to `SEEK #` statement.
