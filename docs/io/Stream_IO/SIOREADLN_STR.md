...
## 1. Syntax & Parameters
`line$ = SIOREADLN$(channel)`

## 2. Description & Usage
Reads a complete line from the stream up to the next CR/LF or LF boundary. Edge Cases: If no newline is found before buffer max, returns chunk. If EOF, returns empty string.
...
Parser Expectation: Integer. Returns String. Mutability: Mutates stream pointer. Execution Path: Iterates characters until `\n` is hit. Troubleshooting: Handle empty strings as EOF condition if stream size is static.

## 6. Cross-References / See Also
- `SIOREAD$`

## 7. Historical Context
The classic `LINE INPUT #` functionality wrapped into a standard function.

## 8. Manual Testing Guide
1. `basicpp-console.exe`.
2. `PRINT SIOREADLN$(1)`.
3. Verify empty string.
