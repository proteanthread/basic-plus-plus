...
## 1. Syntax & Parameters
`status = SIOFLUSH(channel)`

## 2. Description & Usage
Forces the internal stream write buffer to flush its contents out to the underlying hardware or OS file. Edge Cases: A full disk will trigger an error upon flush, not upon write.
...
Parser Expectation: Integer argument. Mutability: Empties internal buffers. Execution Path: Calls native `fflush` or hardware sync. Troubleshooting: Call periodically on critical data.

## 6. Cross-References / See Also
- `SIOWRITE`

## 7. Historical Context
Exposes synchronous flushing to avoid data loss on crash.

## 8. Manual Testing Guide
1. `basicpp-console.exe`.
2. `PRINT SIOFLUSH(1)`.
3. Verify it executes safely.
