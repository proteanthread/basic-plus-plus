...
## 1. Syntax & Parameters
`data$ = SIOREAD$(channel, bytes)`

## 2. Description & Usage
Reads an exact number of bytes from the stream buffer into a string. Edge Cases: If fewer bytes are available before EOF, returns whatever is left.
...
Parser Expectation: Two integers. Mutability: Mutates pool and pointer. Execution Path: `fread` or equivalent stream pull. Troubleshooting: Check length of returned string to verify byte fetch count.
...
Equivalent to `INPUT$` in older dialects.

## 8. Manual Testing Guide
1. `basicpp-console.exe`.
2. `PRINT LEN(SIOREAD$(1, 10))`.
3. Verify 0.
