...
## 1. Syntax & Parameters
`bytes = SIOAVAIL(channel)`

## 2. Description & Usage
Returns the number of bytes available to read immediately from the character/stream I/O channel buffer without blocking. Edge Cases: Returns 0 if buffer is empty or stream is closed.

## 3. Code Examples
```basic
10 IF SIOAVAIL(1) > 0 THEN A$ = SIOREAD$(1, 1)
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_stream.c`.

## 5. Implementation Details
Parser Expectation: Integer argument. Execution Path: Checks VDev stream buffer tail/head pointers. Mutability: Read-only. Troubleshooting: SIO channels must be opened with STREAM mode drivers.

## 6. Cross-References / See Also
- `SIOREAD$`

## 7. Historical Context
Similar to standard `LOC` or `LOF` for sequential files, adapted for virtual streams.

## 8. Manual Testing Guide
1. `basicpp-console.exe`.
2. `PRINT SIOAVAIL(1)`.
3. Verify 0.
