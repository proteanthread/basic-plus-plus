...
## 1. Syntax & Parameters
`bytes = NBYTESWAITING(socket_id)`

## 2. Description & Usage
Returns the number of bytes currently sitting in the network socket receive buffer waiting to be read. This is a non-blocking poll. Edge Cases: If the socket is closed or invalid, it returns 0. If the buffer is overflowing, it returns the maximum TCP window size.

## 3. Code Examples
```basic
10 REM Wait for data
20 IF NBYTESWAITING(1) = 0 THEN GOTO 20
30 PRINT "DATA ARRIVED!"
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_net.c` via `builtin_nbyteswaiting()`.

## 5. Implementation Details
Parser Expectation: Single integer argument (Socket ID). Mutability: Read-only. Execution Path: Invokes the underlying OS `recv()` with `MSG_PEEK` or `ioctlsocket(FIONREAD)` without consuming bytes from the stream. Troubleshooting: If it spins endlessly, ensure `NCONNECTED(socket_id)` is true, as a dropped connection might still report 0 bytes waiting.
...
A modern non-blocking networking feature. Older BASICs relied on synchronous serial interrupts.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Open a dummy socket.
3. Run `PRINT NBYTESWAITING(1)`.
4. Verify it returns 0 when no data is sent.
