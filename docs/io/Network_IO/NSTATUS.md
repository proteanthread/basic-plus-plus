...
## 1. Syntax & Parameters
`status = NSTATUS(socket_id)`

## 2. Description & Usage
Returns the high-level operational status of the socket. Returns 0 if healthy, non-zero for faults (e.g., timeout, connection refused, host unreachable). Edge Cases: Different from `NERROR` as it provides abstract BASIC++ state codes rather than raw OS codes.

## 3. Code Examples
```basic
10 IF NSTATUS(1) <> 0 THEN PRINT "NETWORK DOWN"
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_net.c` via `builtin_nstatus()`.

## 5. Implementation Details
Parser Expectation: Single integer argument. Mutability: Read-only. Execution Path: Queries the VDev driver status. Troubleshooting: Check specific BASIC++ error code maps if status > 0.
...
Standardizes error handling across network virtualization.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. `PRINT NSTATUS(1)`.
3. Verify output matches expected error state for dummy socket.
