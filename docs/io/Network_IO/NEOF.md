...
## 1. Syntax & Parameters
`eof_flag = NEOF(socket_id)`

## 2. Description & Usage
Determines if the network stream has reached the End-Of-File (EOF) marker. Returns -1 (True) if the remote host gracefully closed the transmission via FIN packet, otherwise 0. Edge Cases: A socket might be connected but waiting on data; `NEOF` distinguishes between "waiting" and "permanently closed".

## 3. Code Examples
```basic
10 REM Read until connection closes
20 IF NEOF(1) THEN 50
30 A$ = NREAD$(1, 1024)
40 GOTO 20
50 PRINT "TRANSFER COMPLETE"
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_net.c` via `builtin_neof()`.

## 5. Implementation Details
Parser Expectation: Single integer argument. Mutability: Read-only. Execution Path: Relies on `recv()` returning 0 bytes on a blocking/non-blocking poll to detect the FIN state. Troubleshooting: Use `NERROR` if `NEOF` occurs unexpectedly to rule out an RST packet instead of a FIN packet.
...
Adapts the classic `EOF(channel)` concept specifically for network streams.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Execute `PRINT NEOF(1)`.
3. Ensure it returns -1 (True) since the uninitialized socket stream is effectively at its end.
