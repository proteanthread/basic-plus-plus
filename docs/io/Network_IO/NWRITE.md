...
## 1. Syntax & Parameters
`bytes = NWRITE(socket_id, payload$)`

## 2. Description & Usage
Transmits a raw string payload over the network socket. Returns the actual number of bytes sent. Edge Cases: If the network buffer is full or the peer disconnects mid-transmission, it may return fewer bytes than `LEN(payload$)`.

## 3. Code Examples
```basic
10 SENT = NWRITE(1, "GET / HTTP/1.1\r\n\r\n")
20 PRINT SENT; " BYTES SENT"
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_net.c` via `builtin_nwrite()`.

## 5. Implementation Details
Parser Expectation: Integer ID, String payload. Mutability: Mutates network stream. Execution Path: Resolves string from pool, calls `send()`. If non-blocking, it handles partial writes gracefully. Troubleshooting: If `bytes < LEN(payload$)`, slice the string and send the remainder in a loop.
...
The foundational socket `send` function, recently verified and implemented natively.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. `PRINT NWRITE(1, "TEST")`.
3. Verify it returns an error or 0 for an unconnected socket.
