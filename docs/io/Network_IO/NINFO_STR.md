...
## 1. Syntax & Parameters
`info$ = NINFO$(socket_id, query_type)`

## 2. Description & Usage
Returns string-based metadata about the socket. `query_type` determines what is returned: 1 = Remote IP, 2 = Remote Port, 3 = Local IP, 4 = Protocol Type. Edge Cases: Requesting out-of-bounds `query_type` returns an empty string.

## 3. Code Examples
```basic
10 PRINT "CONNECTED TO: "; NINFO$(1, 1)
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_net.c` via `builtin_ninfo_str()`.

## 5. Implementation Details
Parser Expectation: Two integer arguments. Returns String. Mutability: Mutates String Pool. Execution Path: Invokes `getpeername` or `getsockname` and converts the IP/Port structs into a string payload. Troubleshooting: Ensure the connection is actually established before querying remote metadata.

## 6. Cross-References / See Also
- `NSTATUS`

## 7. Historical Context
Essential for modern peer-to-peer applications built in BASIC++.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Run `PRINT NINFO$(1, 1)`.
3. Verify it returns an empty string for an unopened socket.
