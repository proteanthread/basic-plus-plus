...
## 1. Syntax & Parameters
`http_code = NHTTPSTATUS(socket_id)`

## 2. Description & Usage
A high-level helper that attempts to parse the last HTTP response code (e.g., 200, 404, 500) from a raw socket buffer. Edge Cases: If the data in the buffer is not valid HTTP/1.x, it will return 0 or fail to parse.

## 3. Code Examples
```basic
10 CODE = NHTTPSTATUS(1)
20 IF CODE = 404 THEN PRINT "NOT FOUND"
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_net.c` via `builtin_nhttpstatus()`.

## 5. Implementation Details
Parser Expectation: Single integer argument. Mutability: Mutates the internal HTTP parser state. Execution Path: Scans the first line of the received buffer for the HTTP/1.x standard format and extracts the 3-digit integer. Troubleshooting: Only works if the stream has already received the HTTP header via `NREAD$` or internally.

## 6. Cross-References / See Also
- `NJSONQUERY$`

## 7. Historical Context
A modern convenience function not present in any legacy BASIC.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Run `PRINT NHTTPSTATUS(1)`.
3. Verify it returns 0 for a non-HTTP stream.
