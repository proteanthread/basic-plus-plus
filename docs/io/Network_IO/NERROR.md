...
## 1. Syntax & Parameters
`err_code = NERROR(socket_id)`

## 2. Description & Usage
Returns the last specific network error code (e.g., OS-level socket error, WSA error on Windows, or errno on Linux) encountered by the socket. Returns 0 if no error has occurred. Edge Cases: Error states are persistent until cleared by closing the socket or explicitly querying it (which may clear the flag depending on OS).

## 3. Code Examples
```basic
10 IF NSTATUS(1) <> 0 THEN PRINT "NET FAULT: "; NERROR(1)
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_net.c` via `builtin_nerror()`.

## 5. Implementation Details
Parser Expectation: Single integer argument. Mutability: Read-only (but may clear OS error queues). Execution Path: Calls `getsockopt` with `SO_ERROR`. Troubleshooting: Look up the integer return value against POSIX errno or Windows Sockets Error Codes.

## 6. Cross-References / See Also
- `NSTATUS`

## 7. Historical Context
Crucial for network debugging, eliminating blind failures found in early networking implementations.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Execute `PRINT NERROR(1)` on a closed socket.
3. Verify an appropriate error code (or 0) is returned gracefully.
