...
## 2. Description & Usage
Returns the global state of the Transaction I/O engine. 0 means inactive, 1 means transaction is open and pending commit, negative means fault/rollback triggered. Edge Cases: Call before beginning new transactions to prevent nesting panics.

## 3. Code Examples
```basic
10 IF TXNSTATUS() = 1 THEN PRINT "BUSY"
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_txn.c`.

## 5. Implementation Details
Parser Expectation: No arguments. Mutability: Read-only. Execution Path: Checks VM global state for `txn_active` flag. Troubleshooting: If fault is stuck, issue `ROLLBACK` command to clear engine.
...
A modern database-like abstraction for atomic BASIC++ script changes.
