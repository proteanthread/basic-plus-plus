...
## 1. Syntax & Parameters
`str_val$ = PRETRIEVE$(key$)`

## 2. Description & Usage
Retrieves a string value from the Persistent Key-Value Storage system. Edge Cases: If the key doesn't exist or points to a numerical value, it gracefully returns an empty string.

## 3. Code Examples
```basic
10 PLAYER$ = PRETRIEVE$("PLAYER_NAME")
20 PRINT "WELCOME BACK: "; PLAYER$
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_persist.c` via `builtin_pretrieve_str()`.

## 5. Implementation Details
Parser Expectation: String argument. Returns string. Mutability: Mutates String Pool. Execution Path: Locates the blob by key, allocates pool memory, and streams it in. Troubleshooting: If memory pool exhausts, string might be truncated.
...
Adheres to the strict `$` suffix mandate for string returns.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. Run `PRINT PRETRIEVE$("N/A")`.
3. Verify output is blank.
