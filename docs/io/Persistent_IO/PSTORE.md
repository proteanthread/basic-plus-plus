...
## 1. Syntax & Parameters
`PSTORE key$, value [OR] PSTORE key$, value$`

## 2. Description & Usage
Stores either a numerical or string value into the Persistent Key-Value Storage engine under the given string key. It automatically serializes the data to disk. Edge Cases: If disk is full, or VFS is unmounted, it will silently fail or trigger an OS-level IO trap.

## 3. Code Examples
```basic
10 PSTORE "HISCORE", 9999
20 PSTORE "PLAYER_NAME", "HERO"
```

## 4. Internal C-Source Mapping
Mapped in `source/io/builtins_persist.c` via `builtin_pstore()`.

## 5. Implementation Details
Parser Expectation: Polymorphic statement. Takes string key and either numeric or string value. Mutability: Mutates disk storage state. Execution Path: Dispatches to the internal KVS serialization engine and commits. Troubleshooting: Ensure write permissions on the directory hosting the persistent store.
...
Abstracts away complex file OPEN/PUT/CLOSE loops into a single atomic statement.

## 8. Manual Testing Guide
1. Run `basicpp-console.exe`.
2. `PSTORE "TEST", 123`.
3. `PRINT PRETRIEVE("TEST")`.
4. Verify 123 is printed.
