...
10020 CONST SS_STARS = 80
10030 CONST SS_W = 80
10040 CONST SS_H = 25
10050 CONST SS_CX = 40
10060 CONST SS_CY = 12
10070 CONST SS_DEPTH = 32
10080 CONST SS_SPEED = 0.5
10090 CONST SS_FPS = 50
10100 REM
10110 REM --- Star arrays ---
10120 DIM SX(SS_STARS), SY(SS_STARS), SZ(SS_STARS)
10130 DIM SOX(SS_STARS), SOY(SS_STARS)
...
10190 DIM SS_BUF(2100)
10200 SCREENSAVE 1, 1, 80, 25, SS_BUF()
10210 PRINT CHR$(27);"[?25l";
10220 CLS
10230 REM
10240 REM --- Initialize stars ---
10250 RANDOMIZE TIMER
10260 FOR I = 1 TO SS_STARS
10270   SX(I) = (RND(1) * 40) - 20
10280   SY(I) = (RND(1) * 24) - 12
10290   SZ(I) = RND(1) * SS_DEPTH + 1
...
10340 GFT = TICKS
10350 REM
10360 REM SCREEN LOCK (stub)
10370 REM
10380 FOR I = 1 TO SS_STARS
10390   REM --- Erase old position ---
10400   IF SOX(I) >= 1 AND SOX(I) <= SS_W THEN
10410     IF SOY(I) >= 1 AND SOY(I) <= SS_H THEN
...
10590   PX = INT(SX(I) * 16 / SZ(I) + SS_CX)
10600   PY = INT(SY(I) * 8 / SZ(I) + SS_CY)
10610   REM
...
10740   IF PX >= 1 AND PX <= SS_W THEN
10750     IF PY >= 1 AND PY <= SS_H THEN
10760       LOCATE PY, PX
10770       COLOR CC, 0
10780       PRINT CHR$(CH);
10790     END IF
...
10930 GT2 = TICKS
10940 GT1 = GT2 - GFT
10950 IF GT1 < SS_FPS THEN DELAY SS_FPS - GT1
10955 GFT = TICKS
10956 GOTO 10340
10960 REM
10970 REM --- Dismiss: restore screen ---
10980 SCREENRESTORE SS_BUF()
10990 PRINT CHR$(27);"[?25h";
11000 RETURN
11001 REM
11999 REM =============================================
