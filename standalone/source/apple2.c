/*
 * PROJECT:  Apple II Integer BASIC Interpreter (C89 Port)
 * FILENAME: apple2.c
 * VERSION:  2.1.0
 *
 * Prototype: based on Apple II Integer BASIC (Steve Wozniak, 1977)
 *
 * DESCRIPTION:
 *   A portable interpreter for the Apple II Integer BASIC dialect.
 *   Uses 16-bit signed integer arithmetic with standard operator
 *   precedence. Supports multi-character variable names, DIM'd
 *   integer arrays, DIM'd string variables with slice syntax,
 *   and logical operators AND/OR/NOT.
 *
 * DIALECT NOTES (Apple II Integer BASIC):
 *   - Variables: multi-character names (first 2 chars significant)
 *   - Statements: PRINT, LET, INPUT, GOTO, IF/THEN, REM,
 *     END, DIM, ON...GOTO, BEEP
 *   - Commands: RUN, LIST, NEW, SAVE, LOAD, HELP, BYE,
 *     DEL, CLR, CON, AUTO, MAN
 *   - Functions: ABS(), SGN(), RND(), ASC(), LEN(), TAB()
 ...
 *
 * HOW TO COMPILE:
 *   MSVC:  cl /TC /W4 /WX /Za /O2 /D_CRT_SECURE_NO_WARNINGS apple2.c
 *   GCC:   gcc -ansi -Wall -Wextra -O2 -o apple2 apple2.c
 */
 ...
 *   Do not add C99 or POSIX headers.  Strict C89 only.
 *
 * WHAT TO EXPECT:
 *   <time.h> is used for srand(time(NULL)) at startup.
 *   <ctype.h> functions must always receive (unsigned char) casts.
 *
 * IF SOMETHING BREAKS:
 *   Verify all six includes are present and compiler is in C89 mode.
 * ========================================================================= */
