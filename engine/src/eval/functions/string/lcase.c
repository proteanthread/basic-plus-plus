/**
 * @file lcase.c
 * @brief LCASE$ lowercase conversion function evaluator implementation for BASIC++.
 *
 * 1. WHAT IT DOES:
 * Implements LCASE$ built-in string function, returning a copy of input string `str$` with all uppercase ASCII letters converted to lowercase.
 *
 * 2. WHY IT EXISTS:
 * Provides standard case conversion matching GW-BASIC, QBASIC, ANSI BASIC, and ECMA-116 standards.
 *
 * 3. WHY IT WORKS THIS WAY:
 * Validates single string argument, iterates through characters applying `tolower()`, allocates a new `BppString`, and releases the input string operand `str_release()`.
 *
 * 4. DEPENDENCIES & COMPILATION:
 * Compiled into CMake micro-library target 'func_lcase'. Includes "eval/functions/string/lcase.h",
 ...
 * Support Unicode UTF-8 lowercase transformations.
 *
 * 7. WHAT CANNOT BE CHANGED:
 * Mandatory string release requirement on input string operand `args[0].as.string`.
 *
 * 8. WHAT TO EXPECT:
 * Returns VAL_STRING BValue containing lowercase string or ERR_TYPE_MISMATCH (error 13).
 *
 ...
 * Strict C17 compliance. Standard ISO C `<ctype.h>` `tolower()`.
 *
 * 12. COMPONENT DEPENDENCIES & PREREQUISITE SOURCE FILES:
 * Prerequisite Source Files:
 * - engine/src/runtime/strings.c
 * Prerequisite Header Files:
 * - engine/include/eval/functions/string/lcase.h
