...
static const LangDesc g_rol_desc = {
    .name = "ROL",
...
BValue func_rol_eval(VMContext *vm, const char *uname, int arg_count, BValue *args, BppError *err) {
    ...
    if (runtime_strcmp(uname, "_ROL") != 0 && runtime_strcmp(uname, "ROL") != 0 &&
        runtime_strcmp(uname, "BITS.ROL") != 0 && runtime_strcmp(uname, "ROTL") != 0) {
        return res;
    }

    if (arg_count < 2 || arg_count > 3 || args[0].type == VAL_STRING || args[1].type == VAL_STRING) {
        err->code = 13;
        err->message = "ROL expects two or three numeric arguments";
        return res;
    }
