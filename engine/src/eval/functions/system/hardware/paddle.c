...
// NEEDS: libengine (paddle.h)
// Provides runtime implementation for the PADDLE function in BASIC++.

#include "eval/functions/system/hardware/paddle.h"
#include "runtime/language_descriptor.h"

static const LangDesc g_paddle_desc = {
    .name = "PADDLE",
    .category = "System & Hardware",
    .syntax = "PADDLE(port%)",
    .description = "Reads analog paddle controller position (0..7; autodetects hardware or falls back to mouse X/Y/Wheel).",
...
void func_paddle_register(void) {
    lang_desc_register(&g_paddle_desc);
}
