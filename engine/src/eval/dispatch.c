/**
 * @file dispatch.c
 * @brief Domain Dispatch Registry and built-in function execution dispatcher for BASIC++.
 *
 * 1. WHAT IT DOES:
 * Implements function dispatch routing (`eval_dispatch_func()`), function lookup table registration (`eval_dispatch_init()`), domain routing (DEF FN, DEF STR, DEF GFX), and built-in function execution.
 *
 * 2. WHY IT EXISTS:
 * Routes all built-in function calls (math, string, system, bitwise) and user-defined functions through a unified, high-performance dispatch registry.
 *
 * 3. WHY IT WORKS THIS WAY:
 * Searches registered FunctionEntry tables using length-checked bounded string matching; verifies argument counts and parameter types before invoking C function handlers.
 *
 * 4. DEPENDENCIES & COMPILATION:
 * Compiled into CMake micro-library target 'eval'. Includes "eval/eval.h", "eval/dispatch.h",
 * "runtime/funcreg.h", "vm/vm.h", <string.h>, <math.h>, <stdlib.h>.
 *
 * 5. EDITION INCLUSION & EXCLUSION:
 * Core feature included in all editions ('baspp', 'bpp', 'bs').
 *
 * 6. HOW TO MODIFY OR EXTEND IT:
 * Register new built-in function categories or custom function dispatch targets.
 *
 * 7. WHAT CANNOT BE CHANGED:
 * Function signature `BValue eval_dispatch_func(VMContext *vm, const char *name, size_t len, BValue *args, int argc)` and argument type conversion invariants.
 *
 * 8. WHAT TO EXPECT:
 * Returns BValue result or ERR_UNDEFINED_FUNCTION / ERR_WRONG_NUMBER_OF_ARGUMENTS.
 *
 * 9. WHAT TO DO IF SOMETHING BREAKS:
 * Check function name length parameter (`len`) and function table registration order.
 *
 * 10. ASSUMPTIONS & PRECONDITIONS:
 * Valid initialized VMContext pointer and args array matching `argc`.
 *
 * 11. PORTABILITY & C17 CONCERNS:
 * Strict C17 compliance. Case-insensitive bounded comparison (`strncasecmp`).
 *
...
bool eval_is_builtin_function(const char *name) {
    ...
    if (i > 0 && uname[i - 1] != '$' && i < 62) {
        ...
        if (strcmp(test_name, "CHR$") == 0 ||
            ...
            strcmp(test_name, "PACK$") == 0 ||
            strcmp(test_name, "MICROPLEX$") == 0 ||
