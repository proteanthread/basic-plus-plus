...
/**
 * @file vi.c
 * @brief VI / Vim-compatible modal text editor subsystem implementation.
 *
 * 1. WHAT IT DOES:
 *    Implements VI modal text editing (`vi_run()`) inside the BASIC++ TUI editor framework:
 *    - Command Mode: `h`/`j`/`k`/`l` cursor navigation, `x` delete char, `dd` delete line, `yy` yank, `p` paste, `:w`, `:q`, `/` search.
 *    - Insert Mode: `i` (insert before cursor), `a` (append after cursor), `o` (open new line below), `Esc` (return to command mode).
 *    - Visual Mode: `v` character visual selection, `V` line visual selection.
 *
 * 2. WHY IT EXISTS:
 *    Provides developer productivity for users accustomed to modal Unix/Vim keybindings directly inside the BASIC++ REPL and editor environments.
 *
 * 3. WHY IT WORKS THIS WAY:
 *    Maintains a modal state machine (`VI_MODE_COMMAND`, `VI_MODE_INSERT`, `VI_MODE_VISUAL`). Input keystrokes are processed
 *    character-by-character through `vcon`, rendering status bar line counts and mode badges at the bottom row.
 *
 ...
 *    - Excluded from `bpp` and `bs` (Lite and Script Runner editions) to reduce binary memory footprint.
 *
 * 6. HOW TO MODIFY OR EXTEND IT:
 *    - To add new VI commands (e.g. `dw`, `cw`, `u` undo): extend the command key dispatch matrix in `vi_process_command()`.
 *    - To alter status bar color attributes or mode badges (e.g. `-- INSERT --`): modify `vi_render_status()`.
 *    - To customize key repeat or key mapping macros: adjust definitions in `editor.h`.
 *
 * 7. WHAT CANNOT BE CHANGED:
 *    - Public API function signature `BppError vi_run(EditBuffer *buf, VConContext *vcon)`.
 *    - Standard VI command key mappings (`h`, `j`, `k`, `l`, `Esc`, `:w`, `:q`).
 *    - In-place buffer manipulation model operating on `EditBuffer`.
 *
 * 8. WHAT TO EXPECT:
 *    - Returns `ERR_NONE` when exiting VI mode via `:q` or `:wq`.
 *    - Displays prompt output on the bottom line of the console context.
 *
 * 9. WHAT TO DO IF SOMETHING BREAKS:
 *    - Check active modal state variable (`vi_state->mode`).
 *    - Verify key code translation from ANSI escape sequences in `vcon_read_key()`.
 *    - Inspect buffer boundaries when performing multi-line deletions (`dd`).
 *
 * 10. ASSUMPTIONS & PRECONDITIONS:
 *     - `EditBuffer` must be allocated and valid.
 *     - `VConContext` must have initialized screen matrix bounds.
 *
 * 11. PORTABILITY & C17 CONCERNS:
 *     - Strict C17 compliance (`-std=c17`).
 *     - Uses 7-bit ASCII key matching with cross-platform terminal key mapping via `vcon`.
 *
 * 12. COMPONENT DEPENDENCIES & PREREQUISITE SOURCE FILES:
 *     - Prerequisite C Source Files: `engine/src/editor/edit_buffer.c` (line buffer memory core), `engine/src/device/vcon.c` (virtual console matrix & mode rendering), `engine/lib/platform/plat_console.c` (raw terminal key reads).
