...
/**
 * @file ws.c
 * @brief WordStar-compatible text editor subsystem implementation.
 *
 * 1. WHAT IT DOES:
 *    Implements classic CP/M and MS-DOS WordStar control-key text editing (`ws_run()`):
 *    - Diamond Navigation: `Ctrl+S` (char left), `Ctrl+D` (char right), `Ctrl+E` (line up), `Ctrl+X` (line down), `Ctrl+A` (word left), `Ctrl+F` (word right).
 *    - Block Editing (`Ctrl+K` prefix): `Ctrl+KB` (mark block start), `Ctrl+KK` (mark block end), `Ctrl+KC` (copy block), `Ctrl+KV` (move block), `Ctrl+KY` (delete block).
 *    - File Commands: `Ctrl+KD` (save file), `Ctrl+KX` (save and exit editor), `Ctrl+KQ` (abandon changes and exit).
 *
 * 2. WHY IT EXISTS:
 *    Provides nostalgic, high-efficiency WordStar diamond control-key bindings for CP/M retro-computing enthusiasts and legacy BASIC programmers.
 *
 * 3. WHY IT WORKS THIS WAY:
 *    Uses a two-phase key interpreter state machine (handling `Ctrl+K` and `Ctrl+Q` prefixes). Directly modifies `EditBuffer` lines while tracking block start/end markers.
 *
 ...
 *    - Excluded from `bpp` and `bs` (Lite and Script Runner editions) to reduce binary size.
 *
 * 6. HOW TO MODIFY OR EXTEND IT:
 *    - To add WordStar `Ctrl+Q` prefix commands (e.g. `Ctrl+QS` move to line start, `Ctrl+QD` move to line end): extend `ws_process_q_prefix()`.
 *    - To alter block selection highlight attributes: update `ws_render_line()`.
 *
 * 7. WHAT CANNOT BE CHANGED:
 *    - Public API function signature `BppError ws_run(EditBuffer *buf, VConContext *vcon)`.
 *    - WordStar control-key diamond mapping conventions (`Ctrl+S`/`D`/`E`/`X`).
 *
 * 8. WHAT TO EXPECT:
 *    - Returns `ERR_NONE` on `Ctrl+KX` or `Ctrl+KQ`.
 *    - Updates block markers dynamically during edit operations.
 *
 * 9. WHAT TO DO IF SOMETHING BREAKS:
 *    - Inspect prefix key buffer state (`ws_state->prefix_key`).
 *    - Check block start (`block_start_line`) and end (`block_end_line`) pointer bounds against `buf->line_count`.
 *
 * 10. ASSUMPTIONS & PRECONDITIONS:
 *     - `EditBuffer` must be allocated and valid.
 *     - `VConContext` must be initialized for virtual console output routing.
 *
 * 11. PORTABILITY & C17 CONCERNS:
 *     - Strict C17 compliance (`-std=c17`).
 *     - Pure 7-bit ASCII control character mapping (`0x01`..`0x1A`).
 *
 * 12. COMPONENT DEPENDENCIES & PREREQUISITE SOURCE FILES:
 *     - Prerequisite C Source Files: `engine/src/editor/edit_buffer.c` (line buffer memory core), `engine/src/device/vcon.c` (virtual console output & status rendering), `engine/lib/platform/plat_console.c` (control key reads).
...
#define get_terminal_size ws_get_terminal_size
#define reset_term ws_reset_term
#define init_term ws_init_term
#define get_input ws_get_input
#ifndef STANDALONE_EDITOR
#define main mod_ws_main
#endif
#ifdef _MSC_VER
...
static void ws_debug_hook(struct VMContext *vm, const char *event_type, int line_num, const char *symbol, void *user_data) {
    ...
    if (should_pause) {
        ...
        char prefix[64]; snprintf(prefix, sizeof(prefix), "%d", line_num);
        for (int i = 0; i < num_lines; i++) {
            if (strncmp(text_buffer[i].text, prefix, strlen(prefix)) == 0) {
                if (text_buffer[i].text[strlen(prefix)] == ' ') { cy = i; break; }
            }
        }
        cx = 0;
        refresh_screen();
        ...
        int key;
        do { key = get_input(); } while (key != ' ' && key != 13 && key != 27 && key != 10);
        if (key == 13 || key == 10 || key == 27) debug_step_mode = false; /* Enter/Esc to continue */
        else if (key == ' ') debug_step_mode = true; /* Space to step */
...
void execute_program_ws(int exec_mode) {
#ifdef STANDALONE_EDITOR
    ...
    if (exec_mode > 0) {
        vm_set_debug_hook(current_vm, ws_debug_hook, NULL);
    } else {
...
static void refresh_screen(void) {
    ...
    int ll = (int)(int)strlen(status_bar);
    ...
    while (running) {
        ...
        if (prefix_k) {
            if (c == 8 || c == 'h' || c == 'H') {         
                ...
                execute_program_ws(0);
            } else if (c == 4 || c == 'd' || c == 'D') {  /* ^K^D */
                execute_program_ws(1);
            } else if (c == 20 || c == 't' || c == 'T') { /* ^K^T */
                execute_program_ws(2);
            }
            prefix_k = false;
            continue;
        }
        ...
        switch (c) {
            ...
            case KEY_RIGHT:
                ...
                break;
            case KEY_PGUP:
                cy -= (screen_rows - (help_active ? 5 : 1));
                if (cy < 0) cy = 0;
        ...
        visible_rows = screen_rows - (help_active ? 6 : 1);
