...
/**
 * @file events.c
 * @brief VM asynchronous event loop, event trapping (ON KEY, ON TIMER, ON SIGNAL), and interrupt dispatch for BASIC++.
 *
 * 1. WHAT IT DOES:
 * Implements `vm_events_poll()`, `vm_on_event()`, `vm_event_enable()`, `vm_event_disable()`, and event dispatch mechanisms.
 *
 * 2. WHY IT EXISTS:
 * Provides GW-BASIC and QBASIC event trapping parity (ON KEY(n) GOSUB, ON TIMER(n) GOSUB, ON SIGNAL GOSUB, etc.) without blocking execution.
 *
 * 3. WHY IT WORKS THIS WAY:
 * Polls registered event sources during each VM execution cycle step in `exec.c`, verifies enabled event status, and pushes subroutine return context when an event fires.
 *
 * 4. DEPENDENCIES & COMPILATION:
 * Compiled into CMake library targets 'libbasicpp' and 'libbasicpp_lite'. Includes "vm/vm.h", "vm_internal.h",
 * "platform/platform.h", "device/vdev.h", <stdio.h>, <string.h>.
 *
 * 5. EDITION INCLUSION & EXCLUSION:
 * Included in all editions ('baspp', 'bpp', 'bs').
 *
 * 6. HOW TO MODIFY OR EXTEND IT:
 * Register new custom event triggers (e.g. ON SOCKET, ON GPIO, ON MIDI).
 *
 * 7. WHAT CANNOT BE CHANGED:
 * Non-blocking event polling contract and atomic event handler stack frame creation.
 *
 * 8. WHAT TO EXPECT:
 * `vm_events_poll()` evaluates active event conditions and returns true if an event subroutine branch was triggered.
 *
 * 9. WHAT TO DO IF SOMETHING BREAKS:
 * Check event enablement flags (`vm->events_enabled`) and event registration structures.
 *
 * 10. ASSUMPTIONS & PRECONDITIONS:
 * Valid active `VMContext` with enabled event subsystem.
 *
 * 11. PORTABILITY & C17 CONCERNS:
 * Strict C17 compliance. Thread-safe lock-free event queue structure for signal handoff.
 *
