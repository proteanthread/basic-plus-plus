...
void register_core_statements(VMContext *vm) {
    ...
    stmt_register(vm->stmt_reg, KW_COM,    stmt_com_handler,    "COM",    STMT_FLAG_BOTH);
...
void bios_write_mem_cb(void *user_data, uint32_t addr, uint8_t val) {
    VMContext *vm = (VMContext *)user_data;
    bool intercepted = false;
...
void bios_vdev_sleep_cb(void *user_data, int ms) {
    (void)user_data;
    if (ms > 0) {
...
int bios_vdev_ioctl_cb(void *user_data, int cmd, void *arg) {
    VMContext *vm = (VMContext *)user_data;
    if (!vm) return 0;
...
void bios_get_registers_cb(void *user_data, uint32_t *ax, uint32_t *bx, uint32_t *cx, uint32_t *dx, uint32_t *flags) {
    VMContext *vm = (VMContext *)user_data;
    if (vm) {
        *ax = vm->regs.ax;
        *bx = vm->regs.bx;
...
void bios_set_registers_cb(void *user_data, uint32_t ax, uint32_t bx, uint32_t cx, uint32_t dx, uint32_t flags) {
    VMContext *vm = (VMContext *)user_data;
    if (vm) {
        vm->regs.ax = ax;
        vm->regs.bx = bx;
