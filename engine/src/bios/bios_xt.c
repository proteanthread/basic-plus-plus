...
/**
 * @file bios_xt.c
 * @brief IBM PC/XT (8088) BIOS services, hard disk controller, and cassette tape interface support for BASIC++.
 *
 * 1. WHAT IT DOES:
 * Implements IBM PC/XT BIOS routines (`bios_xt_int13()` fixed disk, `bios_xt_int15()` cassette I/O).
 *
 * 2. WHY IT EXISTS:
 * Provides IBM PC/XT (5160) hardware parity for GW-BASIC 2.0+ and IBM Cassette BASIC.
 *
 * 3. WHY IT WORKS THIS WAY:
 * Maps fixed disk parameters and cassette tape read/write routines (`MOTOR`, `BSAVE`, `BLOAD`) to virtual disk images and Virtual VFS buffers.
 *
 * 4. DEPENDENCIES & COMPILATION:
 * Compiled into CMake micro-library target 'libbios'. Includes "bios/bios.h", "bios/bios_xt.h", <string.h>.
 *
 * 5. EDITION INCLUSION & EXCLUSION:
 * Included in `libbasicpp` ('baspp').
 *
 * 6. HOW TO MODIFY OR EXTEND IT:
 * Support floppy drive sector read/write emulation via disk images (`.img`, `.dsk`).
 *
 * 7. WHAT CANNOT BE CHANGED:
 * XT fixed disk parameter table structure layout.
 *
 * 8. WHAT TO EXPECT:
 * `bios_xt_int13()` handles fixed disk BIOS calls.
 *
 * 9. WHAT TO DO IF SOMETHING BREAKS:
 * Check disk sector offset calculation `(cylinder * heads + head) * sectors + sector - 1`.
 *
 * 10. ASSUMPTIONS & PRECONDITIONS:
 * Valid `BiosContext` pointer with `BIOS_MODEL_XT`.
 *
 * 11. PORTABILITY & C17 CONCERNS:
 * Strict C17 compliance. Virtualized disk block I/O avoids host platform OS dependency.
 *
...
/**
 * @file bios_xt.c
 * @brief IBM XT 5160 authentic BIOS implementation with EDD 3.0 / GPT Disk Support.
 *
 * SECTION 1: WHAT IT DOES, WHY IT EXISTS, AND WHY IT WORKS THIS WAY
 * - What it does: Implements IBM XT 5160 hardware extensions, fixed disk controller
 *   handling, EDD 3.0 LBA48/64 GPT partition translation, INT 13h services, and ROM
 *   release date "08/16/82" at offset 0xFFFF5.
 */
