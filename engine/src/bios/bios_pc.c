...
/**
 * @file bios_pc.c
 * @brief IBM PC 5150 BIOS interrupt vector table and hardware port emulation implementation.
 *
 * 1. WHAT IT DOES:
 *    Implements IBM PC 5150 BIOS interrupt calls (`bios_pc_int10()` video, `bios_pc_int16()` keyboard, `bios_pc_int21()` DOS services),
 *    emulating 8086 registers (`AX`, `BX`, `CX`, `DX`, `SI`, `DI`, `FLAGS`) and BDA (BIOS Data Area at segment `0x0040`).
 *
 * 2. WHY IT EXISTS:
 *    Provides backward compatibility for low-level GW-BASIC and QuickBASIC programs executing `CALL INTERRUPT` or `DEF SEG = &H40`
 *    and accessing direct hardware vectors without requiring real 16-bit real-mode hardware.
 *
 * 3. WHY IT WORKS THIS WAY:
 *    Maps 16-bit register structures (`BppRegisters`) to internal VCon virtual console cursor and memory matrices.
 *
 * 4. DEPENDENCIES & COMPILATION:
 *    - Required Headers: `bios/bios.h`, `bios/bios_types.h`, `types/errors.h`
 *    - CMake Target: Part of `bios` micro-library target in `engine/CMakeLists.txt`.
 *
 * 5. EDITION INCLUSION & EXCLUSION:
 *    - Included in `baspp` (Standard Desktop Edition).
 *    - Excluded from `bpp` and `bs` (Lite and Script Runner editions).
 *
 * 6. HOW TO MODIFY OR EXTEND IT:
 *    - To add new BIOS interrupt handlers (e.g. `INT 13h` disk services): extend `bios_pc_dispatch_int()`.
 *    - To modify video mode 03h/07h memory locations: update BDA segment mappings in `bios_pc_bda_read()`.
 *
 * 7. WHAT CANNOT BE CHANGED:
 *    - 8086 Register layout order in `BppRegisters`.
 *    - 1-to-1 mapping of standard IBM PC interrupt numbers (`0x10`, `0x16`, `0x21`).
 *
 * 8. WHAT TO EXPECT:
 *    - Modifies `BppRegisters` passed by address. Returns `ERR_NONE` on handled interrupt.
 *
 * 9. WHAT TO DO IF SOMETHING BREAKS:
 *    - Verify register high/low byte manipulation (`AH`/`AL` inside `AX`).
 *    - Trace interrupt vector dispatch in `bios_pc_int10()`.
 *
 * 10. ASSUMPTIONS & PRECONDITIONS:
 *     - Register pointer parameters must be non-NULL.
 *
 * 11. PORTABILITY & C17 CONCERNS:
 *     - Strict C17 compliance (`-std=c17`).
 *     - Explicit byte-order handling for 16-bit word extraction.
 *
 * 12. COMPONENT DEPENDENCIES & PREREQUISITE SOURCE FILES:
 *     - Prerequisite C Source Files: `engine/src/bios/bios.c` (BIOS port routing), `engine/src/device/vcon.c` (virtual console matrix rendering).
 *     - Prerequisite Header Surfaces: `engine/include/bios/bios.h`, `engine/include/bios/bios_types.h`, `engine/include/device/vcon.h`, `engine/include/types/errors.h`.
 */
...
/**
 * @file bios_pc.c
 * @brief IBM PC 5150 authentic BIOS implementation.
 *
 * SECTION 1: WHAT IT DOES, WHY IT EXISTS, AND WHY IT WORKS THIS WAY
 * - What it does: Implements authentic IBM PC 5150 hardware structures, BIOS Data Area
 *   initialization (0x400), 8254 PIT (0x40-0x43), 8259 PIC (0x20-0x21), 6845 CRTC (0x3D4/0x3D5),
 *   INT 10h (Video), INT 11h (Equipment), INT 12h (RAM Size), INT 16h (Keyboard), INT 1Ah (Timer),
 *   and ROM release date "04/24/81" at offset 0xFFFF5.
 * - Why it exists: Provides 100% accurate PC 5150 BIOS memory maps and hardware interrupt services.
 */
