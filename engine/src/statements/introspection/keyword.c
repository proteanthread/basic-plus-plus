...
#include "statements/introspection/keyword.h"
...
static const LangDesc g_keyword_desc = {
    .name = "KEYWORD",
...
void stmt_keyword_register(void) {
    lang_desc_register(&g_keyword_desc);
}
...
BppError stmt_keyword_handler(VMContext *vm, LexerContext *lex) {
    ...
    if (runtime_strcasecmp(action, "GET") == 0) {
        ...
        if (val) {
            ...
            vdev_printf(vm_get_vdev(vm), "%s.%s is not set.\n", target_kw, prop_name);
        }
        return err;
    }
    ...
    if (runtime_strcasecmp(action, "SET") == 0) {
        ...
        if (eq_tok.type != TOK_EQ) {
            err.code = 2;
            err.message = "Expected '=' after property name in KEYWORD SET";
            return err;
        }

        BppToken val_tok = lex_next(lex);
        if (val_tok.type != TOK_IDENT && val_tok.type != TOK_STRING && val_tok.type != TOK_NUMBER) {
            err.code = 2;
            err.message = "Expected property value after '=' in KEYWORD SET";
            return err;
        }
        char prop_val[256];
