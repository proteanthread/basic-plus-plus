...
#include "statements/introspection/selftest.h"
#include "stmt/stmt.h"
...
void stmt_selftest_register(void) {
    lang_desc_register(&g_selftest_desc);
}
...
BppError stmt_selftest_handler(VMContext *vm, LexerContext *lex) {
    ...
    // Test 4: Variable Storage Subsystem
    {
        ...
        if (!v2 || v2->type != VAL_INTEGER || v2->as.number != 42.0) {
            err.code = 4003; err.message = "Variable: Value mismatch"; return err;
        }
        vdev_puts(vdev, "  Variable check: PASS\n");
    }

    // Test 5: Array Subsystem
    {
        ArrayContext *arr = vm_get_arr(vm);
        int dims[2] = { 5, 5 };
        BppError aerr = arr_dim(arr, "TESTARR%", 2, dims);
        if (aerr.code != 0) {
            err.code = 5001; err.message = "Array: Allocation failed"; return err;
        }
        arr_set_type(arr, "TESTARR%", VAL_INTEGER);
        int indices[2] = { 2, 3 };
        BValue *elem = arr_get_element(arr, "TESTARR%", 2, indices, &err);
        if (!elem) {
            err.code = 5002; err.message = "Array: Element lookup failed"; return err;
        }
