...
#include "statements/filesystem/binary_ops/stmt_qload.h"
...
static const LangDesc g_qload_desc = {
    .name = "QLOAD",
...
void stmt_qload_register(void) {
    lang_desc_register(&g_qload_desc);
}
...
BppError stmt_qload_handler(VMContext *vm, LexerContext *lex) {
    ...
    if (lex_peek(lex).type == TOK_COMMA) {
        ...
        if (lex_peek(lex).type != TOK_LBRACE && lex_peek(lex).type != TOK_LBRACKET) {
            ...
            user_offset = (uint64_t)off_val.as.number;
    ...
    if (read_bytes < 7) {
        ...
        err.message = "File too short for binary package or memory image";
        return err;
    }
    ...
    if (qheader_is_modern(peek_buf, read_bytes)) {
        ...
        length = hdr->block_length;
        target_offset = has_user_offset ? user_offset : hdr->offset_address;
        if (!has_user_offset && hdr->segment_address != 0) {
        ...
        // Read payload via zero-malloc stack chunk streaming (512-byte sector buffer)
        if (length > 0) {
            uint8_t chunk[512];
            ...
            while (remaining > 0) {
                ...
                if (actual_read < to_read) {
                    ...
                    str_release(vm_get_str(vm), fn_val.as.string);
                    err.code = ERR_BAD_FILE_MODE;
                    err.message = "Truncated payload in binary package";
                    return err;
                }

                raw_crc = qheader_crc32_update(raw_crc, chunk, to_read);

                // Write directly into virtual memory / BIOS / VMem
            ...
            // CRC-32 Validation if checksum was recorded
            if (hdr->crc32 != 0) {
                ...
                if (calc_crc != hdr->crc32) {
                    ...
                    str_release(vm_get_str(vm), fn_val.as.string);
                    err.code = ERR_BAD_FILE_MODE;
                    err.message = "Corrupted binary package: CRC32 checksum mismatch";
                    return err;
                }
            }
        }
    } else if (qheader_is_vintage(peek_buf, read_bytes)) {
        // Super-loader fallback: seamlessly load vintage 7-byte BSAVE format
        uint16_t hdr_offset = (uint16_t)(peek_buf[3] | (peek_buf[4] << 8));
        length = (uint64_t)(peek_buf[5] | (peek_buf[6] << 8));
        target_offset = has_user_offset ? user_offset : (uint64_t)hdr_offset;

        // Position file pointer directly after 7-byte header
        ...
        str_release(vm_get_str(vm), fn_val.as.string);
        err.code = ERR_BAD_FILE_MODE;
        err.message = "Unrecognized binary header format";
        return err;
    }
