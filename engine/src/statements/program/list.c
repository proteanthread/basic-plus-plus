...
static const LangDesc g_list_desc = {
    .name = "LIST",
...
BppError stmt_list_handler(VMContext *vm, LexerContext *lex) {
    ...
    if (tok.type == TOK_STRING) {
        ...
        if (tok.as.string && tok.length > 0) {
            runtime_snprintf(search_pattern, sizeof(search_pattern), "%.*s", (int)tok.length, tok.as.string);
        }
    ...
    if (tok.type == TOK_COMMA) {
        ...
        if (search_pattern[0] == '\0' && (runtime_strstr(val_str, ".") != NULL || runtime_strchr(val_str, '/') != NULL || runtime_strchr(val_str, '\\') != NULL)) {
            runtime_snprintf(out_filename, sizeof(out_filename), "%s", val_str);
        } else {
            runtime_snprintf(search_pattern, sizeof(search_pattern), "%s", val_str);
        }
        str_release(vm_get_str(vm), fn_val.as.string);
    }
    ...
    void *fp = NULL;
    if (out_filename[0] != '\0') {
        fp = platform_file_open(out_filename, "w");
        if (!fp) {
    ...
    for (size_t i = 0; i < count; i++) {
        if (lines[i].line_number >= start_line && lines[i].line_number <= end_line) {
            ...
            if (search_pattern[0] != '\0') {
                ...
                bool found = false;
                size_t text_len = runtime_strlen(text);
                size_t pat_len = runtime_strlen(search_pattern);
                if (pat_len <= text_len) {
                    for (size_t k = 0; k <= text_len - pat_len; k++) {
                        if (runtime_strncasecmp(&text[k], search_pattern, pat_len) == 0) {
                            found = true;
                            break;
                        }
                    }
            ...
            char buf[1024];
            runtime_snprintf(buf, sizeof(buf), "%lld %s\n", (long long)lines[i].line_number, text);
            if (fp) {
                platform_file_write(fp, buf, runtime_strlen(buf));
            } else {
                vdev_puts(vd, buf);
            }
        }
...
void stmt_list_register(void) {
    lang_desc_register(&g_list_desc);
}
