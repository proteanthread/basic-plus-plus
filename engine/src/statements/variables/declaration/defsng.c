...
#include "statements/variables/declaration/defsng.h"
...
void stmt_defsng_register(void) {
    MicroLibMetadata meta = {
        ...
        .help_text = "Sets default type of variables beginning with specified letters to single-precision float.",
...
BppError stmt_defsng_handler(VMContext *vm, LexerContext *lex) {
    ...
    while (true) {
        ...
        if (tok.type != TOK_IDENT) {
            err.code = 2; err.message = "Expected letter in DEFSNG type range";
            return err;
        }
        ...
        if (next.type == TOK_MINUS) {
            ...
            if (end_tok.type != TOK_IDENT) {
                err.code = 2; err.message = "Expected end letter in DEFSNG range";
                return err;
            }
