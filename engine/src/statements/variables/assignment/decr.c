...
#include "statements/variables/assignment/decr.h"
...
static const LangDesc g_decr_desc = {
    .name = "DECR",
...
void stmt_decr_register(void) {
    lang_desc_register(&g_decr_desc);
}
...
BppError stmt_decr_handler(VMContext *vm, LexerContext *lex) {
    ...
    if (tok.type != TOK_IDENT && tok.type != TOK_KEYWORD) {
        err.code = 2;
        err.message = "Syntax error in DECR (expected variable name)";
        return err;
    }
    ...
    if (is_array) {
        ...
        if (arr_elem->type == VAL_STRING) {
            err.code = 13; err.message = "Type mismatch (cannot DECR string)";
            return err;
        }
