...
static const LangDesc g_end_desc = {
    .name = "END",
    ...
    .error_summary = "Error 2: Syntax error (mismatched block terminator)",
...
BppError stmt_end_handler(VMContext *vm, LexerContext *lex) {
    ...
    if (tok.type == TOK_KEYWORD || tok.type == TOK_IDENT) {
        ...
        bool is_if      = (tok.type == TOK_KEYWORD && tok.as.keyword == KW_IF) ||
                          (tok.type == TOK_IDENT && tok.length == 2 && runtime_strncasecmp(tok.start, "IF", 2) == 0);
        bool is_try     = (tok.type == TOK_KEYWORD && tok.as.keyword == KW_TRY) ||
                          (tok.type == TOK_IDENT && tok.length == 3 && runtime_strncasecmp(tok.start, "TRY", 3) == 0);
        bool is_atomic  = (tok.type == TOK_KEYWORD && tok.as.keyword == KW_ATOMIC) ||
                          (tok.type == TOK_IDENT && tok.length == 6 && runtime_strncasecmp(tok.start, "ATOMIC", 6) == 0);
        bool is_sub     = (tok.type == TOK_KEYWORD && (tok.as.keyword == KW_SUB || tok.as.keyword == KW_PROCEDURE)) ||
                          (tok.type == TOK_IDENT && tok.length == 3 && runtime_strncasecmp(tok.start, "SUB", 3) == 0);
        bool is_func    = (tok.type == TOK_KEYWORD && tok.as.keyword == KW_FUNCTION) ||
                          (tok.type == TOK_IDENT && tok.length == 8 && runtime_strncasecmp(tok.start, "FUNCTION", 8) == 0);
        bool is_type    = (tok.type == TOK_KEYWORD && (tok.as.keyword == KW_TYPE || tok.as.keyword == KW_ENUM)) ||
                          (tok.type == TOK_IDENT && tok.length == 4 && runtime_strncasecmp(tok.start, "TYPE", 4) == 0);
        bool is_with    = (tok.type == TOK_KEYWORD && tok.as.keyword == KW_WITH) ||
                          (tok.type == TOK_IDENT && tok.length == 4 && runtime_strncasecmp(tok.start, "WITH", 4) == 0);
        bool is_class   = (tok.type == TOK_KEYWORD && tok.as.keyword == KW_CLASS) ||
                          (tok.type == TOK_IDENT && tok.length == 5 && runtime_strncasecmp(tok.start, "CLASS", 5) == 0);
        bool is_record  = (tok.type == TOK_KEYWORD && tok.as.keyword == KW_RECORD) ||
                          (tok.type == TOK_IDENT && tok.length == 6 && runtime_strncasecmp(tok.start, "RECORD", 6) == 0);
        bool is_when    = (tok.type == TOK_KEYWORD && tok.as.keyword == KW_WHEN) ||
                          (tok.type == TOK_IDENT && tok.length == 4 && runtime_strncasecmp(tok.start, "WHEN", 4) == 0);
        bool is_module  = (tok.type == TOK_KEYWORD && tok.as.keyword == KW_MODULE) ||
                          (tok.type == TOK_IDENT && tok.length == 6 && runtime_strncasecmp(tok.start, "MODULE", 6) == 0);
        bool is_scope   = (tok.type == TOK_KEYWORD && tok.as.keyword == KW_SCOPE) ||
                          (tok.type == TOK_IDENT && tok.length == 5 && runtime_strncasecmp(tok.start, "SCOPE", 5) == 0);
        bool is_picture = (tok.type == TOK_KEYWORD && tok.as.keyword == KW_PICTURE) ||
                          (tok.type == TOK_IDENT && tok.length == 7 && runtime_strncasecmp(tok.start, "PICTURE", 7) == 0);
        bool is_handler = (tok.type == TOK_KEYWORD && tok.as.keyword == KW_HANDLER) ||
                          (tok.type == TOK_IDENT && tok.length == 7 && runtime_strncasecmp(tok.start, "HANDLER", 7) == 0);
