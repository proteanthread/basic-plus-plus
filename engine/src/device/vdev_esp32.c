...
static int vdev_esp32_dev_open(VDev *dev, const char *path, int mode) {
    ...
    Esp32Channel *ch = (Esp32Channel *)runtime_calloc(1, sizeof(Esp32Channel));
    if (!ch) return -1;

    const char *p = path;
    if (runtime_strncmp(p, "ESP32:", 6) == 0 || runtime_strncmp(p, "esp32:", 6) == 0) {
        p += 6;
    }
    ...
    if (runtime_strncasecmp(p, "GPIO/", 5) == 0) {
        ...
        ch->i2c_addr = (uint8_t)runtime_strtol(p + 4, NULL, 0);
...
static int vdev_esp32_dev_read(VDev *dev, void *buffer, int len) {
    ...
    if (ch->read_pos >= ch->read_len) {
        if (ch->type == ESP32_DEV_GPIO) {
            ...
            runtime_snprintf(ch->read_buffer, sizeof(ch->read_buffer), "%d\n", val);
        } else if (ch->type == ESP32_DEV_ADC) {
            int val = esp32_hal_analog_read(ch->pin);
            runtime_snprintf(ch->read_buffer, sizeof(ch->read_buffer), "%d\n", val);
        } else if (ch->type == ESP32_DEV_WIFI) {
            runtime_snprintf(ch->read_buffer, sizeof(ch->read_buffer), "%s\n", iot_wifi_get_ip());
        } else {
            runtime_snprintf(ch->read_buffer, sizeof(ch->read_buffer), "0\n");
        }
        ch->read_len = runtime_strlen(ch->read_buffer);
        ch->read_pos = 0;
    }

    size_t avail = ch->read_len - ch->read_pos;
    size_t to_copy = ((size_t)len < avail) ? (size_t)len : avail;
    runtime_memcpy(buffer, ch->read_buffer + ch->read_pos, to_copy);
    ch->read_pos += to_copy;
    return (int)to_copy;
}
...
static int vdev_esp32_dev_write(VDev *dev, const void *buffer, int len) {
    ...
    runtime_memcpy(tmp, buffer, copy_len);
    tmp[copy_len] = '\0';

    if (ch->type == ESP32_DEV_GPIO) {
        int val = runtime_atoi(tmp);
...
static int vdev_esp32_puts(VDev *dev, const char *s) {
    if (!s) return -1;
    return vdev_esp32_dev_write(dev, s, (int)runtime_strlen(s));
}
