...
#else
void vdev_gfx_beep(VDevContext *ctx) { (void)ctx; }
void vdev_play_sound_freq(double freq, double duration_seconds) { (void)freq; (void)duration_seconds; }
int vdev_music_note_count(void) { return 0; }
int vdev_music_queue_length(void) { return 0; }
void vdev_music_clear(void) {}
void play_mml(VMContext *vm, const char *mml) { (void)vm; (void)mml; }
#endif
...
int vdev_sound_open(const char *filename) {
    ...
    SDL_LockAudioDevice(g_audio_device);
    g_sound_channels[slot].samples = (float *)cvt.buf;
    g_sound_channels[slot].total_samples = cvt.len_cvt / sizeof(float);
...
int vdev_image_load(const char *filename) {
    ...
    uint32_t *pixels = (uint32_t *)calloc(converted->w * converted->h, sizeof(uint32_t));
    if (!pixels) {
    ...
    memcpy(pixels, converted->pixels, converted->w * converted->h * sizeof(uint32_t));
    g_image_slots[slot].pixels = pixels;
    g_image_slots[slot].width = converted->w;
    g_image_slots[slot].height = converted->h;
...
void vdev_image_draw(int handle, int x, int y, int w, int h) {
    ...
    for (int dy = 0; dy < h; dy++) {
        ...
        for (int dx = 0; dx < w; dx++) {
            ...
            if (a == 255) {
                g_pixels[py * g_width + px] = pixel;
            } else {
                uint32_t dst_pixel = g_pixels[py * g_width + px];
                ...
                g_pixels[py * g_width + px] = (255 << 24) | (out_r << 16) | (out_g << 8) | out_b;
            }
        }
    }
}
...
#else
int vdev_image_load(const char *filename) { (void)filename; return -1; }
int vdev_image_create(int w, int h, int mode) { (void)w; (void)h; (void)mode; return -1; }
void vdev_image_free(int handle) { (void)handle; }
int vdev_image_copy(int handle) { (void)handle; return -1; }
void vdev_image_draw(int handle, int x, int y, int w, int h) { (void)handle; (void)x; (void)y; (void)w; (void)h; }
int vdev_image_width(int handle) { (void)handle; return 0; }
int vdev_image_height(int handle) { (void)handle; return 0; }
void vdev_image_free_all(void) {}
#endif
