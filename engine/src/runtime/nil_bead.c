    ...
    switch (val.type) {
        case VAL_NUMBER: {
            ...
            if (written + sizeof(double) <= max_out) {
                runtime_memcpy(out + written, &num, sizeof(double));
        ...
        case VAL_STRING: {
            ...
            if (val.as.string) {
                ...
                if (written + 2 + u16_len <= max_out) {
                    runtime_memcpy(out + written, &u16_len, 2);
                    written += 2;
                    runtime_memcpy(out + written, data, u16_len);
                    written += u16_len;
                }
            } else {
                uint16_t u16_len = 0;
    ...
    switch (tag) {
        case NIL_BEAD_FLPNUM: {
            if (len >= 1 + sizeof(double)) {
                double num;
                runtime_memcpy(&num, buf + 1, sizeof(double));
        ...
        case NIL_BEAD_CHAR: {
            if (len >= 3) {
                uint16_t u16_len;
                runtime_memcpy(&u16_len, buf + 1, 2);
