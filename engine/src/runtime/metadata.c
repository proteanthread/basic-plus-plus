...
bool metadata_register_label(BppMetadataRegistry *reg, const char *label, const char *filename, BppLineNumber line) {
    ...
    for (int i = 0; i < reg->global_label_count; i++) {
        if (runtime_strcasecmp(reg->global_labels[i].label_name, label) == 0) {
            // Update location
...
bool metadata_register_docstring(BppMetadataRegistry *reg, const char *target, const char *docstring) {
    ...
    for (int i = 0; i < reg->docstring_count; i++) {
        if (runtime_strcasecmp(reg->docstrings[i].target_name, target) == 0) {
...
void metadata_pre_scan_line(VMContext *vm, const char *filename, BppLineNumber line_num, const char *text) {
    ...
    if (tok.type == TOK_DOCSTRING) {
        ...
        runtime_memcpy(reg->last_docstring_buffer, tok.as.string, len);
    ...
    // 2. Namespace: ::[namespace]
        ...
        runtime_memcpy(reg->current_namespace, tok.as.string, len);
    ...
    // 3. Global Label: ::label:
        ...
        if (reg->last_docstring_buffer[0] != '\0') {
            char target_key[128];
            runtime_snprintf(target_key, sizeof(target_key), "::%s", label_name);
    ...
    // 4. Procedures: SUB, FUNCTION
        ...
        if (tok.as.keyword == KW_SUB || tok.as.keyword == KW_FUNCTION) {
            ...
            if (name_tok.type == TOK_IDENT) {
                ...
                runtime_memcpy(proc_name, name_tok.as.string, len);
                proc_name[len] = '\0';

                char fully_qualified[256];
                if (reg->current_namespace[0] != '\0') {
                    runtime_snprintf(fully_qualified, sizeof(fully_qualified), "%s.%s", reg->current_namespace, proc_name);
                } else {
                    runtime_snprintf(fully_qualified, sizeof(fully_qualified), "%s", proc_name);
                }
...
bool metadata_register_block(BppMetadataRegistry *reg, const char *type, const char *target, const char *docstring, const char *body) {
    ...
    for (int i = 0; i < reg->metadata_block_count; i++) {
        if (runtime_strcasecmp(reg->metadata_blocks[i].block_type, type) == 0 &&
            runtime_strcasecmp(reg->metadata_blocks[i].target_name, target) == 0) {
            if (docstring && docstring[0] != '\0') {
                ...
                } else if (runtime_strstr(reg->metadata_blocks[i].docstring, docstring) == NULL) {
                    size_t cur_dlen = runtime_strlen(reg->metadata_blocks[i].docstring);
                    if (cur_dlen + runtime_strlen(docstring) + 3 < sizeof(reg->metadata_blocks[i].docstring)) {
                        runtime_snprintf(reg->metadata_blocks[i].docstring + cur_dlen,
            ...
            if (body && body[0] != '\0') {
                size_t cur_len = runtime_strlen(reg->metadata_blocks[i].body);
                if (cur_len > 0) {
                    if (cur_len + runtime_strlen(body) + 2 < sizeof(reg->metadata_blocks[i].body)) {
                        runtime_snprintf(reg->metadata_blocks[i].body + cur_len,
...
static bool is_block_end_marker_simple(const char *text, const char *block_type, const char *block_target, MemoryContext *mem) {
    ...
    if ((tok.type == TOK_IDENT || tok.type == TOK_KEYWORD) && tok.length == 3 && runtime_strncasecmp(tok.start, "END", 3) == 0) {
        ...
        if (next_tok.type == TOK_IDENT || next_tok.type == TOK_KEYWORD) {
            ...
            next_name[nlen] = '\0';
            if (runtime_strcasecmp(next_name, block_type) == 0) {
...
void metadata_pre_scan_program(VMContext *vm, const char *filename) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (tok.type == TOK_DIRECTIVE) {
            ...
            if (runtime_strcasecmp(dir_name, "KEYWORD") == 0 || runtime_strcasecmp(dir_name, "SCOPE") == 0 || runtime_strcasecmp(dir_name, "ALIAS") == 0 || runtime_strcasecmp(dir_name, "OPTION") == 0) {
                ...
                if (target_tok.type == TOK_IDENT || target_tok.type == TOK_KEYWORD || target_tok.type == TOK_STRING) {
                    ...
                    runtime_memcpy(current_block_target, target_tok.start, tlen);
                    current_block_target[tlen] = '\0';

                    if (target_tok.start + target_tok.length < text + runtime_strlen(text) && *(target_tok.start + target_tok.length) == ':') {
                        if (tlen + 1 < 63) {
                ...
                if (has_closing_marker) {
                    ...
                    if (doc_ptr) {
                        doc_ptr += 2;
                        while (*doc_ptr && runtime_isspace((unsigned char)*doc_ptr)) {
                            doc_ptr++;
                        }
                        size_t dlen = runtime_strlen(doc_ptr);
                        if (dlen < sizeof(current_block_docstring)) {
                            runtime_memcpy(current_block_docstring, doc_ptr, dlen);
                            current_block_docstring[dlen] = '\0';
                        } else {
                            runtime_memcpy(current_block_docstring, doc_ptr, sizeof(current_block_docstring) - 1);
        ...
        if (inside_block) {
            ...
            if ((tok.type == TOK_IDENT || tok.type == TOK_KEYWORD) && tok.length == 3 && runtime_strncasecmp(tok.start, "END", 3) == 0) {
                ...
                if (next_tok.type == TOK_IDENT || next_tok.type == TOK_KEYWORD) {
                    ...
                    runtime_memcpy(next_name, next_tok.start, nlen);
                    next_name[nlen] = '\0';
                    if (runtime_strcasecmp(next_name, current_block_type) == 0) {
            ...
            if (tok.type == TOK_IDENT || tok.type == TOK_KEYWORD || tok.type == TOK_STRING) {
                ...
                if (runtime_strcasecmp(target_buf, current_block_target) == 0) {
                    BppToken next_tok = lex_next(lex);
                    if (next_tok.type == TOK_IDENT || next_tok.type == TOK_KEYWORD) {
                        char next_name[64];
                        size_t nlen = (next_tok.length < 63) ? next_tok.length : 63;
                        ...
                        if (runtime_strcasecmp(next_name, current_block_type) == 0 && lex_peek(lex).type == TOK_DOUBLE_COLON) {
                            // Close block!
                            metadata_register_block(reg, current_block_type, current_block_target, current_block_docstring, current_block_body);
                            // Pop parent block
                            if (nest_stack_ptr > 0) {
            ...
            if (tok.type == TOK_IDENT || tok.type == TOK_KEYWORD) {
                ...
                if (runtime_strcasecmp(name_buf, current_block_type) == 0 && lex_peek(lex).type == TOK_DOUBLE_COLON) {
            ...
            if (tok.type == TOK_DOCSTRING) {
                ...
                if (current_block_docstring[0] == '\0') {
                    ...
                    runtime_memcpy(current_block_docstring, tok.as.string, dlen);
                    current_block_docstring[dlen] = '\0';
                }
            }

            // Append text line to body
            size_t current_len = runtime_strlen(current_block_body);
            size_t text_len = text ? runtime_strlen(text) : 0;
            if (text_len > 0 && current_len + text_len + 2 < MAX_BLOCK_BODY_LEN) {
                if (current_len > 0) {
                    runtime_snprintf(current_block_body + current_len, MAX_BLOCK_BODY_LEN - current_len, "\n%s", text);
                } else {
                    runtime_snprintf(current_block_body, MAX_BLOCK_BODY_LEN, "%s", text);
                }
            }
            lex_shutdown(lex);
            continue;
