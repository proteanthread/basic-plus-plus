 ...
 * - What it does: Declares the abstract BGI driver interface, video mode
 *   descriptors, the software rasterizer API, font system, and mode library.
 *   BGI is a clean-room, zero-dependency, software-defined virtual graphics
 *   subsystem inspired by the classic Borland Graphics Interface.
 * - Why it exists: Decouples graphics rendering from any specific backend
 *   (SDL2, Win32 GDI, DRM/KMS, TUI half-block). BGI renders into a software
 *   framebuffer; the host backend simply blits the result.
 * - Why it works this way: A single 32-bit ARGB8888 master framebuffer is the
 *   canonical surface. Heritage modes (CGA, VGA, NES, etc.) use indexed or
 *   planar VRAM internally, synthesized to the master canvas at flush time.
 *
 * SECTION 2: DEVELOPER MAINTENANCE & MODIFICATION GUIDE
 * - What can be changed: New mode registrations, new font tables, new drawing
 *   primitives. All follow the same driver interface pattern.
 * - What cannot be changed: The BGI_VideoMode struct layout (ABI stability),
 *   the putpixel/getpixel function pointer signatures.
 * - What to expect: BGI_init() must be called before any drawing calls.
 *   BGI_shutdown() releases all VRAM and resets state.
 * - What to do if something breaks: Verify BGI_init() was called, verify
 *   active mode handle is valid, check calloc return values.
 *
 * SECTION 3: ASSUMPTIONS & PORTABILITY CONCERNS
 * - Assumptions: Host provides calloc/free. All text is 7-bit ASCII.
 * - Portability concerns: Pure ISO C17 with no OS-specific dependencies.
 *   Font data is embedded as static const arrays.
 *
 * SECTION 4: FUTURE EXPANSIONS & EXTENSION HOOKS
 * - How future expansion can occur safely: Register new heritage modes via
 *   BGI_register_mode(). Add new font tables via bgi_font_register().
 * - How to write external extensions: Fill a BGI_VideoMode descriptor and
