...
static int usb_close(VDev *d)
{
 ...
 CloseHandle((HANDLE)st->platform_handle);
 st->platform_handle = 0;
 }
#elif defined(__linux__)
 if (st->platform_handle >= 0) {
 close((int)st->platform_handle);
...
#ifdef __linux__
 return (int)read((int)st->platform_handle, buf, (size_t)len);
#elif defined(_WIN32) || defined(_WIN64)
 {
 DWORD bytes_read = 0;
 ReadFile((HANDLE)st->platform_handle,
...
#ifdef __linux__
 return (int)write((int)st->platform_handle, buf, (size_t)len);
#elif defined(_WIN32) || defined(_WIN64)
 {
 DWORD bytes_written = 0;
 WriteFile((HANDLE)st->platform_handle,
...
static int usbser_ioctl(VDev *d, int cmd, void *arg)
{
 ...
 {
 DCB dcb;
 HANDLE h = (HANDLE)st->platform_handle;
 memset(&dcb, 0, sizeof(dcb));
 dcb.DCBlength = sizeof(dcb);
 GetCommState(h, &dcb);
...
#if defined(_WIN32) || defined(_WIN64)
 if (st->platform_handle > 0) {
 PurgeComm((HANDLE)st->platform_handle,
